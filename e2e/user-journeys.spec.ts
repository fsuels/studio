...
test.describe('User Journey Testing', () => {
  ...
  test.describe('Document Creation Journey', () => {
    test('should complete full document creation flow', async ({ page }) => {
      ...
      const documentLink = page.getByRole('link', { name: /bill of sale/i }).first();
      ...
      const createButton = page.getByRole('button', { name: /create|start|begin/i }).first();
      ...
      // Step 6: Review step
      const reviewButton = page.getByRole('button', { name: /review|preview/i });
      ...
      const completeButton = page.getByRole('button', { name: /complete|finish|generate/i });
      ...
      // Verify completion
      await expect(page.getByText(/success|completed|generated/i)).toBeVisible({ timeout: 15000 });
    });
    ...
    test('should handle form validation errors gracefully', async ({ page }) => {
      ...
      await expect(page.getByText(/required|error/i)).toBeVisible({ timeout: 5000 });
  ...
  test.describe('User Authentication Journey', () => {
    test('should complete sign up flow', async ({ page }) => {
      ...
      if (await signUpButton.isVisible()) {
        ...
        await page.waitForSelector('[data-testid*="auth"], [data-testid*="signup"], form', { timeout: 10000 });
    ...
    test('should complete sign in flow', async ({ page }) => {
      ...
      if (await signInButton.isVisible()) {
        ...
        await page.waitForSelector('[data-testid*="auth"], [data-testid*="signin"], form', { timeout: 10000 });
  ...
  test.describe('Navigation and Search Journey', () => {
    test('should navigate through main sections', async ({ page }) => {
      ...
      const megaMenuTrigger = page.getByRole('button', { name: /documents|create/i }).first();
      if (await megaMenuTrigger.isVisible()) {
        await megaMenuTrigger.click();
        
        // Wait for mega menu to open
        await page.waitForSelector('[data-testid*="mega-menu"], [role="menu"]', { timeout: 5000 });
        
        // Navigate to a document category
        const categoryLink = page.getByRole('link', { name: /vehicles|real estate|business/i }).first();
    ...
    test('should perform comprehensive search', async ({ page }) => {
      ...
      if (await searchInput.isVisible()) {
        ...
        for (const query of searchQueries) {
          ...
          const results = page.locator('[data-testid*="search-result"], [role="list"]');
  ...
  test.describe('Mobile User Journey', () => {
    test('should complete mobile navigation flow', async ({ page }) => {
      ...
      if (await mobileMenuButton.isVisible()) {
        ...
        await page.waitForSelector('[data-testid*="mobile-menu"], nav', { timeout: 5000 });
        ...
        const categoryLink = page.getByRole('link', { name: /bill of sale|lease/i }).first();
        if (await categoryLink.isVisible()) {
          await categoryLink.click();
          await page.waitForLoadState('networkidle');
        }
  ...
  test.describe('Language Switching Journey', () => {
    test('should switch between languages', async ({ page }) => {
      ...
      const languageSwitcher = page.getByRole('button', { name: /language|EN|ES/i });
      if (await languageSwitcher.isVisible()) {
        ...
        if (await spanishOption.isVisible()) {
          ...
          await expect(page.getByText(/documentos|crear/i)).toBeVisible({ timeout: 10000 });
        }
      }
    });
  });
