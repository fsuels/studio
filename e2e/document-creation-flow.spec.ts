...
test.describe('Document Creation Flow', () => {
  ...
  test('should complete vehicle bill of sale creation flow', async () => {
    ...
    await expect(page.locator('[data-testid="seller-name"]')).toContainText('John Doe');
    await expect(page.locator('[data-testid="buyer-name"]')).toContainText('Jane Smith');
  ...
  test('should handle form validation errors', async () => {
    ...
    await expect(page.locator('[data-testid="error-seller-name"]')).toBeVisible();
    await expect(page.locator('[data-testid="error-buyer-name"]')).toBeVisible();
    await expect(page.locator('[data-testid="error-vehicle-vin"]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="error-seller-name"]')).not.toBeVisible();
  });
  ...
  test('should support multiple languages', async () => {
    ...
    await expect(page.locator('[data-testid="field-label-seller-name"]')).toContainText('Nombre del Vendedor');
...
async function fillVehicleBillOfSaleForm(page: Page) {
  // Seller information
  await page.fill('[data-testid="input-seller-name"]', 'John Doe');
  await page.fill('[data-testid="input-seller-address"]', '123 Main St, Anytown, CA 90210');
  await page.fill('[data-testid="input-seller-phone"]', '555-123-4567');
  
  // Buyer information
  await page.fill('[data-testid="input-buyer-name"]', 'Jane Smith');
  await page.fill('[data-testid="input-buyer-address"]', '456 Oak Ave, Somewhere, CA 90211');
