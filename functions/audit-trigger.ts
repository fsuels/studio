...
export const auditTrailTrigger = onDocumentWritten(
  ...
  async (event) => {
    ...
    if (
      MONITORED_COLLECTIONS.length > 0 &&
      !MONITORED_COLLECTIONS.includes(collectionId)
    ) {
    ...
    try {
      ...
      logger.info(
        `Audit event stored successfully for ${collectionId}/${documentId}`,
      );
    } catch (error) {
...
async function createAuditEvent(
  ...
  const complianceFrameworks = getComplianceFrameworks(
    collectionId,
    dataClassification,
  );
...
function classifyData(
  collectionId: string,
  data: any,
): AuditEvent['compliance']['dataClassification'] {
...
function mapCollectionToEventType(
  collectionId: string,
  changeType: string,
...
function calculateAuditHash(
  event: Omit<AuditEvent, 'currentHash' | 'integrity'>,
): string {
...
async function storeAuditEventWithRetry(
