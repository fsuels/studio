...
exports.reindexDocumentOnUpdate = onDocumentUpdated(
  ...
  async (event) => {
    ...
    if (!contentChanged) {
      logger.info(
        `Document ${docId} updated but content unchanged, skipping reindexing`,
      );
      return;
    }
...
exports.batchIndexDocuments = onSchedule(
  ...
  async (context) => {
    ...
    try {
      ...
      logger.info(
        `Batch indexing completed: ${successful}/${results.length} successful`,
      );
    } catch (error) {
...
exports.cleanupOldVectors = onSchedule(
  ...
  async (context) => {
    ...
    try {
      ...
      logger.info(
        `Found ${validDocIds.size} valid documents, ${stats.totalVectorCount} vectors in index`,
      );
      ...
      if (stats.totalVectorCount > validDocIds.size * 1.1) {
        logger.info(
          'Index may contain orphaned vectors, consider manual cleanup',
        );
      }
    } catch (error) {
