...
const HEADER_KEYS = [
  'headline','subhead','ctaPrimary','ctaSecondary','trustText',
  'pricingTeaser','pricingLinkText','languageName',
  'Just 3 Easy Steps','featureTeaser','stepPrefix','stepOne',
  'nav','SearchBar','TopDocsChips','logoAlt'
]
...
for (const lng of LANGS) {
  ...
  fs.writeFileSync(path.join(outDir,'header.json'),
    JSON.stringify(header,null,2)+'\n'
  )
  fs.writeFileSync(path.join(outDir,'footer.json'),
    JSON.stringify(footer,null,2)+'\n'
  )
  fs.writeFileSync(path.join(outDir,'common.json'),
