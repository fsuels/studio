...
class TemplateVerifier {
  ...
  private globalRules: ValidationRules = {
    ...
    documentTypePatterns: {
      ...
      'child-medical-consent': /Medical Consent|Child|Caregiver|Medical Treatment/i,
      'living-trust': /Living Trust|Trustee|Beneficiary|Trust Property/i,
      'service-agreement': /Service Agreement|Services|Compensation|Deliverables/i,
      'employment-offer-letter': /Employment Offer|Position|Salary|Benefits/i,
      'independent-contractor-agreement': /Independent Contractor|Services|Payment Terms/i,
      'last-will-testament': /Last Will|Executor|Beneficiaries|Estate/i,
      'eviction-notice': /Eviction Notice|Tenant|Premises|Violation/i,
      'operating-agreement': /Operating Agreement|LLC|Members|Capital Contributions/i,
      'commercial-lease-agreement': /Commercial Lease|Lessee|Lessor|Commercial Property/i,
      'employment-termination-letter': /Termination|Employment|Last Day|Final Pay/i,
      'healthcare-power-of-attorney': /Healthcare|Medical Decisions|Healthcare Agent/i,
      'quitclaim-deed': /Quitclaim Deed|Grantor|Grantee|Property Transfer/i,
      'prenuptial-agreement': /Prenuptial Agreement|Future Spouse|Assets|Marriage/i,
      'affidavit-general': /Affidavit|Affiant|Sworn|Notary/i,
      'articles-of-incorporation-biz': /Articles of Incorporation|Corporation|Shares|Directors/i,
      'child-custody-agreement': /Child Custody|Visitation|Parenting Time|Support/i,
      'demand-letter-payment': /Demand for Payment|Debt|Amount Due|Collection/i,
      'divorce-settlement-agreement': /Divorce Settlement|Petitioner|Respondent|Property Division/i,
      'invoice': /Invoice|Bill To|Payment Terms|Amount Due/i,
      'living-will': /Living Will|Healthcare Directive|Life-Sustaining Treatment/i,
      'non-compete-agreement': /Non-Compete|Restricted Territory|Competitive Business/i,
      'partnership-agreement': /Partnership Agreement|Partners|Capital Contributions|Profit Sharing/i
    }
  };
  ...
  private documentRules: Record<string, Partial<ValidationRules>> = {
    'bill-of-sale-vehicle': {
      requiredSections: ['Vehicle Description', 'Purchase Price', 'Condition of Vehicle'],
      requiredVariables: ['{{vin}}', '{{make}}', '{{model}}', '{{year}}', '{{price}}']
    },
    'promissory-note': {
      requiredSections: ['Principal Amount', 'Interest Rate', 'Payment Terms', 'Default'],
      requiredVariables: ['{{principal_amount}}', '{{interest_rate}}', '{{payment_schedule}}']
    },
    'lease-agreement': {
      requiredSections: ['Property Description', 'Term of Lease', 'Rent', 'Security Deposit'],
      requiredVariables: ['{{monthly_rent}}', '{{lease_start_date}}', '{{lease_end_date}}']
    },
    'living-will': {
      requiredSections: ['Healthcare Directives', 'Life-Sustaining Treatment', 'Healthcare Agent'],
      requiredVariables: ['{{declarant_name}}', '{{state}}', '{{healthcare_agent}}']
    },
    'child-custody-agreement': {
      requiredSections: ['Custody Arrangements', 'Visitation Schedule', 'Child Support'],
      requiredVariables: ['{{parent_1_name}}', '{{parent_2_name}}', '{{child_1_name}}']
    }
  };
   ...
   */
  private async verifyTemplate(filePath: string): Promise<TemplateValidationResult> {
   ...
   */
  private applyGlobalRules(content: string, result: TemplateValidationResult): void {
    const rules = this.globalRules;
    
    // Check word count
    if (result.wordCount < rules.minWordCount) {
      result.errors.push(`Document too short: ${result.wordCount} words (minimum: ${rules.minWordCount})`);
    }
    if (result.wordCount > rules.maxWordCount) {
      result.warnings.push(`Document very long: ${result.wordCount} words (typical max: ${rules.maxWordCount})`);
    }
    ...
    if (result.variableCount < 5) {
      result.errors.push(`Too few variables: ${result.variableCount} (minimum: 5)`);
    }
   ...
   */
  private applyDocumentSpecificRules(fileName: string, content: string, result: TemplateValidationResult): void {
   ...
   */
  private checkDocumentTypeConsistency(fileName: string, content: string, result: TemplateValidationResult): void {
    const pattern = this.globalRules.documentTypePatterns[fileName];
    if (!pattern) {
      result.warnings.push(`No content pattern defined for document type: ${fileName}`);
      return;
    }
    
    if (!pattern.test(content)) {
      result.errors.push(`Content does not match expected document type: ${fileName}`);
      
      // Check if it matches a different document type
      for (const [docType, docPattern] of Object.entries(this.globalRules.documentTypePatterns)) {
        if (docType !== fileName && docPattern.test(content)) {
          result.errors.push(`Content appears to be a ${docType} instead of ${fileName}`);
          break;
        }
      }
    }
  ...
  private validateTemplateStructure(content: string, result: TemplateValidationResult): void {
    ...
    const expectedNumbers = Array.from({length: sections.length}, (_, i) => i + 1);
    ...
    if (ifCount !== endIfCount) {
      result.errors.push(`Unbalanced Handlebars conditions: ${ifCount} #if vs ${endIfCount} /if`);
    }
    ...
    for (const [hash, files] of this.duplicateHashes.entries()) {
      if (files.length > 1) {
        ...
        this.validationResults
          ...
          .forEach(r => {
            r.isValid = false;
            r.errors.push('DUPLICATE CONTENT: This file has identical content to other templates');
          });
      }
    }
  }
  ...
  private reportErrors(result: TemplateValidationResult): void {
    ...
    console.log(chalk.gray(`  Stats: ${result.wordCount} words, ${result.variableCount} variables, ${result.sectionCount} sections`));
  }
  ...
  private generateReport(): void {
    ...
    const validTemplates = this.validationResults.filter(r => r.isValid).length;
