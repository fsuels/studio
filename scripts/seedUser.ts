...
async function seedAll() {
  ...
  for (const docData of documents) {
    await setDoc(doc(db, 'users', userId, 'documents', docData.id), {
      ...
      sharing: {
        ...
        permissions:
          docData.id === 'nda_001' ? { 'contractor@example.com': 'sign' } : {},
      },
    ...
    await setDoc(
      doc(db, 'users', userId, 'documents', docData.id, 'versions', 'v1'),
      {
        createdAt: serverTimestamp(),
        editedBy: userId,
  ...
  await setDoc(doc(db, 'users', userId, 'signatures', 'sig_001'), {
    ...
    // 💰 Pricing
    cost: 2.5, // SignWell per-signature cost
  ...
  await setDoc(doc(db, 'users', userId, 'notarizations', 'notary_001'), {
    ...
    // 💰 Cost tracking
    baseFee: 25.0, // Base notarization fee
    platformFee: 5.0, // Your platform markup
    totalFee: 30.0,
  ...
  for (const payment of payments) {
    await setDoc(doc(db, 'users', userId, 'payments', payment.id), {
      ...
      invoiceUrl: payment.stripeInvoiceId
        ? `https://invoice.stripe.com/i/${payment.stripeInvoiceId}`
        : null,
    });
  }
  ...
  await setDoc(doc(db, 'users', userId, 'ai_generations', 'gen_001'), {
    promptUsed:
      'Create a non-disclosure agreement for a freelance graphic designer working on confidential marketing materials',
    documentType: 'nda',
  ...
  await setDoc(doc(db, 'users', userId, 'support', 'ticket_001'), {
    ...
    message:
      'Hi, I need help understanding the witness requirements for my will. Can someone explain?',
    status: 'open',
    priority: 'medium',
  ...
  console.log(
    '✅ Seeded comprehensive legal platform data with third-party integrations:',
  );
  ...
  console.log(
    '- Third-party notarization sessions (Notarize.com/NotaryCam style)',
  );
