...
const documentLibraryPath = path.join(__dirname, '../src/lib/document-library.ts');
...
if (fs.existsSync(templatesDir)) {
  const enTemplates = fs.readdirSync(path.join(templatesDir, 'en')).filter(f => f.endsWith('.md'));
  const esTemplates = fs.readdirSync(path.join(templatesDir, 'es')).filter(f => f.endsWith('.md'));
  ...
  const commonDocs = ['nda', 'lease-agreement', 'promissory-note', 'employment-contract'];
  ...
  commonDocs.forEach(docId => {
    ...
    console.log(`  ${docId}: EN(${enExists ? '✓' : '✗'}) ES(${esExists ? '✓' : '✗'})`);
  });
}

if (fs.existsSync(previewsDir)) {
  const enPreviews = fs.existsSync(path.join(previewsDir, 'en')) ? 
    fs.readdirSync(path.join(previewsDir, 'en')).length : 0;
  const esPreviews = fs.existsSync(path.join(previewsDir, 'es')) ? 
    fs.readdirSync(path.join(previewsDir, 'es')).length : 0;
...
problematicFiles.forEach(filePath => {
  ...
  if (fs.existsSync(fullPath)) {
    ...
    if (filePath.includes('BillOfSalePreview')) {
      ...
      console.log(`  ✓ BillOfSalePreview accepts docId prop: ${hasDocIdProp ? '✓' : '✗'}`);
    }
    
    if (filePath.includes('StickyMobileCTA')) {
      const hasDocIdProp = content.includes('docId?: string');
      console.log(`  ✓ StickyMobileCTA accepts docId prop: ${hasDocIdProp ? '✓' : '✗'}`);
    }
    
    if (filePath.includes('/page.tsx')) {
      const hasGenericHead = content.includes('documentLibrary.find');
      console.log(`  ✓ Page Head function is generic: ${hasGenericHead ? '✓' : '✗'}`);
    }
  }
});

console.log('\n🎯 Summary:');
console.log('The following fixes have been applied to resolve the vehicle bill of sale issue:');
console.log('1. ✓ BillOfSalePreview now accepts a docId prop instead of being hardcoded');
...
console.log('2. Ensure all markdown templates are complete and correctly named');
console.log('3. Clear browser cache if testing locally');
console.log('4. Check that document IDs match between library and template files');
