...
class DocumentQualityVerifier {
  ...
  addError(check, message, file = null) {
    ...
    this.log(`ERROR in ${check}: ${message}${file ? ` (${file})` : ''}`, 'error');
  }
  ...
  addWarning(check, message, file = null) {
    ...
    this.log(`WARNING in ${check}: ${message}${file ? ` (${file})` : ''}`, 'warning');
  }
  ...
  checkDocumentStructure() {
    ...
    const documentDirs = fs.readdirSync(this.usDocsDir)
      .filter(item => {
    ...
    documentDirs.forEach(dir => {
      ...
      const requiredFiles = ['index.ts', 'metadata.ts', 'schema.ts', 'questions.ts'];
  ...
  checkExportConsistency() {
    ...
    documentDirs.forEach(dir => {
      ...
      if (!hasExport) {
        this.addError('Exports', `Directory ${dir} is not exported in index.ts`, dir);
      } else {
    ...
    exportLines.forEach(line => {
      ...
      if (match) {
        ...
        if (!fs.existsSync(dirPath)) {
          this.addError('Exports', `Export references non-existent directory: ${dirName}`, `index.ts`);
        } else {
          this.addSuccess('Exports');
        }
      }
  ...
  checkTemplateCompleteness() {
    ...
    documentDirs.forEach(dir => {
      ...
      if (!fs.existsSync(enTemplatePath)) {
        this.addError('Templates', `Missing English template for ${dir}`, `en/${dir}.md`);
      } else {
        const content = fs.readFileSync(enTemplatePath, 'utf8').trim();
        if (content.length < 100) {
          this.addWarning('Templates', `English template for ${dir} seems too short`, `en/${dir}.md`);
        } else {
      ...
      if (!fs.existsSync(esTemplatePath)) {
        this.addError('Templates', `Missing Spanish template for ${dir}`, `es/${dir}.md`);
      } else {
        const content = fs.readFileSync(esTemplatePath, 'utf8').trim();
        if (content.length < 100) {
          this.addWarning('Templates', `Spanish template for ${dir} seems too short`, `es/${dir}.md`);
        } else {
          this.addSuccess('Templates');
        }
      }
  ...
  checkMetadataValidation() {
    ...
    documentDirs.forEach(dir => {
      ...
      if (fs.existsSync(metadataPath)) {
        ...
        const requiredFields = ['id', 'jurisdiction', 'category', 'languageSupport', 'basePrice', 'translations'];
        requiredFields.forEach(field => {
          if (!content.includes(`${field}:`)) {
            this.addError('Metadata', `Missing required field: ${field}`, `${dir}/metadata.ts`);
          } else {
        ...
        if (!content.includes('translations:')) {
          this.addError('Metadata', 'Missing translations object', `${dir}/metadata.ts`);
        } else {
          if (!content.includes('en:') || !content.includes('es:')) {
            this.addWarning('Metadata', 'Missing English or Spanish translations', `${dir}/metadata.ts`);
          } else {
            this.addSuccess('Metadata');
          }
        }
        ...
        if (idMatch && idMatch[1] !== dir) {
          this.addError('Metadata', `ID "${idMatch[1]}" doesn't match directory name "${dir}"`, `${dir}/metadata.ts`);
        } else if (idMatch) {
          this.addSuccess('Metadata');
        }
        ...
        if (!content.includes('aliases:')) {
          this.addWarning('Metadata', 'No search aliases defined', `${dir}/metadata.ts`);
        } else {
          this.addSuccess('Metadata');
        }
      }
    ...
    documentDirs.forEach(dir => {
      ...
      if (fs.existsSync(schemaPath)) {
        ...
        if (!content.includes('z.object') && !content.includes('z.record')) {
          this.addWarning('Schema', 'No zod object schema found', `${dir}/schema.ts`);
        } else {
          this.addSuccess('Schema');
        }
      }
  ...
  checkTypeScriptCompilation() {
    ...
    try {
      ...
      try {
        ...
        this.addWarning('TypeScript', 'TypeScript compiler not found, skipping compilation check');
        return;
      }
      ...
      this.addWarning('TypeScript', `Compilation check skipped - consider installing TypeScript: ${error.message}`);
    }
  }
  ...
  checkLinting() {
    ...
    try {
      ...
      if (!fs.existsSync(eslintConfigPath)) {
        this.addWarning('Linting', 'ESLint configuration not found, skipping lint check');
        return;
      }
      ...
      this.addWarning('Linting', `Linting issues found but not blocking: ${error.message.split('\n')[0]}`);
    }
  }
  ...
  generateReport() {
    ...
    const reportFile = path.join(reportPath, `quality-report-${Date.now()}.json`);
  ...
  calculateQualityScore() {
    const totalChecks = this.results.passedChecks + this.results.failedChecks + this.results.warningChecks;
    if (totalChecks === 0) return 100;
    
    const errorPenalty = this.results.failedChecks * 2;
    const warningPenalty = this.results.warningChecks * 0.5;
    const score = Math.max(0, 100 - ((errorPenalty + warningPenalty) / totalChecks) * 100);
  ...
  generateRecommendations() {
    ...
    if (this.warnings.length > 10) {
      recommendations.push({
        ...
        actions: ['Review and fix warning messages', 'Consider adding more comprehensive checks']
      });
    }

    const aliasCount = this.warnings.filter(w => w.message.includes('aliases')).length;
    if (aliasCount > 5) {
      recommendations.push({
        priority: 'low',
  ...
  async runAllChecks() {
    ...
    if (report.score >= 95) {
      this.log('🎉 EXCELLENT! Your document system has outstanding quality!', 'success');
