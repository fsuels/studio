...
class PerformanceBenchmark {
  ...
  async analyzeBundleSize() {
    ...
    console.log(`Bundle size: ${totalSize.toFixed(2)}KB (threshold: ${this.results.thresholds.bundleSize}KB)`);
  }
  ...
  async benchmarkComponentRender() {
    ...
    this.results.metrics.componentRender = {
      times: renderTimes,
      average: Object.values(renderTimes).reduce((a, b) => a + b, 0) / components.length,
  ...
  async analyzeCostEfficiency() {
    ...
    console.log(`Estimated Firebase costs: $${costMetrics.firebaseCosts.estimated}/month`);
  }
  ...
  generateReport() {
    ...
    console.log('================================');
    console.log(`Overall Score: ${overallScore}/100 (${this.results.summary.grade})`);
    console.log(`Report saved: ${resultsPath}`);
    
    // Print key metrics
    if (this.results.metrics.bundleSize) {
      console.log(`Bundle Size: ${this.results.metrics.bundleSize.total.toFixed(2)}KB`);
    }
    
    if (this.results.metrics.largestContentfulPaint) {
      console.log(`LCP: ${this.results.metrics.largestContentfulPaint.value}ms (${this.results.metrics.largestContentfulPaint.grade})`);
    }
    
    if (this.results.metrics.costs) {
      console.log(`Firebase Costs: $${this.results.metrics.costs.firebaseCosts.estimated}/month`);
    }
  ...
  generateRecommendations() {
    ...
    if (this.results.metrics.bundleSize && !this.results.metrics.bundleSize.passed) {
      recommendations.push('Optimize bundle size through code splitting and tree shaking');
    }
    
    if (this.results.metrics.largestContentfulPaint && this.results.metrics.largestContentfulPaint.grade !== 'A') {
      recommendations.push('Improve Largest Contentful Paint through image optimization and critical CSS');
    }
    
    if (this.results.metrics.cumulativeLayoutShift && this.results.metrics.cumulativeLayoutShift.value > 0.1) {
