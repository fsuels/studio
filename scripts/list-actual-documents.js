...
function checkTemplate(docId) {
  const enTemplate = path.join(
    __dirname,
    '../public/templates/en',
    `${docId}.md`,
  );
  const esTemplate = path.join(
    __dirname,
    '../public/templates/es',
    `${docId}.md`,
  );
...
completeDocs.forEach((doc, index) => {
  const templateStatus = doc.hasTemplate.en
    ? doc.hasTemplate.es
      ? '🌐'
      : '🇺🇸'
    : '❌';
...
exportedDocs.forEach((doc, index) => {
  const hasStructure = workingUSDocs.find(
    (d) =>
      d.id === doc ||
      d.id ===
        doc
...
templateFiles.forEach((template, index) => {
  const hasDoc = workingUSDocs.find(
    (d) =>
      d.id === template || template.includes(d.id) || d.id.includes(template),
  );
  const status = hasDoc ? '📚' : '🔍';
...
console.log(`┌─────────────────────────────────────┐`);
console.log(
  `│  📚 Complete Documents: ${completeDocs.length.toString().padEnd(11)} │`,
);
console.log(
  `│  📤 Exported Documents: ${exportedDocs.length.toString().padEnd(11)} │`,
);
console.log(
  `│  📄 Template Files: ${templateFiles.length.toString().padEnd(15)} │`,
);
console.log(`└─────────────────────────────────────┘`);
...
console.log(
  '✅ = Complete  ⚠️ = Missing components  📚 = Has document  🔍 = Template only',
);
