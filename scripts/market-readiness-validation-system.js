...
class MarketReadinessValidationSystem {
  constructor() {
    ...
    this.countryPriorities = {
      ...
      'Mexico': { tier: 1, population: 130000000, digitalAdoption: 0.78, legalComplexity: 0.7 },
      'Spain': { tier: 1, population: 47000000, digitalAdoption: 0.92, legalComplexity: 0.8 },
      'Canada': { tier: 1, population: 39000000, digitalAdoption: 0.95, legalComplexity: 0.6 },
      'United Kingdom': { tier: 1, population: 67000000, digitalAdoption: 0.96, legalComplexity: 0.8 },
      
      // Tier 2: Secondary Markets  
      'Colombia': { tier: 2, population: 51000000, digitalAdoption: 0.68, legalComplexity: 0.75 },
      'Argentina': { tier: 2, population: 45000000, digitalAdoption: 0.81, legalComplexity: 0.85 },
      'Peru': { tier: 2, population: 33000000, digitalAdoption: 0.66, legalComplexity: 0.8 },
      'Chile': { tier: 2, population: 19000000, digitalAdoption: 0.87, legalComplexity: 0.7 },
      
      // Tier 3: Future Expansion
      'Ecuador': { tier: 3, population: 18000000, digitalAdoption: 0.65, legalComplexity: 0.75 },
      'Guatemala': { tier: 3, population: 18000000, digitalAdoption: 0.47, legalComplexity: 0.8 },
      'Dominican Republic': { tier: 3, population: 11000000, digitalAdoption: 0.74, legalComplexity: 0.7 }
    };
  ...
  loadMarketRequirementsDatabase() {
    const dbFile = path.join(this.marketRequirementsDir, 'market_requirements.db.json');
    
    if (fs.existsSync(dbFile)) {
      const data = JSON.parse(fs.readFileSync(dbFile, 'utf8'));
      this.marketRequirementsDb = new Map(Object.entries(data));
      console.log(`📋 Loaded ${this.marketRequirementsDb.size} market requirement sets`);
    }
  }
    ...
    this.marketRequirementsDb.set('Mexico', {
      ...
      regulatoryCompliance: {
        ...
        eSignatureLegality: {
          status: 'validated',
          score: 0.9,
          requirements: ['FIEL (Advanced Electronic Signature)', 'e.firma integration']
        },
        businessRegistration: {
          status: 'pending',
          score: 0.3,
        ...
        taxCompliance: {
          status: 'pending',
          score: 0.2,
          requirements: ['SAT compliance', 'CFDI integration', 'Tax calculation']
        }
      },
      technicalReadiness: {
        languageLocalization: {
          status: 'validated',
          score: 0.95,
          requirements: ['Mexican Spanish localization', 'Legal terminology validation']
        },
        currencySupport: {
          status: 'validated',
          score: 1.0,
        ...
        paymentMethods: {
          ...
          score: 0.6,
          requirements: ['OXXO integration', 'SPEI transfers', 'Credit card processing']
        },
        technicalInfrastructure: {
          status: 'validated',
          score: 0.9,
          requirements: ['Mexico region deployment', 'CDN coverage', 'Latency optimization']
        },
        mobileOptimization: {
          status: 'validated',
          score: 0.85,
      ...
      marketPreparation: {
        localPartners: {
          status: 'in_progress',
          score: 0.7,
          requirements: ['Legal advisory partners', 'Payment processor partnerships']
        },
        customerSupport: {
          status: 'pending',
          score: 0.3,
          requirements: ['Spanish language support', 'Mexico timezone coverage']
        },
        marketingMaterials: {
          status: 'in_development',
          score: 0.5,
        ...
        competitorAnalysis: {
          status: 'completed',
          score: 1.0,
          requirements: ['Market research completed', 'Competitive positioning defined']
        },
        pricingStrategy: {
          status: 'validated',
          score: 0.8,
      ...
      operationalReadiness: {
        staffTraining: {
          status: 'pending',
          score: 0.2,
          requirements: ['Mexican legal system training', 'Cultural sensitivity training']
        },
        processDocumentation: {
          status: 'in_progress',
          score: 0.6,
        ...
        escalationProcedures: {
          status: 'pending',
          score: 0.3,
          requirements: ['Management escalation paths', 'Legal escalation procedures']
        }
      }
    });
    ...
    this.marketRequirementsDb.set('Spain', {
      ...
      regulatoryCompliance: {
        dataProtectionLaws: {
          status: 'validated',
          score: 1.0,
          requirements: ['GDPR compliance', 'AEPD registration', 'Data residency'],
        ...
        taxCompliance: {
          status: 'in_progress',
          score: 0.7,
          requirements: ['VAT compliance', 'Tax calculation', 'AEAT integration']
        }
      },
      technicalReadiness: {
        languageLocalization: {
          status: 'validated',
          score: 0.98,
          requirements: ['Peninsular Spanish', 'Legal terminology', 'Regional variations']
        },
        currencySupport: {
          status: 'validated',
          score: 1.0,
        ...
        paymentMethods: {
          status: 'validated',
          score: 0.95,
          requirements: ['SEPA payments', 'Spanish banking integration', 'Card processing']
        },
        technicalInfrastructure: {
          status: 'validated',
          score: 0.9,
          requirements: ['EU-West region', 'GDPR infrastructure', 'Data residency']
        },
        mobileOptimization: {
          status: 'validated',
          score: 0.92,
      ...
      operationalReadiness: {
        ...
        processDocumentation: {
          status: 'validated',
          score: 0.8,
          requirements: ['Spain-specific processes', 'EU compliance procedures']
        },
        qualityAssurance: {
          status: 'validated',
          score: 0.95,
        ...
        escalationProcedures: {
          status: 'validated',
          score: 0.85,
          requirements: ['Management escalation', 'Legal escalation', 'Regulatory escalation']
        }
      }
    });

    this.saveMarketRequirementsDatabase();
    console.log(`✅ Seeded market requirements for ${this.marketRequirementsDb.size} markets`);
  }
  ...
  calculateComplianceScore(marketData) {
    ...
    Object.entries(this.validationCategories).forEach(([categoryName, categoryInfo]) => {
  ...
  generateCategoryBreakdown(marketData) {
    ...
    Object.entries(this.validationCategories).forEach(([categoryName, categoryInfo]) => {
      const categoryData = marketData[categoryName];
      if (!categoryData) {
        breakdown[categoryName] = { score: 0, weight: categoryInfo.weight, status: 'missing' };
        return;
      }
      ...
      breakdown[categoryName] = {
        score: requirementCount > 0 ? Math.round((categoryScore / requirementCount) * 1000) / 1000 : 0,
        weight: categoryInfo.weight,
        requirementCount,
        statuses,
        weightedContribution: Math.round(((categoryScore / requirementCount) * categoryInfo.weight) * 1000) / 1000
      };
    });
      ...
      Object.entries(categoryData).forEach(([reqName, requirement]) => {
        if (!requirement || requirement.score < criticalThreshold) {
          blockers.push({
            category: categoryName,
            requirement: reqName,
            severity: requirement && requirement.score < criticalThreshold ? 'high' : 'critical',
            score: requirement ? requirement.score : 0,
            status: requirement ? requirement.status : 'missing',
            issue: requirement ? 'Below critical threshold' : 'Requirement missing'
          });
        }
      });
    });
  ...
  generateRecommendations(marketData, adjustedScore) {
    ...
    Object.entries(this.validationCategories).forEach(([categoryName, categoryInfo]) => {
      ...
      const avgCategoryScore = requirementCount > 0 ? categoryScore / requirementCount : 0;
  ...
  saveValidationReport(country, validation) {
    const reportFile = path.join(this.reportsDir, `${country}-validation-${this.getDateString()}.json`);
  ...
  async validateStagedRollout(countries) {
      ...
      try {
        ...
        if (validation.launchApproved) {
          ...
          results.blockedMarkets.push({
            country,
            score: validation.adjustedScore,
            blockers: validation.blockers.filter(b => b.severity === 'critical').length,
    ...
    results.summary.averageScore = Math.round((totalScore / countries.length) * 1000) / 1000;
  ...
  estimateReadyDate(validation) {
    const criticalBlockers = validation.blockers.filter(b => b.severity === 'critical').length;
    const highBlockers = validation.blockers.filter(b => b.severity === 'high').length;
  ...
  generateStagedRolloutReport(results) {
    const reportFile = path.join(this.reportsDir, `staged-rollout-validation-${this.getDateString()}.json`);
    ...
    if (results.rolloutSequence.length > 0) {
      ...
      results.rolloutSequence.forEach((market, index) => {
        console.log(`   ${index + 1}. ${market.country} (Score: ${market.score}, Tier ${market.tier}, Launch: ${market.estimatedLaunchDate})`);
      });
      console.log('');
    }
    ...
    if (results.blockedMarkets.length > 0) {
      ...
      results.blockedMarkets.forEach(market => {
        console.log(`   • ${market.country} (Score: ${market.score}, ${market.blockers} critical blockers, Ready: ${market.estimatedReadyDate})`);
      });
      console.log('');
    }
...
if (require.main === module) {
  ...
  switch (command) {
    case 'validate':
      if (!country) {
        console.error('❌ Please specify a country: npm run market-validate validate Mexico');
        process.exit(1);
      }
      
      validator.validateMarketReadiness(country)
        .then(result => {
          console.log(`\n🎯 Market validation complete for ${country}!`);
          console.log(`   Compliance Score: ${result.complianceScore}`);
          console.log(`   Adjusted Score: ${result.adjustedScore}`);
          console.log(`   Launch Approved: ${result.launchApproved ? '✅ YES' : '🚫 NO'}`);
          
          if (!result.launchApproved) {
            console.log(`   Critical Blockers: ${result.blockers.filter(b => b.severity === 'critical').length}`);
          }
    ...
    case 'staged':
      ...
      validator.validateStagedRollout(countries)
        .then(results => {
    ...
    case 'scores':
      ...
      Object.entries(scores)
        ...
        .forEach(([country, data]) => {
          ...
          console.log(`   ${status} ${country}: ${data.adjustedScore} (Tier ${data.tier})`);
        });
      break;
    ...
    default:
      ...
      console.log('  npm run market-validate validate <country>  # Validate specific market');
      console.log('  npm run market-validate staged [countries]  # Validate staged rollout');
      console.log('  npm run market-validate scores              # Show all market scores');
      console.log('');
