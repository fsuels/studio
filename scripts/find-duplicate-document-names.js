...
function extractDocumentInfo(filePath) {
  try {
    ...
    const enNameMatch = content.match(
      /translations:\s*{[\s\S]*?en:\s*{[\s\S]*?name:\s*['"`]([^'"`]+)['"`]/,
    );
    if (enNameMatch) {
    ...
    const esNameMatch = content.match(
      /translations:\s*{[\s\S]*?es:\s*{[\s\S]*?name:\s*['"`]([^'"`]+)['"`]/,
    );
    if (esNameMatch) {
    ...
    const frNameMatch = content.match(
      /translations:\s*{[\s\S]*?fr:\s*{[\s\S]*?name:\s*['"`]([^'"`]+)['"`]/,
    );
    if (frNameMatch) {
...
function findDuplicates() {
  const documentsDir = path.join(__dirname, '../src/lib/documents');
  ...
  for (const [name, docs] of nameGroups.entries()) {
    if (docs.length > 1) {
      // Check if they're actually different documents
      ...
      if (uniqueIds.length > 1) {
        ...
        const uniqueDocs = uniqueIds.map((id) => docs.find((d) => d.id === id));

        for (const doc of uniqueDocs) {
          const relativePath = doc.path
            .replace(documentsDir, '')
            .replace(/^\//, '');
  ...
  for (let i = 0; i < allNames.length; i++) {
    for (let j = i + 1; j < allNames.length; j++) {
      ...
      if (areSimilar(name1, name2)) {
        ...
        const allDocs = [...new Set([...docs1, ...docs2].map((d) => d.id))];
