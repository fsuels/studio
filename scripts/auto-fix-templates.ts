...
class TemplateFixer {
    ...
    for (const lang of languages) {
      ...
      for (const file of files) {
        ...
        if (result.fixesApplied.length > 0) {
          console.log(chalk.green(`✅ ${file} - Fixed ${result.fixesApplied.length} issues`));
  ...
  private fixHandlebarsBalance(content: string, result: FixResult): string {
    ...
    if (ifMatches.length !== endIfMatches.length) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (openIfs > 0 && (lines[i].match(/^##\s+\d+\./) || i === lines.length - 1)) {
          while (openIfs > 0) {
            lines.splice(i, 0, '{{/if}}');
            openIfs--;
            i++;
  ...
  private fixFooterFormat(content: string, result: FixResult): string {
    const oldFooter = '_Template generated by 123 LegalDoc. Replace bracketed fields with actual data._';
    const newFooter = '*Template generated by 123LegalDoc - Professional Legal Document Platform*';
    ...
    if (!content.includes(newFooter) && !content.includes('Template generated by 123LegalDoc')) {
   ...
   */
  private addMissingElements(content: string, fileName: string, result: FixResult): string {
    // Check for legal notice
    if (!content.includes('IMPORTANT LEGAL NOTICE')) {
      const documentType = fileName.replace('.md', '').replace(/-/g, ' ')
      ...
      if (footerMatch) {
        content = content.replace(footerMatch[0], legalNotice + '\n\n' + footerMatch[0]);
      } else {
    ...
    if (!content.toLowerCase().includes('signature')) {
      ...
      if (legalNoticeIndex > -1) {
        content = content.slice(0, legalNoticeIndex - 5) + signatureSection + content.slice(legalNoticeIndex - 5);
      } else {
  ...
  private generateFixReport(): void {
    ...
    const filesFixed = this.fixResults.filter(r => r.fixesApplied.length > 0).length;
    const totalFixes = this.fixResults.reduce((sum, r) => sum + r.fixesApplied.length, 0);
    ...
    const reportPath = path.join(this.templateDir, '..', '..', 'template-fix-report.json');
    const report = {
    ...
    if (totalFixes > 0) {
      ...
      console.log(chalk.yellow('Please run npm run verify-templates to confirm all issues are resolved.'));
    } else {
