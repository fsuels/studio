...
class IntegratedQualitySystem {
  ...
  async runComprehensiveValidation() {
    ...
    try {
      ...
      const markets = process.argv.slice(2).filter(arg => !arg.startsWith('--'));
      if (markets.length > 0) {
      ...
      const criticalIssues = this.decisionEngine.actionResults.filter(r => r.severity === 'CRITICAL').length;
      ...
      if (shouldBlock || criticalIssues > 0) {
        ...
        console.log('📊 All systems operational with minor issues handled automatically');
        process.exit(0);
      }
  ...
  async runTranslationCheck() {
    try {
      execSync('node scripts/translation-validation-system.js', { stdio: 'pipe' });
  ...
  async runSEOCheck() {
    try {
      execSync('node scripts/seo-content-uniqueness-system.js', { stdio: 'pipe' });
  ...
  async processResultsWithDecisionEngine() {
    ...
    if (this.reports.quality) {
      const qualityDecisions = this.decisionEngine.processQualityResults(this.reports.quality);
      allDecisions = allDecisions.concat(qualityDecisions);
    }
    
    // Process legal results
    if (this.reports.legal) {
      const legalDecisions = this.decisionEngine.processLegalResults(this.reports.legal);
      allDecisions = allDecisions.concat(legalDecisions);
    }
    
    // Process translation results
    if (this.reports.translation) {
      const translationDecisions = this.decisionEngine.processTranslationResults(this.reports.translation);
      allDecisions = allDecisions.concat(translationDecisions);
    }
    
    // Process SEO results
    if (this.reports.seo) {
      const seoDecisions = this.decisionEngine.processSEOResults(this.reports.seo);
      allDecisions = allDecisions.concat(seoDecisions);
    }
    
    // Process market readiness results
    if (this.reports.market) {
      const marketDecisions = this.decisionEngine.processMarketReadinessResults(this.reports.market);
  ...
  generateFinalReport() {
    ...
    const enhancedReportPath = path.join(__dirname, '../decision-reports', `integrated-report-${Date.now()}.json`);
  ...
  calculateOverallStatus() {
    ...
    const criticalIssues = this.decisionEngine.actionResults.filter(r => r.severity === 'CRITICAL').length;
    const highIssues = this.decisionEngine.actionResults.filter(r => r.severity === 'HIGH').length;
  ...
  generateActionsSummary() {
    const autoFixed = this.decisionEngine.actionResults.filter(r => r.autoFixSuccessful).length;
    const escalated = this.decisionEngine.actionResults.filter(r => r.escalated).length;
