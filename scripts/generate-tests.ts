...
class TestGenerator {
  ...
  private extractExports(content: string): string[] {
    ...
    const exportMatches = content.match(
      /export\s+(?:default\s+)?(?:const\s+|function\s+)?(\w+)/g,
    );
    if (exportMatches) {
      exports.push(
        ...exportMatches.map((match) =>
          match.replace(
            /export\s+(?:default\s+)?(?:const\s+|function\s+)?/,
            '',
          ),
        ),
      );
  ...
  private generateAccessibilityAssertions(
    ...
    if (analysis.isInteractive) {
      assertions.push(
        "expect(screen.getByTestId('testId')).toHaveAttribute('tabIndex')",
      );
    }
