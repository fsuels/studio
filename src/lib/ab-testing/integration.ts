...
class ABTestingIntegrationService {
  ...
  isFeatureEnabledWithExperiments(
    ...
    if (experiment && experiment.status === 'running') {
      const variantId = experimentEngine.assignUserToExperiment(
        experiment.id,
        context.userId,
      );
      ...
      if (variantId) {
        ...
        if (
          variant?.featureConfig &&
          variant.featureConfig[featureKey] !== undefined
        ) {
  ...
  trackFunnelStepWithExperiments(step: FunnelStep, userId: string): void {
  ...
  async getExperimentResultsWithFunnelInsights(experimentId: string) {
    ...
    const funnelInsights =
      await this.getFunnelInsightsForExperiment(experiment);
  ...
  generateExperimentRecommendations(): Array<{
    ...
    if (funnelMetrics.overall.visitToDraft < 30) {
      recommendations.push({
        ...
        description:
          'Low visit-to-draft conversion suggests landing page optimization opportunity',
        estimatedImpact: 15,
        effort: 'medium' as const,
        suggestedTest: {
          hypothesis:
            'Clearer value proposition and social proof will increase draft starts',
          variants: [
            'Current',
            'Enhanced Headlines + Testimonials',
            'Video Demo',
          ],
    ...
    if (funnelMetrics.overall.draftToCheckout < 60) {
      recommendations.push({
        ...
        description:
          'Poor draft-to-checkout conversion indicates form friction',
        estimatedImpact: 20,
        effort: 'low' as const,
        suggestedTest: {
          hypothesis:
            'Reducing form fields and adding progress indicators will improve completion',
          variants: [
            'Current Form',
            'Simplified Form',
    ...
    if (funnelMetrics.overall.checkoutToSigned < 80) {
      recommendations.push({
        ...
        suggestedTest: {
          hypothesis:
            'Enhanced security badges and transparent pricing will reduce checkout abandonment',
          variants: [
            'Current Checkout',
    ...
    if (runningExperiments.length < 3) {
      recommendations.push({
        ...
        suggestedTest: {
          hypothesis:
            'AI-powered document suggestions will improve user engagement and conversion',
          variants: [
            'No AI Suggestions',
            'Basic Suggestions',
  ...
  private getExperimentForFeature(featureKey: string): Experiment | null {
    const runningExperiments = experimentEngine.getRunningExperiments();
    return (
      runningExperiments.find((exp) => exp.featureFlag === featureKey) || null
    );
  }

  private determineABTestCohort(
    userExperiments: Record<string, string>,
  ): string {
    ...
    const cohortParts = Object.entries(userExperiments)
      .map(
        ([expId, variantId]) =>
          `${expId.split('_')[1]}-${variantId.split('_')[1]}`,
      )
      .sort()
      .join('_');
