...
export interface MonitoringAlert {
  id: string;
  experimentId: string;
  type: 'significance_reached' | 'sample_size_reached' | 'duration_exceeded' | 'performance_drop' | 'error_rate_high';
...
class ExperimentMonitoringService {
  ...
  async checkExperimentHealth(experiment: Experiment): Promise<ExperimentHealth> {
  ...
  private async analyzeExperimentHealth(
    ...
    // Check sample size
    const totalSampleSize = Object.values(results.sampleSizes).reduce((sum, size) => sum + size, 0);
    if (totalSampleSize < experiment.minSampleSize) {
      ...
      if (completionRate < 0.5 && daysRunning > 7) {
        issues.push({
          ...
          recommendation: 'Consider increasing traffic allocation or extending duration'
        });
        status = 'warning';
      }
    }
    ...
    if (results.isStatisticallySignificant && results.confidence >= 0.95) {
      issues.push({
        ...
        recommendation: 'Consider stopping experiment and implementing winning variant'
      });
    }
    ...
    const hasLowPerformingVariant = conversionRates.some(rate => rate < avgConversionRate * 0.7);
    
    if (hasLowPerformingVariant && totalSampleSize > experiment.minSampleSize * 0.5) {
      issues.push({
        type: 'performance_concern',
        severity: 'high',
        message: 'One or more variants showing significantly lower conversion rates',
  ...
  private estimateTimeToSignificance(
    ...
    const remainingSampleNeeded = Math.max(0, experiment.minSampleSize - currentSampleSize);
  ...
  private async generateHealthAlerts(experiment: Experiment, health: ExperimentHealth): Promise<void> {
  ...
  async calculateGrowthMetrics(timeframe: GrowthMetrics['timeframe']): Promise<GrowthMetrics> {
    ...
    for (const experiment of experiments) {
      ...
      if (experiment.status === 'running') {
        ...
        if (results.isStatisticallySignificant && results.winningVariant) {
          ...
          const baselineRevenue = this.calculateBaselineRevenue(experiment, timeframe);
          const liftPercentage = results.effectSize || 0;
          const experimentRevenueImpact = baselineRevenue * (liftPercentage / 100);
    ...
    return {
      ...
      totalRevenue,
      revenueImpact,
      conversionLift: conversionLift / Math.max(statisticallySignificantWins, 1),
      experimentsRunning,
      experimentsCompleted,
      statisticallySignificantWins,
  ...
  private calculateBaselineRevenue(experiment: Experiment, timeframe: GrowthMetrics['timeframe']): number {
  ...
  async autoManageExperiments(): Promise<void> {
    ...
    for (const experiment of runningExperiments) {
      ...
      if (daysRunning > experiment.estimatedDuration * 2 && !results.isStatisticallySignificant) {
  ...
  private async autoStopExperiment(
    ...
    const alert: MonitoringAlert = {
      ...
      experimentId: experiment.id,
      type: reason === 'significance' ? 'significance_reached' : 'duration_exceeded',
      priority: 'medium',
  ...
  async generatePerformanceReport(): Promise<{
    ...
    return {
      summary: {
        ...
        successRate: completedExperiments.length > 0 ? (significantExperiments / completedExperiments.length) * 100 : 0,
        avgImpact: significantExperiments > 0 ? totalImpact / significantExperiments : 0
      },
