...
import type { LegalDocument, Question, UpsellClause } from '@/types/documents'; // Import types
...
// The main library of legal documents - initialized here
const initialDocumentLibrary: LegalDocument[] = [
  // --- Category 1: Finance ---
  {
    id: 'promissory-note',
  ...
  },
  {
    ...
    schema: BillOfSaleSchema, // Ensure BillOfSaleSchema is defined and imported correctly
    questions: [
      ...
      { id: 'sale_date', label: 'Date of Sale', type: 'date', required: true, tooltip: "The date the sale is finalized." },
      { id: 'existing_liens', label: 'Existing Liens (if any, otherwise leave blank or "None")', type: 'text', placeholder: 'e.g., None, or Loan with XYZ Bank', tooltip: "Disclose any outstanding loans or claims against the vehicle." },
      { id: 'as_is', label: 'Is the vehicle sold "as-is"?', type: 'boolean', required: true, tooltip: "Select 'Yes' if sold without warranties, 'No' if warranties are provided." },
      { id: 'warranty_text', label: 'Warranty Details (if not "as-is")', type: 'textarea', placeholder: 'e.g., 30-day warranty on drivetrain (if applicable)', tooltip: "If warranties are offered, describe them here. This field is only active if 'Sold As-Is' is 'No'." },
      { id: 'state', label: 'State of Sale (Governing Law & Notary)', type: 'select', required: true, options: usStates.map(s => ({ value: s.value, label: s.label })), tooltip: "The U.S. state whose laws will govern this agreement and where notarization may occur." },
      { id: 'county', label: 'County (for Notary Acknowledgment)', type: 'text', required: false, tooltip: "The county within the state where the document is signed, typically for notarization purposes." }
    ],
    upsellClauses: [
  ...
  },
  {
    ...
    schema: z.object({
      ...
      invoiceDate: z.string().min(1), // Should be date, but keeping string for simplicity if form sends string
      dueDate: z.string().optional(), // Same as above
      lineItems: z.string().min(1), // This should ideally be an array of objects
  ...
  // --- Category 4: Family ---
  {
    ...
    schema: z.object({
      ...
      hasChildren: z.enum(['yes', 'no'], { errorMap: () => ({ message: "Please specify if there are minor children."}) }),
      propertyDivision: z.string().min(1, "Property division details are required."),
...
// Create a mutable copy for further operations
export let documentLibrary: LegalDocument[] = [...initialDocumentLibrary];

// Append additional documents to the main library
documentLibrary.push(...documentLibraryAdditions);
