...
const getFirebaseConfig = () => {
  ...
  if (!config.apiKey || !config.projectId) {
    console.error(
      'Firebase config error: apiKey or projectId is missing. Check environment variables or default config.',
    );
  ...
  const usingEnvVars = Object.keys(defaultFirebaseConfig).some(
    (key) => process.env[`NEXT_PUBLIC_FIREBASE_${key.toUpperCase()}`],
  );
  console.log(
    `Firebase config loaded. Using ${usingEnvVars ? 'environment variables' : 'default values'}. Project ID: ${config.projectId}`,
  );
