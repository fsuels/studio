...
export async function loadDocumentConfig(
  ...
  console.log(
    `🌍 CONFIG LOADER: Environment - CDN: ${env.enableCDN}, Fallback: ${env.fallbackToTypeScript}`
  );
  ...
  if (env.enableCDN) {
    try {
      ...
      if (jsonConfig) {
        console.log(
          `✅ CONFIG LOADER: Loaded JSON config for ${docType}/${jurisdiction}`
        );
        return jsonConfig;
      }
    } catch (error) {
      console.warn(
        `⚠️ CONFIG LOADER: JSON config failed for ${docType}/${jurisdiction}:`,
        error
      );
    }
  }
  ...
  if (env.fallbackToTypeScript) {
    try {
      ...
      if (tsConfig) {
        console.log(
          `✅ CONFIG LOADER: Loaded TypeScript config for ${docType}/${jurisdiction}`
        );
        return tsConfig;
      }
    } catch (error) {
      console.warn(
        `⚠️ CONFIG LOADER: TypeScript config failed for ${docType}/${jurisdiction}:`,
        error
      );
    }
  }
...
async function loadJSONConfig(
  ...
  try {
    ...
    if (!rawConfig.questions && rawConfig.overlayConfig) {
      console.log(
        `🔧 JSON CONFIG: Auto-generating questions from overlay for ${docType}/${jurisdiction}`
      );
    ...
    console.log(
      `✅ JSON CONFIG: Valid configuration loaded for ${docType}/${jurisdiction}`
    );
...
async function loadTypeScriptConfig(
  ...
  console.log(
    `📦 TS CONFIG: Loading TypeScript config for ${docType}/${jurisdiction}`
  );
  ...
  try {
    ...
    if (country !== 'us') {
      throw new Error(
        `TypeScript configs only support US jurisdiction, got: ${country}`
      );
    }
    ...
    throw new Error(
      `TypeScript config not available for document type: ${docType}`
    );
  } catch (error) {
...
async function loadVehicleBillOfSaleConfig(
  ...
  if (!compliance) {
    throw new Error(
      `No compliance configuration found for state: ${state} (code: ${stateCode})`
    );
  }
  ...
  const config: DocumentConfig = {
    ...
    // ⚠️ Don't emit overlayConfig if we don't actually have mappings
    overlayConfig: undefined,
  };

  console.log(
    `🔄 TS CONFIG: Converted legacy config for ${state} Vehicle Bill of Sale`
  );
...
// Export validation for external use if needed
