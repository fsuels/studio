...
interface ChatMessage {
  ...
  content: string;
  messageType: 'text' | 'quick_reply' | 'document_suggestion' | 'form_help' | 'legal_explanation';
  intent?: string;
  confidence?: number;
  attachments?: Array<{
...
export class LegalAIAssistant {
  ...
  private escalationQueue: Array<{sessionId: string; reason: string; priority: number}> = [];
    ...
    this.knowledgeBase = [
      {
        ...
        answer: 'At-will employment means that either the employer or employee can terminate the employment relationship at any time, for any reason (except illegal reasons), with or without notice. However, employment contracts can modify this relationship.',
      ...
      },
      {
        ...
        answer: 'LLCs provide limited liability protection, tax flexibility, operational simplicity, and credibility. They protect personal assets from business debts and allow pass-through taxation while maintaining business formality.',
        keywords: ['llc', 'limited liability', 'benefits', 'formation', 'protection'],
        jurisdiction: ['US'],
      ...
      },
      {
        ...
        answer: 'NDAs are generally enforceable if they protect legitimate business interests, have reasonable scope and duration, and don\'t unreasonably restrict competition. Enforceability varies by state.',
        keywords: ['nda', 'non-disclosure', 'enforceable', 'confidentiality', 'valid'],
        jurisdiction: ['US'],
        documentTypes: ['nda'],
      ...
      },
      {
        ...
        answer: 'A lease should include rent amount, lease term, security deposit, maintenance responsibilities, pet policies, early termination clauses, and local law compliance provisions.',
        keywords: ['lease', 'rental', 'agreement', 'terms', 'landlord', 'tenant'],
        jurisdiction: ['US'],
  ...
  private initializeIntents() {
    this.intents = [
      ...
      },
      {
        ...
        description: 'User has questions about legal requirements or compliance',
        examples: [
  ...
  private analyzeIntent(message: string): { intent: string; confidence: number; entities: any } {
    ...
    for (const intent of this.intents) {
      ...
      const confidence = matches > 0 ? (score / intent.examples.length) * intent.confidence : 0;
  ...
  private extractEntities(message: string): any {
    ...
    const documentTypes = ['employment contract', 'nda', 'non-disclosure', 'lease', 'llc', 'service agreement'];
    ...
    const businessTypes = ['startup', 'corporation', 'llc', 'partnership', 'sole proprietorship'];
  ...
  private handleDocumentRecommendation(session: ChatSession, entities: any): ChatMessage {
    const recommendations = [];
    let content = "I'd be happy to help you choose the right document! ";

    if (entities.business_type) {
      if (entities.business_type.includes('startup') || entities.business_type.includes('llc')) {
        recommendations.push({
          type: 'document_suggestion',
          url: '/docs/llc-operating-agreement/start',
          title: 'LLC Operating Agreement'
        });
        content += "For your business, I recommend starting with an LLC Operating Agreement. ";
      }
    }
    ...
    if (recommendations.length === 0) {
      content += "To give you the best recommendation, could you tell me more about your situation?";
    }
  ...
  private handleLegalExplanation(message: string, entities: any): ChatMessage {
    ...
    if (relevantKB) {
      ...
      explanation = "I'd be happy to explain legal concepts! Could you specify which term or concept you'd like me to explain?";
    }
  ...
  private handleFormAssistance(session: ChatSession, entities: any): ChatMessage {
    ...
    if (entities.field_name) {
      ...
      if (entities.field_name.includes('name')) {
        content += "Enter the full legal name as it appears on official documents.";
      } else if (entities.field_name.includes('address')) {
        content += "Use the complete mailing address including street, city, state, and ZIP code.";
  ...
  private handleComplianceQuestion(session: ChatSession, entities: any): ChatMessage {
    let content = "Legal compliance requirements vary by state and document type. ";
    ...
    if (entities.jurisdiction && entities.document_type) {
      ...
      if (entities.jurisdiction.includes('california')) {
        content += "California has specific disclosure requirements and employee protections that must be included.";
      }
    } else {
      content += "To provide accurate compliance information, please let me know your state and document type.";
    }
  ...
  private handlePricingQuestion(session: ChatSession, entities: any): ChatMessage {
...
What would you like help with today?`;
  ...
  private getWelcomeQuickReplies(language: string): string[] {
    const quickReplies: Record<string, string[]> = {
      'en': ['Choose a document', 'Explain legal terms', 'Pricing info', 'Get started'],
      'es': ['Elegir documento', 'Explicar términos', 'Info de precios', 'Comenzar'],
  ...
  // End chat session
  endSession(sessionId: string, satisfactionRating?: number): { success: boolean; summary: any } {
    const session = this.sessions.get(sessionId);
    if (!session) {
      throw new Error(`Session ${sessionId} not found`);
    }
  ...
  getPerformanceMetrics(): {
    ...
    const escalationRate = allSessions.length > 0 ? (escalatedSessions / allSessions.length) * 100 : 0;
    ...
    const durations = allSessions
      ...
      .map(s => new Date(s.lastActivity).getTime() - new Date(s.startedAt).getTime());
