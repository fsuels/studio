...
class AIUsageAnalyticsEngine {
  ...
  getModelPerformanceComparison(endpoint?: AIEndpoint): Array<{
    ...
    return Object.entries(modelGroups)
      .map(([model, modelMetrics]) => {
        const successfulMetrics = modelMetrics.filter((m) => m.success);
        const totalCost = modelMetrics.reduce(
          (sum, m) => sum + m.totalCostUsd,
          0,
        );
        const avgLatency =
          modelMetrics.reduce((sum, m) => sum + m.latencyMs, 0) /
          modelMetrics.length;
        const successRate =
          (successfulMetrics.length / modelMetrics.length) * 100;
        const avgQuality =
          successfulMetrics
  ...
  getCostOptimizationOpportunities(): AIUsageAnalytics['optimization']['costSavingOpportunities'] {
    ...
    if (expensiveOperations.length > 0) {
      const totalCost = expensiveOperations.reduce(
        (sum, m) => sum + m.totalCostUsd,
        0,
      );
      opportunities.push({
        ...
        impact: 'high',
        recommendation:
          'Review prompt efficiency and consider switching to cheaper models for non-critical operations',
      });
    }
    ...
    for (const [model, modelMetrics] of Object.entries(modelRetryRates)) {
      const retryRate =
        modelMetrics.filter((m) => m.retryCount > 0).length /
        modelMetrics.length;
      if (retryRate > 0.15) {
        // More than 15% retry rate
        ...
        opportunities.push({
          ...
          impact: 'medium',
          recommendation:
            'Optimize prompts to reduce model confusion and improve success rate',
        });
      }
    }
    ...
    if (longPrompts.length > 10) {
      const promptCost = longPrompts.reduce(
        (sum, m) => sum + m.promptCostUsd,
        0,
      );
      opportunities.push({
        ...
        impact: 'medium',
        recommendation:
          'Optimize prompt templates to reduce token usage while maintaining quality',
      });
    }
  ...
  private calculateCosts(
    ...
    if (!costs) {
      ...
      const completionCost =
        (completionTokens / 1000) * defaultCosts.completionCostPer1000;
      return {
        promptCost,
        completionCost,
    ...
    const completionCost =
      (completionTokens / 1000) * costs.completionCostPer1000;
  ...
  private calculateOverviewMetrics(metrics: AIUsageMetric[]) {
    ...
    const successRate =
      (metrics.filter((m) => m.success).length / totalRequests) * 100;

    // Documents with AI usage
    const documentsWithAI = new Set(
      metrics.filter((m) => m.documentId).map((m) => m.documentId),
    ).size;
    const costPerDocument =
      documentsWithAI > 0 ? totalCost / documentsWithAI : 0;
    ...
    for (const [model, modelMetrics] of Object.entries(modelCosts)) {
      const modelCost = modelMetrics.reduce(
        (sum, m) => sum + m.totalCostUsd,
        0,
      );
      const modelSuccess =
        modelMetrics.filter((m) => m.success).length / modelMetrics.length;
    ...
    const topEndpoint =
      (Object.entries(endpointUsage).sort(
        ([, a], [, b]) => b.length - a.length,
      )[0]?.[0] as AIEndpoint) || 'document_generation';
  ...
  private calculateModelBreakdown(metrics: AIUsageMetric[]) {
    ...
    return Object.entries(modelGroups)
      .map(([model, modelMetrics]) => {
        ...
        const avgLatency =
          modelMetrics.reduce((sum, m) => sum + m.latencyMs, 0) / requests;
        const successRate =
          (modelMetrics.filter((m) => m.success).length / requests) * 100;
  ...
  private calculateEndpointAnalysis(metrics: AIUsageMetric[]) {
    ...
    return Object.entries(endpointGroups)
      .map(([endpoint, endpointMetrics]) => {
        ...
        const cost = endpointMetrics.reduce(
          (sum, m) => sum + m.totalCostUsd,
          0,
        );
        const avgLatency =
          endpointMetrics.reduce((sum, m) => sum + m.latencyMs, 0) / requests;
        const successRate =
          (endpointMetrics.filter((m) => m.success).length / requests) * 100;
        ...
        const topModel =
          (Object.entries(modelUsage).sort(
            ([, a], [, b]) => b.length - a.length,
          )[0]?.[0] as AIModel) || 'gpt-3.5-turbo';

        // Calculate business impact
        const conversions = endpointMetrics.filter(
          (m) => m.conversionContribution,
        ).length;
  ...
  private calculateTrends(metrics: AIUsageMetric[], timeframe: string) {
    ...
    const dailyCosts = Object.entries(dailyGroups)
      .map(([date, dayMetrics]) => ({
        date,
    ...
    // Latency trends
    const latencyTrends = Object.entries(dailyGroups)
      .map(([date, dayMetrics]) => {
        const latencies = dayMetrics
          .map((m) => m.latencyMs)
          .sort((a, b) => a - b);
        const avgLatency =
    ...
    // Quality trends
    const qualityTrends = Object.entries(dailyGroups)
      .map(([date, dayMetrics]) => {
        const qualityMetrics = dayMetrics.filter(
          (m) => m.qualityScore && m.userRating,
        );
        const avgQuality =
        ...
        const userRating =
          qualityMetrics.length > 0
            ? qualityMetrics.reduce((sum, m) => sum + (m.userRating || 0), 0) /
              qualityMetrics.length
            : 0;
  ...
  private getModelRecommendations(metrics: AIUsageMetric[]) {
    ...
    for (const [endpoint, endpointMetrics] of Object.entries(endpointGroups)) {
      const modelPerformance = this.getModelPerformanceComparison(
        endpoint as AIEndpoint,
      );
      ...
      if (modelPerformance.length >= 2) {
        ...
        if (current && recommended && current.model !== recommended.model) {
          const costSaving =
            (current.avgCost - recommended.avgCost) * current.usage;
          ...
          if (costSaving > 1) {
            ...
            recommendations.push({
              ...
              costSaving,
              performanceImpact:
                recommended.avgLatency < current.avgLatency
                  ? 'Faster'
                  : 'Slightly slower',
            });
          }
  ...
  private getPromptOptimizations(metrics: AIUsageMetric[]) {
    ...
    for (const [endpoint, endpointMetrics] of Object.entries(endpointGroups)) {
      const avgPromptLength =
        endpointMetrics.reduce((sum, m) => sum + m.promptLength, 0) /
        endpointMetrics.length;
      const avgSystemPromptLength =
        endpointMetrics.reduce((sum, m) => sum + m.systemPromptLength, 0) /
        endpointMetrics.length;
      ...
      if (avgPromptLength > 6000) {
        ...
        const costSaving =
          endpointMetrics.reduce((sum, m) => sum + m.promptCostUsd, 0) * 0.2;

        optimizations.push({
          endpoint: endpoint as AIEndpoint,
          issue: 'Prompts are longer than optimal',
          suggestion:
            'Review prompt templates for redundant instructions and examples',
      ...
      if (avgSystemPromptLength > 2000) {
        ...
        const costSaving = endpointMetrics.reduce(
          (sum, m) => sum + m.promptCostUsd * 0.3,
          0,
        );
        ...
        optimizations.push({
          ...
          suggestion:
            'Streamline system prompts to essential instructions only',
          estimatedTokenSaving: tokenSaving,
          estimatedCostSaving: costSaving,
        });
      }
    ...
    return optimizations.sort(
      (a, b) => b.estimatedCostSaving - a.estimatedCostSaving,
    );
  }
  ...
  private calculateBudgetTracking(metrics: AIUsageMetric[]) {
    ...
    const currentMonthMetrics = metrics.filter((m) => {
      ...
      return (
        metricDate.getMonth() === currentMonth &&
        metricDate.getFullYear() === currentYear
      );
    });

    const currentSpend = currentMonthMetrics.reduce(
      (sum, m) => sum + m.totalCostUsd,
      0,
    );
  ...
  private groupBy<T>(
    array: T[],
    keyFunc: string | ((item: T) => string),
  ): Record<string, T[]> {
    const groups: Record<string, T[]> = {};

    for (const item of array) {
      const key =
        typeof keyFunc === 'string' ? (item as any)[keyFunc] : keyFunc(item);
      if (!groups[key]) {
        groups[key] = [];
      }
  ...
  private async updateRealTimeAggregations(
    ...
    console.log(
      `Updating real-time aggregations for ${metric.endpoint} with ${metric.model}`,
    );
  }
