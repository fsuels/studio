...
class EnhancedPdfGenerator {
  ...
  async generateDocument(
    ...
    try {
      ...
      const config = PDFFooterManager.getFooterConfigForDocumentType(
  ...
  private async generateDocumentContent(
    ...
    for (const [key, value] of Object.entries(options.content)) {
      ...
      const contentLines = this.wrapText(
        sectionContent,
        currentPage.getSize().width - 2 * margin,
        font,
        11,
      );
  ...
  private async addSignatureSection(
    ...
    if (config.requiresNotarySection) {
      ...
      const adjustedSealPosition =
        this.footerManager?.calculateNotarySealPosition(
          currentPage,
  ...
  private needsSignatureSection(documentType: string): boolean {
    ...
    return !['receipt', 'invoice', 'report', 'certificate'].some((keyword) =>
