...
import { FOOTER_NOTE, FOOTER_STYLE, FOOTER_CONFIG, UPL_PROTECTION } from '../pdf-constants';
...
class PDFFooterManager {
  ...
  constructor(font: PDFFont) {
    this.font = font;
    this.footerHeight = FOOTER_STYLE.fontSize + FOOTER_STYLE.margin.bottom + FOOTER_CONFIG.minContentGap;
  }
  ...
  addFooter(page: PDFPage, options: FooterOptions = {}): PageLayout {
    const { hasNotarySeal = false, isFinalPage = false, customText, language = 'en' } = options;
    const pageSize = page.getSize();
    
    // Determine footer text
    const footerText = customText || (language === 'es' ? 
    ...
    if (hasNotarySeal && isFinalPage && FOOTER_CONFIG.autoScale) {
      ...
      console.log(`[FooterManager] Auto-scaling: Footer moved up ${FOOTER_CONFIG.notarySealOffset}" for notary seal`);
    }

    // Center the footer text
    const textWidth = this.estimateTextWidth(footerText, FOOTER_STYLE.fontSize);
    const maxWidth = pageSize.width - (FOOTER_STYLE.margin.left + FOOTER_STYLE.margin.right);
   ...
   */
  canFitContent(page: PDFPage, requiredHeight: number, options: FooterOptions = {}): boolean {
  ...
  validateUPLCompliance(pages: PDFPage[]): {
    ...
    if (FOOTER_STYLE.fontSize < 6) {
      issues.push('Footer font size too small for legal compliance (minimum 6pt recommended)');
    }

    // Check for severability protection
    if (pages.length > 1 && !UPL_PROTECTION.severabilityProtection) {
      recommendations.push('Multi-page documents benefit from per-page disclaimers for severability protection');
    }

    // Validate copy/scan resilience
    if (!UPL_PROTECTION.copyScanResilience) {
      recommendations.push('Consider adding footer to every page for copy/scan resilience');
    }
  ...
  addFooterToAllPages(
    ...
    console.log(`[FooterManager] Added footers to ${pages.length} pages with UPL protection`);
    return layouts;
  }
   ...
   */
  calculateNotarySealPosition(page: PDFPage, defaultX: number, defaultY: number): { x: number; y: number } {
    const layout = this.calculatePageLayout(page, { hasNotarySeal: true, isFinalPage: true });
    ...
    if (adjustedY !== defaultY) {
      console.log(`[FooterManager] Notary seal moved from Y:${defaultY} to Y:${adjustedY} to avoid footer clash`);
    }
