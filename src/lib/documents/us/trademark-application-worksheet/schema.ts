...
export const TrademarkApplicationWorksheetSchema = z.object({
  ...
  trademarkType: z.enum(['word', 'design', 'combined'], { errorMap: () => ({ message: 'Trademark type is required.' }) }),
  trademarkDescription: z.string().min(1, 'Trademark description is required.'),
  goodsServices: z.string().min(1, 'Description of goods/services is required.'),
  ...
  signatureDate: z.string().min(1, 'Signature date is required.').regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format'),
});

export type TrademarkApplicationWorksheetData = z.infer<typeof TrademarkApplicationWorksheetSchema>;