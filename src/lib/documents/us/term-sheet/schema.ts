...
export const termSheetSchema = z.object({
  ...
  // Valuation
  preMoneyValuation: z.number().min(0, 'Pre-money valuation must be non-negative'),
  postMoneyValuation: z.number().min(0, 'Post-money valuation must be non-negative'),
  
  // Investor Information
  leadInvestor: z.string().min(1, 'Lead investor name is required'),
  investorType: z.enum(['individual', 'venture-capital', 'private-equity', 'angel', 'strategic']),
  ...
  antiDilutionProvision: z.enum(['weighted-average', 'full-ratchet', 'none']).default('weighted-average'),
  ...
  protectiveProvisions: z.array(z.string()).default([
  ...
  // Closing Conditions
  closingConditions: z.array(z.string()).default([
