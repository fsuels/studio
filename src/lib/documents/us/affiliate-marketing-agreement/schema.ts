...
export const AffiliateMarketingAgreementSchema = z.object({
  ...
  commissionType: z.enum(['percentage', 'fixed', 'tiered', 'hybrid']).default('percentage'),
  ...
  // Payment Terms
  paymentSchedule: z.enum(['weekly', 'bi-weekly', 'monthly', 'quarterly']).default('monthly'),
  paymentMethod: z.enum(['check', 'direct-deposit', 'paypal', 'wire-transfer']).default('direct-deposit'),
  minimumPayoutThreshold: z.string().optional(),
  paymentDelay: z.string().optional(),
  currency: z.string().default('USD'),
  
  // Tracking and Attribution
  trackingMethod: z.enum(['cookies', 'links', 'codes', 'pixels']).default('cookies'),
  ...
  reportingFrequency: z.enum(['real-time', 'daily', 'weekly', 'monthly']).default('real-time'),
  dashboardAccess: z.boolean().default(true),
  
  // Customer Service
  customerSupportResponsibility: z.enum(['merchant', 'affiliate', 'shared']).default('merchant'),
  returnsHandling: z.enum(['merchant', 'affiliate', 'shared']).default('merchant'),
  refundsHandling: z.enum(['merchant', 'affiliate', 'shared']).default('merchant'),
  chargebacksResponsibility: z.enum(['merchant', 'affiliate', 'shared']).default('merchant'),
  ...
  communicationMethod: z.enum(['email', 'phone', 'portal', 'slack']).default('email'),
  responseTimeframe: z.string().optional(),
  meetingFrequency: z.enum(['weekly', 'monthly', 'quarterly', 'as-needed']).default('as-needed'),
  ...
  intellectualPropertyOwnership: z.enum(['merchant', 'affiliate', 'shared']).default('merchant'),
