...
export const FranchiseDisclosureAgreementSchema = z.object({
  ...
  businessType: z.enum(['restaurant', 'retail', 'service', 'hospitality', 'automotive', 'education', 'other']).optional(),
  systemDescription: z.string().optional(),
  numberOfUnits: z.string().optional(),
  companyOwnedUnits: z.string().optional(),
  
  // Territory and Location
  territoryGrant: z.enum(['exclusive', 'non-exclusive', 'protected', 'none']).default('exclusive'),
  territoryDescription: z.string().optional(),
  territoryRadius: z.string().optional(),
  siteSelection: z.enum(['franchisor', 'franchisee', 'mutual']).default('mutual'),
  ...
  royaltyBase: z.enum(['gross-sales', 'net-sales', 'fixed', 'sliding-scale']).default('gross-sales'),
