...
export const LotteryPoolContractSchema = z.object({
  ...
  // Pool Rules
  purchaseFrequency: z.enum(['weekly', 'bi-weekly', 'monthly', 'per-drawing']).default('weekly'),
  numberSelection: z.enum(['quick-pick', 'group-choice', 'administrator-choice']).default('quick-pick'),
  missedPaymentPolicy: z.string().optional(),
  
  // Winnings Distribution
  winningsDistribution: z.enum(['equal-shares', 'proportional', 'contribution-based']).default('equal-shares'),
  minimumPayout: z.string().optional(),
  taxResponsibility: z.enum(['individual', 'pool', 'shared']).default('individual'),
  ...
  communicationMethod: z.enum(['email', 'text', 'phone', 'in-person']).default('email'),
  winningsNotification: z.boolean().default(true),
  
  // Dispute Resolution
  disputeResolution: z.enum(['discussion', 'mediation', 'arbitration']).default('discussion'),
