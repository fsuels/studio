...
export const RestaurantAgreementSchema = z.object({
  ...
  // Agreement Type
  agreementType: z.enum(['partnership', 'management', 'franchise', 'lease', 'supplier', 'employment']).default('partnership'),
  agreementDescription: z.string().optional(),
  
  // Restaurant Information
  restaurantName: z.string().min(1, 'Restaurant name is required'),
  restaurantAddress: z.string().optional(),
  restaurantType: z.enum(['fast-food', 'casual-dining', 'fine-dining', 'cafe', 'bar', 'food-truck', 'other']).default('casual-dining'),
  ...
  // Menu and Pricing
  menuControl: z.enum(['joint', 'primary-party', 'secondary-party', 'chef']).default('joint'),
  pricingControl: z.enum(['joint', 'primary-party', 'secondary-party', 'manager']).default('joint'),
  menuChangeApproval: z.boolean().default(true),
  specialMenuItems: z.string().optional(),
  
  // Staffing
  staffingResponsibility: z.enum(['joint', 'primary-party', 'secondary-party', 'manager']).default('joint'),
  ...
  supplierSelection: z.enum(['joint', 'primary-party', 'secondary-party', 'manager']).default('joint'),
  foodCostTargets: z.string().optional(),
  inventoryManagement: z.string().optional(),
  qualityStandards: z.string().optional(),
  
  // Marketing and Branding
  brandingControl: z.enum(['joint', 'primary-party', 'secondary-party']).default('joint'),
  ...
  // Dispute Resolution
  disputeResolutionMethod: z.enum(['mediation', 'arbitration', 'court']).default('mediation'),
  mediationProcedures: z.string().optional(),
  governingLaw: z.string().optional(),
  
  // Intellectual Property
  trademarkOwnership: z.enum(['joint', 'primary-party', 'secondary-party']).default('joint'),
  recipeOwnership: z.enum(['joint', 'primary-party', 'secondary-party', 'chef']).default('joint'),
