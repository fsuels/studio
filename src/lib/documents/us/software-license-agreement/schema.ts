...
export const SoftwareLicenseAgreementSchema = z.object({
  ...
  organizationType: z
    .enum(['individual', 'business', 'non-profit', 'government'])
    .default('individual'),
  ...
  // License Type
  licenseType: z
    .enum(['perpetual', 'subscription', 'trial', 'educational', 'enterprise'])
    .default('perpetual'),
  licenseScope: z
    .enum(['single-user', 'multi-user', 'site-license', 'enterprise'])
    .default('single-user'),
  ...
  paymentSchedule: z
    .enum(['one-time', 'monthly', 'annual', 'quarterly'])
    .default('one-time'),
  maintenanceFee: z.string().optional(),
  supportFee: z.string().optional(),
  paymentMethod: z
    .enum(['credit-card', 'check', 'wire-transfer', 'purchase-order'])
    .default('credit-card'),
