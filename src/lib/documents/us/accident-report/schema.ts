...
export const accidentReportSchema = z.object({
  ...
  accidentInfo: z.object({
    ...
    dateOfAccident: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format'),
    timeOfAccident: z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Time must be in HH:MM format'),
    location: z.string().min(1, 'Accident location is required'),
    city: z.string().min(1, 'City is required'),
    state: z.string().min(2, 'State is required'),
    zipCode: z.string().regex(/^\d{5}(-\d{4})?$/, 'Invalid ZIP code'),
    county: z.string().optional(),
    weatherConditions: z.enum(['clear', 'cloudy', 'rain', 'snow', 'fog', 'other']),
    roadConditions: z.enum(['dry', 'wet', 'icy', 'snowy', 'construction', 'other']),
    lightConditions: z.enum(['daylight', 'dawn', 'dusk', 'dark_lighted', 'dark_unlighted']),
    trafficControl: z.enum(['none', 'traffic_signal', 'stop_sign', 'yield_sign', 'officer', 'other']).optional()
  }),
  ...
  vehicle1: z.object({
    ...
    vehicle: z.object({
      ...
      vehicleType: z.enum(['car', 'truck', 'suv', 'van', 'motorcycle', 'bus', 'other'])
    }),
    insurance: z.object({
      company: z.string().min(1, 'Insurance company is required'),
      policyNumber: z.string().min(1, 'Policy number is required'),
      agentName: z.string().optional(),
      agentPhone: z.string().regex(/^\(\d{3}\) \d{3}-\d{4}$/, 'Phone must be in format (XXX) XXX-XXXX').optional()
    }),
    damage: z.object({
  ...
  vehicle2: z.object({
    ...
    vehicle: z.object({
      ...
      plateState: z.string().optional(),
      vin: z.string().optional(),
      vehicleType: z.enum(['car', 'truck', 'suv', 'van', 'motorcycle', 'bus', 'other']).optional()
    }).optional(),
    insurance: z.object({
  ...
  description: z.object({
    howAccidentOccurred: z.string().min(50, 'Please provide a detailed description (at least 50 characters)'),
  ...
  // Report Completion
  reportInfo: z.object({
    completedBy: z.string().min(1, 'Name of person completing report is required'),
