...
export const ArchitectContractSchema = z.object({
  ...
  projectType: z.enum(['residential-new', 'residential-renovation', 'commercial-new', 'commercial-renovation', 'institutional', 'industrial', 'mixed-use']),
  ...
  // Compensation
  feeStructure: z.enum(['fixed-fee', 'hourly-rate', 'percentage-of-construction', 'cost-plus-fee']),
  totalFee: z.string().optional(),
  hourlyRate: z.string().optional(),
  percentageOfConstruction: z.string().optional(),
  
  // Payment Terms
  paymentSchedule: z.enum(['monthly', 'phase-based', 'milestone-based', 'other']),
  ...
  professionalLiabilityInsurance: z.string().min(1, 'Professional liability insurance is required'),
