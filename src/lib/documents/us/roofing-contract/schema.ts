...
export const RoofingContractSchema = z.object({
  ...
  roofingType: z.enum(['repair', 'replacement', 'new-installation', 'partial-replacement', 'inspection-repair']),
  roofType: z.enum(['asphalt-shingles', 'metal', 'tile', 'slate', 'flat-roof', 'wood-shingles', 'other']),
  ...
  // Payment Terms
  paymentSchedule: z.enum(['down-payment-completion', 'progress-payments', 'payment-on-completion']),
  ...
  permitCosts: z.enum(['owner-pays', 'contractor-pays', 'included-in-price']).optional(),
  inspectionRequired: z.boolean().default(false),
  
  // Cleanup and Waste
  debrisRemoval: z.boolean().default(true),
  debrisDisposal: z.enum(['contractor-handles', 'owner-handles', 'shared-responsibility']),
