...
export const ParentingPlanSchema = z.object({
  ...
  // Legal Custody
  legalCustody: z.enum(['joint', 'parent1-sole', 'parent2-sole']).default('joint'),
  majorDecisions: z.enum(['joint', 'parent1', 'parent2']).default('joint'),
  educationalDecisions: z.enum(['joint', 'parent1', 'parent2']).default('joint'),
  medicalDecisions: z.enum(['joint', 'parent1', 'parent2']).default('joint'),
  religiousDecisions: z.enum(['joint', 'parent1', 'parent2']).default('joint'),
  
  // Physical Custody
  physicalCustody: z.enum(['joint', 'parent1-primary', 'parent2-primary']).default('joint'),
  ...
  // Transportation
  transportationResponsibility: z.enum(['parent1', 'parent2', 'shared', 'exchanging-parent']).default('exchanging-parent'),
  exchangeLocation: z.string().optional(),
  exchangeTime: z.string().optional(),
  longDistanceTransportation: z.enum(['shared', 'requesting-parent']).default('requesting-parent'),
  
  // Communication
  communicationMethod: z.enum(['phone', 'video-call', 'text', 'email']).default('phone'),
  communicationSchedule: z.string().optional(),
  phoneAccessRules: z.string().optional(),
  parentCommunication: z.enum(['direct', 'through-app', 'email-only']).default('direct'),
  
  // Child Support
  childSupportPayer: z.enum(['parent1', 'parent2', 'none']).optional(),
  childSupportAmount: z.string().optional(),
  childSupportSchedule: z.enum(['weekly', 'bi-weekly', 'monthly']).default('monthly'),
  extraExpenses: z.enum(['shared', 'parent1', 'parent2']).default('shared'),
  
  // Education
  schoolChoice: z.enum(['mutual-agreement', 'residential-parent', 'parent1', 'parent2']).default('mutual-agreement'),
  schoolActivities: z.enum(['both-attend', 'residential-parent', 'alternating']).default('both-attend'),
  parentTeacherConferences: z.enum(['both-attend', 'separate', 'alternating']).default('both-attend'),
  educationalExpenses: z.enum(['shared', 'parent1', 'parent2']).default('shared'),
  
  // Healthcare
  healthInsuranceProvider: z.enum(['parent1', 'parent2', 'both']).default('parent1'),
  medicalExpenses: z.enum(['shared', 'parent1', 'parent2']).default('shared'),
  medicalDecisionMaking: z.enum(['joint', 'parent1', 'parent2']).default('joint'),
  emergencyMedicalCare: z.string().optional(),
  
  // Extracurricular Activities
  activityDecisions: z.enum(['joint', 'residential-parent', 'parent1', 'parent2']).default('joint'),
  activityTransportation: z.enum(['shared', 'enrolling-parent', 'residential-parent']).default('enrolling-parent'),
  ...
  // Dispute Resolution
  disputeResolution: z.enum(['direct-discussion', 'mediation', 'parenting-coordinator', 'court']).default('mediation'),
  mediatorSelection: z.enum(['mutual-agreement', 'court-appointed']).default('mutual-agreement'),
  ...
  reviewSchedule: z.enum(['annual', 'bi-annual', 'as-needed']).default('as-needed'),
