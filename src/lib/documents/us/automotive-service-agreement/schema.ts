...
export const AutomotiveServiceAgreementSchema = z.object({
  ...
  // Service Details
  serviceType: z.enum(['repair', 'maintenance', 'inspection', 'diagnostic', 'comprehensive']).default('repair'),
  ...
  // Parts and Materials
  partsSupplier: z.enum(['oem', 'aftermarket', 'refurbished', 'customer-supplied']).default('oem'),
  ...
  // Payment Terms
  paymentMethod: z.enum(['cash', 'check', 'credit-card', 'financing']).default('credit-card'),
  paymentTerms: z.enum(['due-on-completion', 'net-30', 'deposit-required', 'progress-payments']).default('due-on-completion'),
