...
export const MaritimeCharterAgreementSchema = z.object({
  ...
  vesselType: z.enum(['cargo', 'tanker', 'container', 'bulk-carrier', 'yacht', 'fishing', 'passenger']).optional(),
  ...
  // Charter Type
  charterType: z.enum(['bareboat', 'time-charter', 'voyage-charter', 'demise-charter']).default('time-charter'),
  ...
  paymentTerms: z.enum(['daily', 'monthly', 'lump-sum', 'per-voyage']).default('monthly'),
  ...
  maintenanceResponsibility: z.enum(['owner', 'charterer', 'shared']).default('owner'),
  ...
  // Dispute Resolution
  disputeResolution: z.enum(['arbitration', 'court', 'mediation']).default('arbitration'),
