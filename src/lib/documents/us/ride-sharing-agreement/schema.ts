...
export const RideSharingAgreementSchema = z.object({
  ...
  // Trip Details
  tripType: z.enum(['one-time', 'recurring', 'daily-commute', 'occasional']).default('one-time'),
  ...
  // Cost Sharing
  costSharingMethod: z.enum(['per-trip', 'weekly', 'monthly', 'per-mile', 'gas-only']).default('per-trip'),
  ...
  // Payment Terms
  paymentMethod: z.enum(['cash', 'venmo', 'paypal', 'split-apps', 'other']).default('cash'),
  paymentSchedule: z.enum(['upfront', 'end-of-trip', 'weekly', 'monthly']).default('end-of-trip'),
