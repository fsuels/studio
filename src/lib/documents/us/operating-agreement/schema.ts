...
export const operatingAgreementSchema = z.object({
  ...
  principal_office_address: z.string().min(1, 'Principal office address is required'),
  registered_agent_name: z.string().min(1, 'Registered agent name is required'),
  registered_agent_address: z.string().min(1, 'Registered agent address is required'),
  federal_ein: z.string().regex(/^\d{2}-\d{7}$/, 'EIN must be in format XX-XXXXXXX').optional().or(z.literal('')),
  articles_filing_date: z.string().min(1, 'Articles filing date is required'),
  business_purpose: z.string().min(10, 'Business purpose must be at least 10 characters'),

  // Member 1 (Required)
  member_1_name: z.string().min(1, 'Member 1 name is required'),
  member_1_address: z.string().min(1, 'Member 1 address is required'),
  member_1_contribution: z.number().min(0, 'Contribution must be 0 or greater'),
  member_1_percentage: z.number().min(0).max(100, 'Percentage must be between 0 and 100'),
  ...
  total_initial_capital: z.number().min(0, 'Total capital must be 0 or greater'),
  ...
  ordinary_decision_authority: z.string().min(1, 'Decision authority is required'),
  major_decision_threshold: z.string().min(1, 'Major decision threshold is required'),
  ...
  financial_statement_deadline: z.string().min(1, 'Financial statement deadline is required'),
  banking_authority: z.string().default('Manager(s)'),

  // Transfer Restrictions
  transfer_approval_requirement: z.string().min(1, 'Transfer approval requirement is required'),
  ...
  amendment_requirement: z.string().default('unanimous written consent of all Members'),
  ...
  fiduciary_duty_provisions: z.string().default('Members owe fiduciary duties to the Company and each other as provided by law'),
  tax_elections: z.string().default('The Company shall elect to be taxed as a partnership for federal income tax purposes')
}).refine((data) => {
  // Validate member 2 fields if member 2 name is provided
  if (data.member_2_name) {
    return data.member_2_address && 
  ...
  message: "All Member 2 fields are required when Member 2 name is provided",
  path: ["member_2_name"]
}).refine((data) => {
  // Validate member 3 fields if member 3 name is provided
  if (data.member_3_name) {
    return data.member_3_address && 
  ...
  message: "All Member 3 fields are required when Member 3 name is provided",
  path: ["member_3_name"]
}).refine((data) => {
  // Validate ownership percentages add up to 100
  const totalPercentage = data.member_1_percentage + 
  ...
  const totalMemberUnits = data.member_1_units + 
  ...
  const totalContributions = data.member_1_contribution + 
                            (data.member_2_contribution || 0) + 
                            (data.member_3_contribution || 0);
  return totalContributions === data.total_initial_capital;
}, {
  message: "Total initial capital must equal sum of all member contributions",
  ...
  return {
    ...
    mediation_location: data.mediation_required ? data.governing_state : undefined,
    arbitration_location: data.arbitration_required ? data.governing_state : undefined,
    litigation_jurisdiction: !data.arbitration_required ? data.governing_state : undefined
  };
});