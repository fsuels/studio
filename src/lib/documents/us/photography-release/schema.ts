...
export const PhotographyReleaseSchema = z.object({
  ...
  photoType: z.enum(['portrait', 'event', 'commercial', 'artistic', 'documentary']).optional(),
  ...
  compensationType: z.enum(['cash', 'prints', 'credit', 'none']).default('none'),
  ...
  copyrightOwner: z.enum(['photographer', 'subject', 'shared']).default('photographer'),
  exclusiveRights: z.boolean().default(false),
  transferRights: z.boolean().default(false),
  
  // Release Scope
  releaseScope: z.enum(['limited', 'unlimited', 'specific-project']).default('limited'),
