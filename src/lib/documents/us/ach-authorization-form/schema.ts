...
export const AchAuthorizationFormSchema = z.object({
  ...
  routingNumber: z.string().min(9).max(9).regex(/^\d{9}$/, 'Routing number must be 9 digits'),
  accountNumber: z.string().min(1),
  accountType: z.enum(['Checking', 'Savings']),
  transactionType: z.enum(['One-time', 'Recurring']),
  paymentAmount: z.coerce.number().positive(),
  paymentFrequency: z.enum(['Weekly', 'Bi-weekly', 'Monthly', 'Quarterly']).optional(),
...
export type AchAuthorizationFormData = z.infer<typeof AchAuthorizationFormSchema>;
