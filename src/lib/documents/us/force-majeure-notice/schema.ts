...
export const forceMajeureNoticeSchema = z.object({
  ...
  event: z.object({
    ...
    eventDescription: z
      ...
      .min(
        100,
        'Please provide detailed description (at least 100 characters)',
      ),
    eventStartDate: z
      .string()
      .regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format'),
  ...
  performanceImpact: z.object({
    affectedObligations: z
      .array(
        z.object({
          obligation: z.string(),
          impactType: z.enum([
            'complete_impossibility',
            'partial_impossibility',
            'significant_delay',
            'increased_cost',
          ]),
    ...
    causalConnection: z
      .string()
      .min(50, 'Please explain how the event prevents performance'),
  ...
  communication: z.object({
    ...
    ongoingCommunication: z.object({
      ...
      communicationMethod: z
        .enum(['email', 'phone', 'written', 'meetings'])
        .optional(),
    }),
