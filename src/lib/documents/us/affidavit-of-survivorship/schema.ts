...
export const AffidavitOfSurvivorshipSchema = z.object({
  ...
  survivorSSN: z.string().min(1, "Survivor's Social Security Number is required."),
  ...
  propertyType: z.enum(['real_estate', 'bank_account', 'investment_account', 'vehicle', 'other']),
  propertyTypeOther: z.string().optional(),

  // Ownership Information
  ownershipType: z.enum(['joint_tenants', 'tenants_entirety', 'joint_account', 'other']),
  ownershipTypeOther: z.string().optional(),
  ownershipPercentages: z.string().min(1, 'Ownership percentages are required.'),
  
  // Original Ownership Documentation
  originalDeedDate: z.string().min(1, 'Original deed/document date is required.'),
  ...
  survivorshipDescription: z.string().min(1, 'Survivorship description is required.'),
