...
export const HuntingLeaseAgreementSchema = z.object({
  ...
  propertyOwnershipType: z.enum(['individual', 'corporation', 'partnership', 'trust']).default('individual'),
  ...
  // Hunting Rights
  huntingRightsType: z.enum(['exclusive', 'non-exclusive', 'shared']).default('exclusive'),
  gameTypes: z.array(z.enum(['deer', 'turkey', 'duck', 'dove', 'quail', 'rabbit', 'squirrel', 'wild-hog', 'elk', 'bear', 'waterfowl'])).default(['deer']),
  huntingMethods: z.array(z.enum(['bow', 'rifle', 'shotgun', 'muzzleloader', 'crossbow'])).default(['bow', 'rifle']),
  bagLimits: z.string().optional(),
  trophyRestrictions: z.string().optional(),
  
  // Payment Terms
  annualFee: z.string().optional(),
  paymentSchedule: z.enum(['annual', 'semi-annual', 'seasonal']).default('annual'),
  ...
  equipmentOwnership: z.enum(['lessee', 'landowner', 'shared']).default('lessee'),
  standInsurance: z.boolean().default(false),
  standInspection: z.boolean().default(true),
  equipmentMaintenance: z.enum(['lessee', 'landowner', 'shared']).default('lessee'),
