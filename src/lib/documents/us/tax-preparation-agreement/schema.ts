...
export const TaxPreparationAgreementSchema = z.object({
  ...
  // Fee Structure
  feeStructure: z.enum(['flat-fee', 'hourly', 'per-form', 'complexity-based']).default('flat-fee'),
  baseFee: z.string().optional(),
  hourlyRate: z.string().optional(),
  additionalFormFees: z.boolean().default(false),
  complexityFactors: z.array(z.string()).default([]),
  paymentSchedule: z.enum(['upfront', 'completion', 'split']).default('completion'),
  ...
  accuracyStandard: z.enum(['reasonable-care', 'best-efforts', 'professional-standard']).default('professional-standard'),
  clientReviewRequired: z.boolean().default(true),
  preparerLiability: z.enum(['limited', 'fee-limitation', 'full-liability']).default('limited'),
  errorsOmissionsInsurance: z.boolean().default(false),
  liabilityLimitation: z.string().optional(),
  
  // Penalties and Interest
  penaltyResponsibility: z.enum(['client', 'preparer', 'case-by-case']).default('client'),
  interestResponsibility: z.enum(['client', 'preparer', 'case-by-case']).default('client'),
  ...
  // Communications
  preferredCommunication: z.enum(['phone', 'email', 'in-person', 'portal']).default('email'),
  ...
  // Refund Processing
  refundHandling: z.enum(['direct-deposit', 'check', 'client-choice']).default('client-choice'),
