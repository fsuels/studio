...
export const paymentbondSchema = z.object({
  ...
  projectType: z.enum(['residential', 'commercial', 'industrial', 'public', 'private', 'other']),
  ...
  bondTermination: z.enum(['project-completion', 'specific-date', 'payment-completion', 'other']),
  
  // Coverage Details
  coverageScope: z.array(z.enum([
    'labor-costs',
    'material-costs',
    'equipment-costs',
  ...
  // Covered Parties
  coveredParties: z.array(z.object({
    partyType: z.enum(['subcontractor', 'supplier', 'laborer', 'materialman', 'other']),
    partyName: z.string(),
  ...
  noticeMethod: z.enum(['certified-mail', 'registered-mail', 'personal-service', 'email']).optional(),
  
  // Joint Venture Provisions
  jointVenture: z.boolean().default(false),
  jointVenturePartners: z.array(z.object({
