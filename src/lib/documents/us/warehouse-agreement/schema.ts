...
export const WarehouseAgreementSchema = z.object({
  ...
  clientType: z
    .enum(['manufacturer', 'retailer', 'distributor', 'ecommerce'])
    .optional(),

  // Storage Services
  storageType: z
    .enum(['general', 'refrigerated', 'frozen', 'hazardous', 'pharmaceutical'])
    .default('general'),
  storageSpace: z.string().optional(),
  palletPositions: z.string().optional(),
  rackingType: z
    .enum(['selective', 'drive-in', 'push-back', 'flow-rack'])
    .optional(),
  ...
  inventoryTracking: z
    .enum(['fifo', 'lifo', 'fefo', 'lot-tracking'])
    .default('fifo'),
  ...
  pickingServices: z
    .enum(['single-order', 'batch', 'zone', 'wave'])
