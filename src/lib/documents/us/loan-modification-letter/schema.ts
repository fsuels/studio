...
export const LoanModificationLetterSchema = z.object({
  ...
  requestedStartDate: z.string().min(1).regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format'),
  supportingDocuments: z.array(z.string()).optional(),
  contactPhone: z.string().min(1),
  contactEmail: z.string().email().optional(),
});

export type LoanModificationLetterData = z.infer<typeof LoanModificationLetterSchema>;
