...
export const GeneralLiabilityWaiverSchema = z.object({
  ...
  // Waiver Scope
  waiverScope: z.enum(['negligence-only', 'all-claims', 'specific-claims']).default('all-claims'),
  ...
  medicalExpenseResponsibility: z.enum(['participant', 'organization', 'insurance']).optional(),
  ...
  // Duration and Scope
  waiverDuration: z.enum(['single-event', 'multiple-events', 'ongoing', 'specified-period']).default('single-event'),
