...
export const ArbitrationAgreementSchema = z.object({
  ...
  // Agreement Context
  agreementType: z.enum(['standalone', 'part-of-contract', 'employment', 'consumer', 'commercial']).default('standalone'),
  underlyingAgreement: z.string().optional(),
  relationshipType: z.string().optional(),
  effectiveDate: z.string().min(1, 'Effective date is required'),
  
  // Scope of Arbitration
  disputeScope: z.enum(['all-disputes', 'contract-disputes', 'specific-disputes', 'excluding-certain-disputes']).default('all-disputes'),
  ...
  // Arbitration Rules
  arbitrationRules: z.enum(['aaa-commercial', 'aaa-consumer', 'aaa-employment', 'jams', 'cpradr', 'custom', 'state-rules']).default('aaa-commercial'),
  ...
  arbitratorSelection: z.enum(['mutual-agreement', 'organization-appointment', 'striking-method', 'rotation-list']).default('organization-appointment'),
  ...
  venueSelection: z.enum(['fixed-location', 'convenient-location', 'party-agreement', 'arbitrator-decision']).default('fixed-location'),
  ...
  // Costs and Fees
  costAllocation: z.enum(['each-pays-own', 'loser-pays', 'proportional', 'arbitrator-decision']).default('each-pays-own'),
  arbitratorFees: z.enum(['shared-equally', 'initiating-party', 'losing-party', 'proportional']).default('shared-equally'),
  administrativeFees: z.enum(['shared-equally', 'initiating-party', 'losing-party', 'proportional']).default('initiating-party'),
  attorneyFees: z.enum(['each-pays-own', 'prevailing-party', 'arbitrator-decision']).default('each-pays-own'),
  ...
  awardType: z.enum(['reasoned-award', 'standard-award', 'summary-award']).default('reasoned-award'),
