...
export const LivestockPurchaseAgreementSchema = z.object({
  ...
  // Livestock Details
  animalType: z.enum(['cattle', 'horses', 'pigs', 'sheep', 'goats', 'chickens', 'ducks', 'turkeys', 'alpacas', 'llamas']).default('cattle'),
  animalBreed: z.string().optional(),
  numberOfAnimals: z.string().optional(),
  animalAge: z.string().optional(),
  animalWeight: z.string().optional(),
  animalSex: z.enum(['male', 'female', 'mixed', 'steers', 'heifers']).optional(),
  ...
  weightBasis: z.enum(['live-weight', 'dressed-weight', 'carcass-weight']).default('live-weight'),
  paymentMethod: z.enum(['cash', 'check', 'bank-transfer', 'financing']).default('cash'),
  ...
  deliveryMethod: z.enum(['buyer-pickup', 'seller-delivery', 'third-party']).default('buyer-pickup'),
