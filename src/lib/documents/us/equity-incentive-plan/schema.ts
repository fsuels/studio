...
export const schema = z.object({
  ...
  // Share Authorization
  totalSharesAuthorized: z
    .string()
    .min(1, 'Total shares authorized is required'),
  ...
  employmentRequirement: z
    .string()
    .min(1, 'Employment requirement is required'),

  // Stock Options
  optionExercisePrice: z.string().optional(),
  pricingMethod: z
    .enum(['Fair Market Value', 'Board Determined', 'Formula Based'])
    .optional(),
  ...
  // Termination Events
  terminationProvisions: z
    .string()
    .min(1, 'Termination provisions are required'),
  changeInControlProvisions: z
    .string()
    .min(1, 'Change in control provisions are required'),
  ...
  planTermination: z
    .string()
    .min(1, 'Plan termination provisions are required'),
