...
export const DebtValidationLetterSchema = z.object({
  ...
  dateOfLastPayment: z.string().min(1).regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format').optional(),
  ...
  additionalRequests: z.array(z.enum([
    'Cease Communication',
...
export type DebtValidationLetterData = z.infer<typeof DebtValidationLetterSchema>;
