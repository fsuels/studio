...
export const BookkeepingServicesAgreementSchema = z.object({
  ...
  // Service Scope
  serviceType: z
    .enum(['basic-bookkeeping', 'full-service', 'custom-package'])
    .default('basic-bookkeeping'),
  ...
  // Fee Structure
  feeStructure: z
    .enum(['monthly-retainer', 'hourly', 'per-transaction', 'project-based'])
  ...
  // Payment Terms
  paymentSchedule: z
    .enum(['monthly', 'quarterly', 'upon-completion', 'advance'])
    .default('monthly'),
  ...
  reportDeliveryMethod: z
    .enum(['email', 'portal', 'mail', 'in-person'])
    .default('email'),
  ...
  communicationMethod: z
    .enum(['phone', 'email', 'portal', 'in-person'])
    .default('email'),
  ...
  liabilityScope: z
    .enum(['negligence', 'errors-omissions', 'full-liability'])
    .default('negligence'),
  ...
  modificationRequirements: z
    .enum(['written-only', 'mutual-consent', 'email-acceptable'])
