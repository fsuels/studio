...
export const PersonalTrainingAgreementSchema = z.object({
  ...
  // Package and Pricing
  packageType: z.enum(['single-session', 'package', 'monthly', 'ongoing']).default('package'),
  ...
  // Payment Terms
  paymentSchedule: z.enum(['per-session', 'package-upfront', 'monthly', 'weekly']).default('package-upfront'),
  paymentMethod: z.enum(['cash', 'check', 'credit-card', 'bank-transfer']).default('credit-card'),
