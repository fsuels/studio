...
export const RoyaltyAgreementSchema = z.object({
  ...
  intellectualProperty: z.string().min(1, 'Intellectual property description is required.'),
  royaltyRate: z.string().min(1, 'Royalty rate is required.'),
  royaltyBase: z.enum(['net-sales', 'gross-sales', 'units', 'revenue'], { errorMap: () => ({ message: 'Royalty base is required.' }) }),
  minimumRoyalty: z.string().optional(),
  paymentSchedule: z.enum(['monthly', 'quarterly', 'annually', 'other'], { errorMap: () => ({ message: 'Payment schedule is required.' }) }),
