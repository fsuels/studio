...
export const SubcontractorAgreementSchema = z.object({
  ...
  contractorAddress: z.string().min(1, 'General contractor address is required'),
  ...
  tradeSpecialty: z.enum(['electrical', 'plumbing', 'hvac', 'roofing', 'flooring', 'drywall', 'painting', 'concrete', 'masonry', 'landscaping', 'other']),
  ...
  pricingMethod: z.enum(['lump-sum', 'unit-price', 'cost-plus', 'time-and-material']),
  paymentSchedule: z.enum(['completion', 'progress-payments', 'monthly', 'milestone-based']),
  ...
  changeOrderApproval: z.enum(['written-only', 'verbal-allowed', 'email-allowed']).default('written-only'),
  changeOrderPricing: z.string().optional(),
  
  // Materials and Equipment
  materialProcurement: z.enum(['subcontractor', 'contractor', 'owner', 'shared']).default('subcontractor'),
  ...
  lienWaiverTiming: z.enum(['progress-payments', 'final-payment', 'both']).optional(),
