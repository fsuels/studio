...
export const GamingAgreementSchema = z.object({
  ...
  competitionLevel: z.enum(['amateur', 'semi-professional', 'professional']).default('amateur'),
  platform: z.enum(['pc', 'console', 'mobile', 'multiple']).default('pc'),
  
  // Agreement Type
  agreementType: z.enum(['player-contract', 'tournament', 'sponsorship', 'coaching', 'streaming']).default('player-contract'),
  ...
  // Financial Terms
  compensationType: z.enum(['salary', 'prize-share', 'hourly', 'tournament-winnings', 'sponsorship']).default('salary'),
  ...
  contentOwnership: z.enum(['player', 'organization', 'shared']).default('shared'),
