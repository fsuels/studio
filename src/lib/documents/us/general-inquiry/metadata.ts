...
export const generalInquiry: LegalDocument = {
  ...
  translations: {
    ...
    es: {
      ...
      description: "Para situaciones donde un documento específico no está claro o no se necesita de inmediato.",
  ...
  questions: [
    { id: "inquiryDetails", label: "documents.us.general-inquiry.inquiryDetails.label", type: "textarea", required: true },
    { id: "desiredOutcome", label: "documents.us.general-inquiry.desiredOutcome.label", type: "text" },
    { id: 'state', label: 'documents.us.general-inquiry.state.label', type: 'select', options: usStates.map(s => ({ value: s.value, label: s.label })) }
  ]
};