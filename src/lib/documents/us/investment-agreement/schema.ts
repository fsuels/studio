...
export const InvestmentAgreementSchema = z.object({
  ...
  investorType: z.enum(['individual', 'entity', 'fund', 'trust']).default('individual'),
  accreditedInvestor: z.boolean().default(false),
  investorExperience: z.enum(['novice', 'intermediate', 'experienced', 'professional']).optional(),
  
  // Company/Issuer Information
  companyName: z.string().min(1, 'Company name is required'),
  companyAddress: z.string().optional(),
  companyType: z.enum(['corporation', 'llc', 'partnership', 'trust']).default('corporation'),
  ...
  investmentType: z.enum(['equity', 'debt', 'convertible', 'hybrid']).default('equity'),
  securityType: z.enum(['common-stock', 'preferred-stock', 'note', 'warrant', 'option']).optional(),
  ...
  returnType: z.enum(['fixed', 'variable', 'profit-sharing', 'appreciation']).optional(),
  paymentSchedule: z.enum(['monthly', 'quarterly', 'annually', 'at-exit']).optional(),
  interestRate: z.string().optional(),
  
  // Exit Strategy
  exitStrategy: z.enum(['ipo', 'acquisition', 'buyback', 'dissolution']).optional(),
  ...
  communicationMethod: z.enum(['email', 'mail', 'online-portal']).default('email'),
