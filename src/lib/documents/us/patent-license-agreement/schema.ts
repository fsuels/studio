...
export const PatentLicenseAgreementSchema = z.object({
  ...
  licensorType: z.enum(['individual', 'corporation', 'partnership', 'llc', 'other']),
  
  // Licensee Information
  licenseeName: z.string().min(1, 'Licensee name is required'),
  licenseeAddress: z.string().min(1, 'Licensee address is required'),
  licenseeType: z.enum(['individual', 'corporation', 'partnership', 'llc', 'other']),
  ...
  additionalPatents: z.array(z.object({
    title: z.string(),
  ...
  licenseScope: z.enum(['make', 'use', 'sell', 'make-use', 'make-sell', 'use-sell', 'make-use-sell', 'import', 'full-rights']),
  ...
  // Territory
  territoryScope: z.enum(['worldwide', 'united-states', 'specific-countries', 'specific-regions']),
  ...
  royaltyStructure: z.enum(['running-royalty', 'lump-sum', 'milestone', 'hybrid']).optional(),
  ...
  // Payment Terms
  paymentFrequency: z.enum(['monthly', 'quarterly', 'annually', 'upon-sale']).optional(),
  ...
  maintenanceResponsibility: z.enum(['licensor', 'licensee', 'shared']).default('licensor'),
  prosecutionCosts: z.enum(['licensor-pays', 'licensee-pays', 'shared']).default('licensor-pays'),
  
  // Improvements and Developments
  improvementRights: z.enum(['none', 'licensor-owns', 'licensee-owns', 'shared', 'grant-back']).default('none'),
  ...
  // Patent Enforcement
  enforcementRights: z.enum(['licensor-only', 'licensee-only', 'both', 'cooperation']).default('licensor-only'),
  infringementNotification: z.boolean().default(true),
  cooperationInEnforcement: z.boolean().default(true),
  enforcementCosts: z.enum(['licensor-pays', 'licensee-pays', 'shared', 'recoverer-pays']).default('licensor-pays'),
  
  // Invalidity and Non-infringement
  invalidityChallenge: z.enum(['prohibited', 'allowed', 'terminates-license']).default('allowed'),
  ...
  recordingResponsibility: z.enum(['licensor', 'licensee', 'both']).default('licensee'),
