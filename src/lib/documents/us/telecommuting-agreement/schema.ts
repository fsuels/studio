...
export const schema = z.object({
  ...
  equipmentReturnPolicy: z.string().min(1, 'Equipment return policy is required'),
  
  // Communication Requirements
  communicationTools: z.string().min(1, 'Communication tools are required'),
  meetingAttendanceRequirement: z.string().min(1, 'Meeting attendance requirement is required'),
  responseTimeExpectation: z.string().min(1, 'Response time expectation is required'),
  ...
  dataSecurityRequirements: z.string().min(1, 'Data security requirements are required'),
  ...
  insuranceCoverage: z.string().min(1, 'Insurance coverage details are required'),
