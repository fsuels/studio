...
export const ChildCareContractSchema = z.object({
  ...
  careType: z.enum(['ongoing', 'occasional', 'temporary', 'emergency']).default('occasional'),
  ...
  paymentMethod: z.enum(['cash', 'check', 'bank-transfer', 'app']).default('cash'),
  paymentSchedule: z.enum(['after-each-session', 'weekly', 'bi-weekly', 'monthly']).default('after-each-session'),
  ...
  communicationMethod: z.enum(['text', 'call', 'app', 'written']).default('text'),
