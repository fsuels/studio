...
export const SolarEnergyAgreementSchema = z.object({
  ...
  propertyType: z.enum(['residential', 'commercial', 'industrial', 'agricultural']).default('residential'),
  ...
  inverterType: z.enum(['string', 'power-optimizer', 'microinverter', 'central']).optional(),
  inverterBrand: z.string().optional(),
  
  // Installation Details
  installationType: z.enum(['rooftop', 'ground-mount', 'carport', 'tracker']).default('rooftop'),
  roofType: z.enum(['asphalt-shingle', 'tile', 'metal', 'flat', 'membrane']).optional(),
  roofCondition: z.enum(['excellent', 'good', 'fair', 'needs-repair']).optional(),
  ...
  maintenanceResponsibility: z.enum(['owner', 'installer', 'shared']).default('owner'),
  ...
  meterType: z.enum(['net-meter', 'bidirectional', 'production-meter']).optional(),
  timeOfUseRates: z.boolean().default(false),
  gridTieRequirements: z.string().optional(),
  
  // System Ownership
  systemOwnership: z.enum(['customer', 'third-party', 'shared']).default('customer'),
  ...
  performanceReporting: z.enum(['monthly', 'quarterly', 'annually']).default('monthly'),
