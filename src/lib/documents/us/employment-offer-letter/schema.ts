...
export const schema = z.object({
  ...
  startDate: z.string().min(1, 'Start date is required.').regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format'),
  salary: z.string().min(1, 'Salary is required.'),
  payFrequency: z.enum(['hourly', 'weekly', 'bi-weekly', 'monthly', 'annually'], {
