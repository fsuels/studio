...
export const WaterRightsAgreementSchema = z.object({
  ...
  ownerType: z
    .enum(['individual', 'corporation', 'municipality', 'irrigation-district'])
    .default('individual'),
  ...
  waterSource: z
    .enum(['surface', 'groundwater', 'spring', 'well', 'river', 'lake'])
    .optional(),
  ...
  measurementUnit: z
    .enum(['acre-feet', 'gallons', 'cubic-feet', 'liters'])
    .default('acre-feet'),
  dryYearAdjustments: z.boolean().default(false),

  // Agreement Type
  agreementType: z
    .enum(['sale', 'lease', 'transfer', 'exchange', 'sharing'])
    .default('lease'),
  ...
  paymentSchedule: z
    .enum(['annually', 'semi-annually', 'quarterly', 'monthly', 'lump-sum'])
    .default('annually'),
  ...
  deliveryMethod: z
    .enum(['canal', 'pipeline', 'ditch', 'natural-flow', 'pumping'])
    .optional(),
  deliveryPoint: z.string().optional(),
  infrastructureMaintenance: z
    .enum(['owner', 'user', 'shared'])
    .default('user'),
  ...
  shortageAllocation: z
    .enum(['proportional', 'priority', 'negotiated'])
    .default('priority'),
  ...
  // Dispute Resolution
  disputeResolution: z
    .enum(['negotiation', 'mediation', 'arbitration', 'water-court'])
    .default('mediation'),
