...
export const AviationCharterAgreementSchema = z.object({
  ...
  // Charter Type
  charterType: z.enum(['one-way', 'round-trip', 'multi-leg', 'time-based']).default('round-trip'),
  flightType: z.enum(['domestic', 'international', 'cross-border']).default('domestic'),
  charterPurpose: z.enum(['business', 'leisure', 'emergency', 'medical', 'cargo']).default('business'),
  
  // Financial Terms
  charterFee: z.string().optional(),
  hourlyRate: z.string().optional(),
  fuelSurcharge: z.string().optional(),
  landingFees: z.enum(['included', 'additional', 'charterer-pays']).default('additional'),
  overnightFees: z.string().optional(),
  cancellationFee: z.string().optional(),
  
  // Payment Terms
  paymentMethod: z.enum(['cash', 'check', 'wire-transfer', 'credit-card']).default('wire-transfer'),
  paymentSchedule: z.enum(['advance', 'upon-completion', 'split']).default('advance'),
  ...
  crewAccommodation: z.enum(['operator', 'charterer', 'shared']).default('charterer'),
  ...
  loadingUnloading: z.enum(['operator', 'charterer', 'shared']).default('charterer'),
  ...
  smokingPolicy: z.enum(['prohibited', 'designated-areas', 'permitted']).default('prohibited'),
