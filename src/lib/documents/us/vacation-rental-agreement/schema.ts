...
export const VacationRentalAgreementSchema = z.object({
  ...
  propertyType: z.enum(['house', 'apartment', 'condo', 'cabin', 'villa', 'other']).default('house'),
  ...
  // Payment Terms
  paymentSchedule: z.enum(['full-upfront', 'deposit-then-balance', 'installments']).default('deposit-then-balance'),
  ...
  utilitiesResponsibility: z.enum(['owner', 'guest', 'shared']).default('owner'),
  maintenanceReporting: z.boolean().default(true),
  
  // Cancellation Policy
  cancellationPolicy: z.enum(['strict', 'moderate', 'flexible', 'super-strict']).default('moderate'),
