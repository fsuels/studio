...
export const SocialMediaManagementAgreementSchema = z.object({
  ...
  // Service Scope
  serviceType: z.enum(['management-only', 'content-creation', 'full-service', 'consulting']).default('full-service'),
  ...
  postingFrequency: z.enum(['daily', 'multiple-daily', 'weekly', 'custom']).default('daily'),
  ...
  approvalProcess: z.enum(['pre-approval', 'post-approval', 'no-approval']).default('pre-approval'),
  ...
  // Fee Structure
  pricingModel: z.enum(['monthly-retainer', 'hourly', 'per-post', 'performance-based']).default('monthly-retainer'),
  ...
  // Payment Terms
  paymentSchedule: z.enum(['monthly', 'quarterly', 'advance']).default('monthly'),
  ...
  communicationMethod: z.enum(['email', 'phone', 'slack', 'project-management']).default('email'),
  responseTimeframe: z.string().optional(),
  meetingFrequency: z.enum(['weekly', 'bi-weekly', 'monthly', 'quarterly']).default('monthly'),
