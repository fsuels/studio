...
export const ceaseDesistLetterSchema = z.object({
  ...
  violation: z.object({
    ...
    specificActivities: z.array(z.string()).min(1, 'Please list at least one specific activity'),
    dateFirstOccurred: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format').optional(),
    ongoingViolation: z.boolean(),
    frequency: z.enum(['daily', 'weekly', 'monthly', 'sporadic', 'one_time']).optional()
  }),
  ...
  legalBasis: z.object({
    ...
    intellectualPropertyRights: z.array(z.object({
  ...
  // Demands
  demands: z.object({
    immediateActions: z.array(z.string()).min(1, 'Please specify at least one immediate action required'),
  ...
  // Timeline
  timeline: z.object({
    complianceDeadline: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format'),
  ...
  delivery: z.object({
    deliveryMethod: z.enum(['certified_mail', 'personal_service', 'email', 'multiple_methods']),
    trackingNumbers: z.array(z.string()).optional(),
    deliveryDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format').optional(),
