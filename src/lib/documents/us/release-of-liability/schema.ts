...
export const ReleaseOfLiabilitySchema = z.object({
  ...
  releaseeType: z.enum(['individual', 'business', 'organization']).default('business'),
  ...
  claimsReleased: z.array(z.enum(['personal-injury', 'property-damage', 'emotional-distress', 'financial-loss', 'all-claims'])).default(['all-claims']),
  futureClaimsIncluded: z.boolean().default(true),
  knownClaimsOnly: z.boolean().default(false),
  
  // Consideration
  considerationAmount: z.string().optional(),
  considerationType: z.enum(['monetary', 'services', 'participation', 'other']).default('participation'),
