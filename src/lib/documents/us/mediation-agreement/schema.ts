...
export const MediationAgreementSchema = z.object({
  ...
  party1Type: z.enum(['individual', 'business', 'organization']).default('individual'),
  party2Name: z.string().min(1, 'Party 2 name is required'),
  party2Address: z.string().min(1, 'Party 2 address is required'),
  party2Type: z.enum(['individual', 'business', 'organization']).default('individual'),
  additionalParties: z.array(z.string()).default([]),
  
  // Dispute Information
  disputeDescription: z.string().min(1, 'Dispute description is required'),
  disputeType: z.enum(['contract', 'employment', 'family', 'commercial', 'personal-injury', 'property', 'other']).default('contract'),
  disputeValue: z.string().optional(),
  disputeDate: z.string().optional(),
  relationshipContext: z.string().optional(),
  
  // Mediation Process
  mediationType: z.enum(['voluntary', 'court-ordered', 'contractual-requirement', 'pre-litigation']).default('voluntary'),
  mediationGoal: z.enum(['settlement', 'communication', 'relationship-repair', 'understanding']).default('settlement'),
  mediationFormat: z.enum(['in-person', 'virtual', 'hybrid', 'shuttle']).default('in-person'),
  sessionStructure: z.enum(['single-session', 'multiple-sessions', 'open-ended']).default('single-session'),
  
  // Mediator Information
  mediatorSelection: z.enum(['mutual-agreement', 'organization-appointment', 'rotation-list', 'named-mediator']).default('mutual-agreement'),
  ...
  locationSelection: z.enum(['fixed-location', 'neutral-location', 'rotating-location', 'virtual-only']).default('neutral-location'),
  schedulingMethod: z.enum(['mutual-agreement', 'mediator-availability', 'organization-scheduling']).default('mutual-agreement'),
  timeframe: z.string().optional(),
  sessionDuration: z.string().optional(),
  
  // Participation Requirements
  principalAttendance: z.boolean().default(true),
  attorneyAttendance: z.enum(['required', 'optional', 'prohibited', 'advisory-only']).default('optional'),
  ...
  costSharing: z.enum(['equal-split', 'proportional', 'initiating-party', 'ability-to-pay', 'other']).default('equal-split'),
  mediatorFees: z.enum(['hourly', 'daily', 'flat-fee', 'sliding-scale']).default('hourly'),
  ...
  draftingResponsibility: z.enum(['mediator', 'parties', 'attorneys', 'mutual']).default('parties'),
