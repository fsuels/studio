...
export const incidentReportSchema = z.object({
  ...
  basicInfo: z.object({
    ...
    dateOfIncident: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format'),
    timeOfIncident: z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Time must be in HH:MM format'),
    dateReported: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format'),
    timeReported: z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/, 'Time must be in HH:MM format'),
  ...
  incidentType: z.object({
    category: z.enum(['injury', 'illness', 'near_miss', 'property_damage', 'security', 'environmental', 'other']),
  ...
  injuredPersons: z.array(z.object({
    ...
    contactInfo: z.object({
      ...
      emergencyPhone: z.string().regex(/^\(\d{3}\) \d{3}-\d{4}$/, 'Phone must be in format (XXX) XXX-XXXX').optional()
    }).optional()
  })).optional(),
  ...
  injuryDetails: z.object({
    ...
    medicalTreatment: z.object({
      ...
      returnToWork: z.enum(['same_day', 'next_day', 'restricted_duty', 'time_off', 'unknown']).optional()
    }).optional()
  }).optional(),

  // Incident Description
  description: z.object({
    whatHappened: z.string().min(50, 'Please provide a detailed description (at least 50 characters)'),
    whatWasDoing: z.string().min(20, 'Please describe what the person was doing'),
    equipmentInvolved: z.array(z.string()).optional(),
    materialsInvolved: z.array(z.string()).optional(),
    weatherConditions: z.string().optional(),
    lightingConditions: z.enum(['adequate', 'poor', 'very_poor', 'dark']).optional()
  }),
  ...
  supervisorReview: z.object({
    ...
    reviewDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Date must be in YYYY-MM-DD format').optional(),
