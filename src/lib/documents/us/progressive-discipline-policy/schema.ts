...
export const schema = z.object({
  ...
  seriousOffenseConsequences: z.string().min(1, 'Serious offense consequences are required'),
  
  // Documentation Requirements
  documentationRequirements: z.string().min(1, 'Documentation requirements are required'),
  recordRetentionPeriod: z.string().min(1, 'Record retention period is required'),
  ...
  hrConsultationRequirement: z.string().min(1, 'HR consultation requirement is required'),
  
  // Special Circumstances
  attendanceIssuesPolicy: z.string().min(1, 'Attendance issues policy is required'),
  performanceIssuesPolicy: z.string().min(1, 'Performance issues policy is required'),
