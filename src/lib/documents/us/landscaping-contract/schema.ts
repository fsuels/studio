...
export const LandscapingContractSchema = z.object({
  ...
  serviceType: z.enum(['design-installation', 'installation-only', 'maintenance', 'one-time-service', 'seasonal-maintenance']),
  ...
  plantSelection: z.enum(['contractor-selects', 'owner-selects', 'joint-selection']),
  ...
  priceType: z.enum(['fixed-price', 'hourly-rate', 'per-square-foot', 'per-plant']),
  hourlyRate: z.string().optional(),
  additionalServices: z.string().optional(),
  
  // Payment Terms
  paymentSchedule: z.enum(['upfront', 'progress-payments', 'monthly', 'seasonal', 'completion']),
  ...
  liabilityInsurance: z.string().min(1, 'Liability insurance amount is required'),
  ...
  waterSource: z.enum(['owner-provides', 'contractor-provides', 'municipal']).optional(),
  waterCosts: z.enum(['owner-pays', 'contractor-pays', 'included-in-price']).optional(),
  ...
  changeOrderApproval: z.enum(['written-only', 'owner-approval', 'contractor-discretion']),
