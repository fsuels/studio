...
export const BarAgreementSchema = z.object({
  ...
  primaryPartyRole: z.enum(['owner', 'partner', 'manager', 'investor']).default('owner'),
  ...
  secondaryPartyRole: z.enum(['owner', 'partner', 'manager', 'investor']).default('partner'),
  
  // Bar Information
  barName: z.string().min(1, 'Bar name is required'),
  barAddress: z.string().optional(),
  barType: z.enum(['sports-bar', 'cocktail-bar', 'dive-bar', 'wine-bar', 'nightclub', 'pub', 'other']).default('sports-bar'),
  ...
  dailyOperations: z.enum(['joint', 'primary-party', 'secondary-party', 'manager']).default('joint'),
  staffManagement: z.enum(['joint', 'primary-party', 'secondary-party', 'manager']).default('joint'),
  inventoryManagement: z.enum(['joint', 'primary-party', 'secondary-party', 'manager']).default('joint'),
  financialManagement: z.enum(['joint', 'primary-party', 'secondary-party']).default('joint'),
  
  // Beverage Program
  drinkMenuControl: z.enum(['joint', 'primary-party', 'secondary-party', 'bartender']).default('joint'),
  pricingControl: z.enum(['joint', 'primary-party', 'secondary-party']).default('joint'),
  supplierSelection: z.enum(['joint', 'primary-party', 'secondary-party']).default('joint'),
  specialtyDrinks: z.string().optional(),
  
  // Food Service
  foodService: z.boolean().default(false),
  kitchenFacilities: z.boolean().default(false),
  foodMenuControl: z.enum(['joint', 'primary-party', 'secondary-party', 'chef']).default('joint'),
  ...
  socialMediaManagement: z.enum(['joint', 'primary-party', 'secondary-party']).default('joint'),
  ...
  smokingPolicy: z.enum(['no-smoking', 'smoking-allowed', 'designated-area']).default('no-smoking'),
