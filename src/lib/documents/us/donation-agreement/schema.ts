...
export const DonationAgreementSchema = z.object({
  ...
  donorType: z
    .enum(['individual', 'corporation', 'foundation', 'trust', 'estate'])
    .default('individual'),
  ...
  paymentMethod: z
    .enum(['check', 'wire-transfer', 'credit-card', 'cash', 'other'])
    .default('check'),
  paymentSchedule: z
    .enum(['one-time', 'installments', 'monthly', 'quarterly', 'annual'])
    .default('one-time'),
  ...
  reportingFrequency: z
    .enum(['quarterly', 'semi-annual', 'annual', 'none'])
    .default('annual'),
