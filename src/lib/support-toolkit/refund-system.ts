...
export interface RefundRequest {
  ...
  userId: string;
  amount: number;
  reason: 'customer_request' | 'technical_issue' | 'billing_error' | 'fraud_protection' | 'goodwill' | 'chargeback';
...
export interface RefundAuditEvent {
  id: string;
  timestamp: number;
  action: 'created' | 'approved' | 'rejected' | 'processed' | 'failed' | 'cancelled' | 'escalated';
  actor: {
...
export class RefundSystem {
  ...
  static async createRefundRequest(
    ...
    try {
      ...
      if (amount > orderData.payment.amount) {
        throw new RefundSystemError('Refund amount exceeds order total', 'INVALID_AMOUNT');
      }
      ...
      if (totalRefunded + amount > orderData.payment.amount) {
        throw new RefundSystemError('Total refunds would exceed order amount', 'EXCEEDS_ORDER_TOTAL');
      }
      ...
      const autoApprovalResult = await this.checkAutoApproval(refundRequest, orderData);
      if (autoApprovalResult.approved) {
        refundRequest.status = 'approved';
        refundRequest.metadata.autoApprovalRules = autoApprovalResult.appliedRules;
        ...
        refundRequest.auditTrail.push({
          ...
          timestamp: Date.now(),
          action: 'approved',
          actor: { type: 'system', id: 'auto-approval', name: 'Auto-Approval System' },
      ...
      throw new RefundSystemError('Failed to create refund request', 'CREATION_FAILED', { error: error.message });
    }
  }
  ...
  static async approveRefund(
    ...
    try {
      ...
      if (!refundDoc.exists()) {
        throw new RefundSystemError('Refund request not found', 'REFUND_NOT_FOUND');
      }

      const refund = refundDoc.data() as RefundRequest;
      
      if (refund.status !== 'pending') {
        throw new RefundSystemError('Refund is not in pending status', 'INVALID_STATUS');
      }
      ...
      const auditEvent: RefundAuditEvent = {
        ...
        timestamp: Date.now(),
        action: 'approved',
        actor: { type: 'support_agent', id: approvedBy.agentId, name: approvedBy.name },
      ...
      if (error instanceof RefundSystemError) throw error;
      throw new RefundSystemError('Failed to approve refund', 'APPROVAL_FAILED', { error: error.message });
    }
  }
    ...
    try {
      ...
      try {
        if (refund.metadata.refundMethod === 'store_credit') {
          ...
          await this.addStoreCredit(refund.userId, refund.amount, 'refund', refund.orderId, refundId);
          
          auditEvent = {
            id: `audit_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
            timestamp: Date.now(),
            action: 'processed',
            actor: { type: 'system', id: 'store-credit', name: 'Store Credit System' },
          ...
          auditEvent = {
            ...
            timestamp: Date.now(),
            action: 'processed',
            actor: { type: 'system', id: 'stripe', name: 'Stripe Payment System' },
            details: `Stripe refund processed: ${stripeRefund.id}`,
            metadata: { stripeRefundId: stripeRefund.id, stripeStatus: stripeRefund.status },
            systemData: { requestId: refundId }
          };
        ...
        const failureAuditEvent: RefundAuditEvent = {
          ...
          timestamp: Date.now(),
          action: 'failed',
          actor: { type: 'system', id: 'refund-processor', name: 'Refund Processing System' },
        ...
        throw new RefundSystemError('Refund processing failed', 'PROCESSING_FAILED', { error: error.message });
      }

    } catch (error) {
      if (error instanceof RefundSystemError) throw error;
      throw new RefundSystemError('Failed to process refund', 'PROCESSING_ERROR', { error: error.message });
    }
  }
  ...
  static async getStoreCreditBalance(userId: string): Promise<number> {
    ...
    const transactions = snapshot.docs.map(doc => doc.data() as StoreCreditTransaction);
  ...
  private static async checkAutoApproval(
    ...
    if (refund.reason === 'technical_issue' && 
    ...
    if (orderData.customer.customerSegment === 'premium' && 
