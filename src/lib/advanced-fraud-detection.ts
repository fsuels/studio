...
class AdvancedFraudDetectionEngine {
  ...
  async assessFraudRisk(
    ...
    try {
      ...
      const velocityCheck = await this.performVelocityChecks(
        orderData,
      ...
      const chargebackScore = await this.predictChargebackRisk(
        orderData,
        velocityCheck,
        deviceFingerprint,
      );
  ...
  private async performVelocityChecks(
    ...
    // Email velocity
    const emailVelocity = await this.getEmailVelocity(
    ...
    // IP velocity
    const ipVelocity = await this.getIPVelocity(
      orderData.customer.ipLocation.ip,
      last24h,
      lastWeek,
    );

    // Device velocity
    const deviceVelocity = await this.getDeviceVelocity(
      deviceId,
      last24h,
      lastWeek,
    );
    ...
    // Card velocity
    const cardVelocity = await this.getCardVelocity(
  ...
  private async predictChargebackRisk(
    ...
    const factors: ChargebackRiskFactors = {
      ...
      chargebackHistory: await this.getChargebackHistory(
        orderData.customer.email,
      ),
      ...
      avgOrderValue:
        orderData.customer.totalSpent /
        Math.max(orderData.customer.totalOrders, 1),
      ...
      highRiskCountry: this.isHighRiskCountry(
  ...
  private calculateDeviceScore(device: DeviceFingerprint): number {
    ...
    if (recentLocations.length >= 3) {
      const timeSpan =
        ...
        ).getTime() - new Date(recentLocations[0].timestamp).getTime();
  ...
  private calculateGeographicScore(orderData: any): number {
    ...
    if (ipCountry !== billingCountry) {
      ...
      if (
        this.isHighRiskCountry(ipCountry) ||
        this.isHighRiskCountry(billingCountry)
      ) {
        score += 200;
      }
    }
  ...
  private calculateChargebackProbability(
    factors: ChargebackRiskFactors,
  ): number {
    ...
    if (factors.chargebackHistory > 0)
      probability += factors.chargebackHistory * 0.15;
  ...
  private async getEmailVelocity(
    email: string,
    since24h: Date,
    sinceWeek: Date,
  ) {
  ...
  private async getDeviceVelocity(
    deviceId: string,
    since24h: Date,
    sinceWeek: Date,
  ) {
  ...
  private identifyRiskFactors(data: any): FraudRiskAssessment['riskFactors'] {
    ...
    if (data.velocityCheck.email.orderCount24h > 2) {
      factors.push({
        ...
        severity:
          data.velocityCheck.email.orderCount24h > 5 ? 'critical' : 'high',
    ...
    const countries = new Set(
      data.deviceFingerprint.locations.map((l: any) => l.country),
    );
    if (countries.size > 2) {
      factors.push({
        category: 'geographic',
  ...
  private async getCardType(
    cardLast4: string,
  ): Promise<'credit' | 'debit' | 'prepaid'> {
    return ['credit', 'debit', 'prepaid'][Math.floor(Math.random() * 3)] as any;
  }

  private async getBINRiskLevel(
    cardLast4: string,
  ...
  private createFailsafeAssessment(
...
export function generateDeviceFingerprint(
  ...
  return new Promise((resolve) => {
    ...
    // Plugin detection
    deviceData.plugins = Array.from(navigator.plugins || [])
      .map((p) => p.name)
      .slice(0, 10);
...
export function createFraudAssessmentSummary(
  assessment: FraudRiskAssessment,
): string {
