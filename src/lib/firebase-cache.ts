...
import { doc, getDoc, setDoc, query, where, collection, getDocs, DocumentSnapshot } from 'firebase/firestore'
...
export class FirebaseCache {
  ...
  // Set cached data
  async set<T>(id: string, data: T, params?: Record<string, any>): Promise<void> {
    const key = this.generateKey(id, params)
  ...
  async batchGet<T>(ids: string[], params?: Record<string, any>): Promise<Map<string, T>> {
    ...
    try {
      ...
      querySnapshot.forEach(doc => {
        ...
        if (!isExpired) {
          ...
          const originalId = uncachedIds.find(id => 
            this.generateKey(id, params) === data.key
          )
  ...
  async clear(id: string, params?: Record<string, any>): Promise<void> {
    ...
    try {
      ...
      const docRef = doc(db, this.config.collection, key)
      await setDoc(docRef, { deleted: true, deletedAt: Date.now() }, { merge: true })
    } catch (error) {
...
export const legalResearchCache = new FirebaseCache(CACHE_CONFIGS.LEGAL_RESEARCH)
export const templateCache = new FirebaseCache(CACHE_CONFIGS.TEMPLATES)
export const userDocumentCache = new FirebaseCache(CACHE_CONFIGS.USER_DOCUMENTS)
export const seoContentCache = new FirebaseCache(CACHE_CONFIGS.SEO_CONTENT)
export const stateRequirementsCache = new FirebaseCache(CACHE_CONFIGS.STATE_REQUIREMENTS)
...
export async function getCachedStateRequirements(state: string, documentType: string) {
  return stateRequirementsCache.getOrCompute(
    ...
    async () => {
      ...
      return {
        state,
        documentType,
        requirements: [`Requirement 1 for ${state}`, `Requirement 2 for ${state}`],
...
export async function cleanupExpiredCache() {
  try {
    const collections = Object.values(CACHE_CONFIGS).map(config => config.collection)
