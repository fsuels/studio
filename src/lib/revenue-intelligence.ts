...
export class RevenueIntelligenceEngine {
  ...
  calculateRevenueMetrics(): RevenueMetrics {
    ...
    const mrrGrowth = previousMonth.recurring > 0 
  ...
  calculateCustomerLTV(): CustomerLifetimeValue[] {
    ...
    for (const [email, customer] of this.customers) {
      const customerOrders = this.orders.filter(o => o.customer.email === email);
      
      if (customerOrders.length === 0) continue;
      
      const totalRevenue = customerOrders.reduce((sum, order) => sum + order.payment.amount, 0);
      const avgOrderValue = totalRevenue / customerOrders.length;
      
      // Calculate purchase frequency (orders per month)
      const firstOrder = new Date(customerOrders[0].createdAt);
      const lastOrder = new Date(customerOrders[customerOrders.length - 1].createdAt);
      const monthsActive = Math.max(1, (lastOrder.getTime() - firstOrder.getTime()) / (30 * 24 * 60 * 60 * 1000));
      ...
      const daysSinceLastOrder = (Date.now() - lastOrder.getTime()) / (24 * 60 * 60 * 1000);
      const avgDaysBetweenOrders = monthsActive * 30 / customerOrders.length;
      const customerLifespan = Math.max(6, Math.min(36, avgDaysBetweenOrders * 12 / 30)); // 6-36 months
  ...
  calculateCohortAnalysis(): CohortAnalysis[] {
    ...
    for (const [email, customer] of this.customers) {
      ...
      const cohortMonth = new Date(firstOrder.createdAt).toISOString().slice(0, 7); // YYYY-MM
      
      if (!cohorts.has(cohortMonth)) {
        cohorts.set(cohortMonth, {
          cohortMonth,
          cohortSize: 0,
          retentionRates: { month1: 0, month3: 0, month6: 0, month12: 0, month24: 0 },
          revenueRetention: { month1: 0, month3: 0, month6: 0, month12: 0, month24: 0 },
          averageRevenue: 0,
          totalRevenue: 0,
          churnedCustomers: 0,
          expandedCustomers: 0
      ...
      const retention = this.calculateCustomerRetention(customerOrders, new Date(firstOrder.createdAt));
      const revenue = customerOrders.reduce((sum, order) => sum + order.payment.amount, 0);
    ...
    for (const cohort of cohorts.values()) {
      if (cohort.cohortSize > 0) {
        cohort.retentionRates.month1 = (cohort.retentionRates.month1 / cohort.cohortSize) * 100;
        cohort.retentionRates.month3 = (cohort.retentionRates.month3 / cohort.cohortSize) * 100;
        cohort.retentionRates.month6 = (cohort.retentionRates.month6 / cohort.cohortSize) * 100;
        cohort.retentionRates.month12 = (cohort.retentionRates.month12 / cohort.cohortSize) * 100;
        cohort.retentionRates.month24 = (cohort.retentionRates.month24 / cohort.cohortSize) * 100;
    ...
    return Array.from(cohorts.values()).sort((a, b) => a.cohortMonth.localeCompare(b.cohortMonth));
  }
  ...
  predictChurn(): ChurnPrediction[] {
    ...
    for (const [email, customer] of this.customers) {
      ...
      const daysSinceLastOrder = (Date.now() - new Date(lastOrder.createdAt).getTime()) / (24 * 60 * 60 * 1000);
      ...
      const recommendedActions = this.generateRetentionRecommendations(churnRisk, factors);
  ...
  identifyRevenueLeakage(): RevenueLeakage[] {
    ...
    const churnRevenue = churnedCustomers.reduce((sum, customer) => sum + customer.totalSpent, 0);
    ...
    const refundRevenue = refunds.reduce((sum, order) => sum + order.payment.amount, 0);
  ...
  private getCurrentMonthRevenue() {
    ...
    const orders = this.orders.filter(o => o.createdAt.startsWith(currentMonth));
    const revenue = orders.reduce((sum, order) => sum + order.payment.amount, 0);
  ...
  private predictCustomerLTV(customer: CustomerInfo, orders: Order[]): number {
    ...
    const baseValue = orders.reduce((sum, order) => sum + order.payment.amount, 0);
  ...
  private segmentCustomer(ltv: number, riskScore: number): 'high_value' | 'medium_value' | 'low_value' | 'at_risk' {
  ...
  private calculateCustomerRetention(orders: Order[], firstOrderDate: Date) {
    ...
    milestones.forEach(months => {
      ...
      const hasOrderAfterMilestone = orders.some(order => 
        new Date(order.createdAt) >= milestone
      );
  ...
  private extractChurnFeatures(customer: CustomerInfo, orders: Order[]) {
    ...
    return {
      ...
      avgOrderValue: orders.reduce((sum, o) => sum + o.payment.amount, 0) / orders.length,
  ...
  private generateRetentionRecommendations(risk: string, factors: any[]): string[] {
  ...
  private calculateAvgDaysBetweenOrders(orders: Order[]): number {
    ...
    const totalDays = (new Date(orders[orders.length - 1].createdAt).getTime() - 
                     new Date(orders[0].createdAt).getTime()) / (24 * 60 * 60 * 1000);
  ...
  private calculateOrderFrequencyDecline(orders: Order[]): number {
    ...
    const firstPeriodDays = (new Date(firstHalf[firstHalf.length - 1].createdAt).getTime() - 
                           new Date(firstHalf[0].createdAt).getTime()) / (24 * 60 * 60 * 1000);
    const secondPeriodDays = (new Date(secondHalf[secondHalf.length - 1].createdAt).getTime() - 
                            new Date(secondHalf[0].createdAt).getTime()) / (24 * 60 * 60 * 1000);
