...
import { getLocationFromIP, getCachedLocationFromIP, type LocationResult } from './geolocation';
import { 
  stateRegulations, 
  getStateRisk, 
  canPurchaseInState, 
...
export async function checkUserCompliance(options: ComplianceCheckOptions): Promise<ComplianceResult> {
  try {
    // Get user location
    ...
    const purchaseAllowed = options.bypassGeoblock || canPurchaseInState(stateCode);
    const action: ComplianceEvent['action'] = purchaseAllowed ? 'checkout_allowed' : 'checkout_blocked';
...
function getBlockingFactors(regulation: StateRegulation): string[] {
  ...
  if (regulation.risk === 'red') {
    ...
    if (regulation.specialRequirements?.includes('Named in-state attorney review required')) {
      factors.push('Attorney review mandatory');
    }
    
    if (regulation.specialRequirements?.includes('State registration required')) {
  ...
  switch (regulation.risk) {
    case 'green':
      if (regulation.requiresRegistration) {
        recommendations.push('Consider regulatory sandbox registration for enhanced compliance');
      }
      recommendations.push('Enhanced legal protections available in this state');
      break;
      
    case 'amber':
      recommendations.push('Review all disclaimers carefully');
      recommendations.push('Save your session - you can complete purchase anytime');
      break;
      
    case 'red':
      recommendations.push('Join our waitlist for priority access when available');
      recommendations.push('Consider consulting with a local attorney');
      if (regulation.specialRequirements?.includes('Attorney involvement required')) {
        recommendations.push('We are working on attorney partnerships in your state');
      }
      break;
  }
...
export function getDisclaimerText(level: 'basic' | 'enhanced' | 'strict', stateCode?: string): string {
  const baseDisclaimer = "123LegalDoc provides do-it-yourself legal document templates. We are not a law firm and do not provide legal advice.";
...
export function getStateSpecificTerms(stateCode: string): string[] {
  ...
  if (regulation.risk === 'red') {
    terms.push('This service is not currently available in your state due to regulatory restrictions.');
    terms.push('By joining our waitlist, you agree to be notified when service becomes available.');
  }
  
  if (regulation.requiresRegistration) {
    terms.push('This service operates under regulatory oversight in your state.');
  }
...
export class ComplianceMonitor {
  ...
  getMetrics(since?: Date): ComplianceMetrics {
    ...
    const metrics: ComplianceMetrics = {
      ...
      allowedPurchases: recentEvents.filter(e => e.action === 'checkout_allowed').length,
      blockedPurchases: recentEvents.filter(e => e.action === 'checkout_blocked').length,
      byRiskLevel: { green: 0, amber: 0, red: 0 },
      byState: {},
      geolocationFailures: recentEvents.filter(e => e.userState === 'UNKNOWN').length,
      lastUpdated: new Date().toISOString()
    };
    
    recentEvents.forEach(event => {
      metrics.byRiskLevel[event.riskLevel]++;
      metrics.byState[event.userState] = (metrics.byState[event.userState] || 0) + 1;
    });
