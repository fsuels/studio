...
import { CollaborationClient, CollaborationUser, PresenceInfo, Comment, Mention } from '@/lib/collaboration/client';
...
export interface UseCollaborationReturn {
  ...
  disconnect: () => void;
  addComment: (content: string, position?: { line: number; column: number }) => void;
  resolveComment: (commentId: string) => void;
  createMention: (userId: string, content: string, position?: { line: number; column: number }) => void;
...
export function useCollaboration({
  ...
  }, [enableRealtime, user, documentId, currentUser, isConnecting, isConnected, getAuthToken]);
  ...
  // Event handlers
  const addEventListener = React.useCallback((event: string, callback: Function): (() => void) => {
  ...
  const onUserJoined = React.useCallback((callback: (user: CollaborationUser) => void) => {
    return addEventListener('user_joined', callback);
  }, [addEventListener]);

  const onUserLeft = React.useCallback((callback: (userId: string) => void) => {
    return addEventListener('user_left', callback);
  }, [addEventListener]);

  const onCommentAdded = React.useCallback((callback: (comment: Comment) => void) => {
    return addEventListener('comment_added', callback);
  }, [addEventListener]);

  const onMentionReceived = React.useCallback((callback: (mention: Mention) => void) => {
    return addEventListener('mention_received', callback);
  }, [addEventListener]);

  const onDocumentChanged = React.useCallback((callback: (changes: any) => void) => {
