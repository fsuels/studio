  ...
  try {
    ...
    switch (metric) {
      ...
      case 'document_completion_rate':
        response = await getDocumentCompletionRate(period, granularity, segment);
        break;
      ...
      case 'document_types_popularity':
        response = await getDocumentTypesPopularity(period, granularity, segment);
        break;
      case 'user_retention':
      ...
      case 'fraud_prevention_savings':
        response = await getFraudPreventionSavings(period, granularity, segment);
        break;
      default:
...
async function getChurnRate(period: string, granularity: string, segment?: string): Promise<MetricResponse> {
  ...
  return {
    ...
    data: {
      ...
      change: change * 100,
      changeType: change > 0.001 ? 'increase' : change < -0.001 ? 'decrease' : 'stable',
      chartData,
      format: 'percentage',
      unit: '',
    },
...
async function getConversionRate(period: string, granularity: string, segment?: string): Promise<MetricResponse> {
...
async function getRevenue(period: string, granularity: string, segment?: string): Promise<MetricResponse> {
  const daysCount = parsePeriodToDays(period);
  const intervals = generateTimeIntervals(daysCount, granularity);
  ...
  return {
    ...
    data: {
      ...
      previous,
      change,
      changeType: change > 100 ? 'increase' : change < -100 ? 'decrease' : 'stable',
      chartData,
      format: 'currency',
      unit: '',
    },
...
async function getDocumentCompletionRate(period: string, granularity: string, segment?: string): Promise<MetricResponse> {
  const daysCount = parsePeriodToDays(period);
  const intervals = generateTimeIntervals(daysCount, granularity);
...
async function getUserAcquisition(period: string, granularity: string, segment?: string): Promise<MetricResponse> {
  const daysCount = parsePeriodToDays(period);
  const intervals = generateTimeIntervals(daysCount, granularity);
  
  const dailyAvg = 45; // 45 new users per day
  const current = daysCount * dailyAvg + Math.random() * dailyAvg * daysCount * 0.3;
...
async function getAvgOrderValue(period: string, granularity: string, segment?: string): Promise<MetricResponse> {
  const daysCount = parsePeriodToDays(period);
  const intervals = generateTimeIntervals(daysCount, granularity);
  
  const current = 67.50 + Math.random() * 20; // $67-87 AOV
...
async function getCustomerLifetimeValue(period: string, granularity: string, segment?: string): Promise<MetricResponse> {
...
async function getDocumentTypesPopularity(period: string, granularity: string, segment?: string): Promise<MetricResponse> {
  const chartData = [
    { name: 'Bill of Sale', value: 2845, category: 'document_type', color: '#3b82f6' },
    { name: 'Promissory Note', value: 1967, category: 'document_type', color: '#ef4444' },
    { name: 'Lease Agreement', value: 1543, category: 'document_type', color: '#10b981' },
    { name: 'Employment Contract', value: 1234, category: 'document_type', color: '#f59e0b' },
    { name: 'NDA', value: 987, category: 'document_type', color: '#8b5cf6' },
    { name: 'Power of Attorney', value: 756, category: 'document_type', color: '#f97316' },
  ];
...
async function getUserRetention(period: string, granularity: string, segment?: string): Promise<MetricResponse> {
...
async function getPaymentSuccessRate(period: string, granularity: string, segment?: string): Promise<MetricResponse> {
  const daysCount = parsePeriodToDays(period);
  const intervals = generateTimeIntervals(daysCount, granularity);
...
async function getSessionDuration(period: string, granularity: string, segment?: string): Promise<MetricResponse> {
  const daysCount = parsePeriodToDays(period);
  const intervals = generateTimeIntervals(daysCount, granularity);
...
async function getFraudPreventionSavings(period: string, granularity: string, segment?: string): Promise<MetricResponse> {
  const daysCount = parsePeriodToDays(period);
  const intervals = generateTimeIntervals(daysCount, granularity);
  ...
  return {
    ...
    data: {
      ...
      previous,
      change,
      changeType: change > 50 ? 'increase' : change < -50 ? 'decrease' : 'stable',
      chartData,
      format: 'currency',
      unit: 'saved',
    },
