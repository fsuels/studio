...
function getTopCompanies(requests: any[]): { companyName: string; count: number }[] {
  const companyCounts = requests.reduce((acc, request) => {
...
function getRequestsByIntendedUse(requests: any[]): { use: string; count: number }[] {
  const useCounts = requests.reduce((acc, request) => {
