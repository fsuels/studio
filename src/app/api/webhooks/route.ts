...
export async function POST(request: NextRequest) {
  try {
    ...
    const { url, events, organizationId, metadata, retryPolicy } = validation.data;

    // Check rate limits (max 10 webhooks per user)
    const registry = WebhookRegistry.getInstance();
    const existingWebhooks = await registry.getSubscriptions(user.uid, organizationId);
...
export async function GET(request: NextRequest) {
  try {
    ...
    const registry = WebhookRegistry.getInstance();
    const subscriptions = await registry.getSubscriptions(user.uid, organizationId);
...
async function validateWebhookOwnership(webhookId: string, userId: string, registry: WebhookRegistry) {
