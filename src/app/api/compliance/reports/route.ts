...
async function generateDetailedComplianceReport(
  ...
  const report = {
    ...
    userData: includeUserData ? userDataRecords.slice(0, 100) : [], // Limit for privacy
...
function analyzeComplianceMetrics(events: any[], _reportType: string) {
  ...
  return {
    ...
    complianceEvents: {
      ...
      documentOperations: events.filter((e) =>
        e.eventType?.includes('DOCUMENT_'),
      ).length,
    },
    dataRetention: {
      oldestEvent:
    ...
    integrityChecks: {
      ...
      integrityScore:
        events.length > 0
          ? (events.filter((e) => e.hash).length / events.length) * 100
          : 0,
    },
  };
}
...
function generateRecommendations(
  ...
  if (metrics.totalEvents === 0) {
    recommendations.push(
      '🚨 No audit events found - ensure audit logging is properly configured',
    );
  }

  if (metrics.integrityChecks.integrityScore < 95) {
    recommendations.push(
      '⚠️ Some events lack cryptographic hashes - verify audit event creation process',
    );
  }

  if (metrics.complianceEvents.policyInteraction === 0) {
    recommendations.push(
      '📋 No policy interaction events - ensure terms acceptance is being logged',
    );
  }

  if (metrics.weeklyEvents === 0) {
    recommendations.push(
      '📊 No recent activity - monitor system usage and engagement',
    );
  }

  if (complianceLevel === 'compliant') {
    recommendations.push(
      '✅ System meets compliance requirements - maintain current practices',
    );
  }

  if (recommendations.length === 0) {
    recommendations.push(
      '✅ All compliance checks passed - excellent audit trail maintenance',
    );
  }
...
function analyzeGDPRCompliance(events: any[]) {
  const dataExportEvents = events.filter(
    (e) => e.eventType === 'DATA_EXPORT',
  ).length;
  const consentEvents = events.filter((e) =>
    e.eventType?.includes('CONSENT'),
  ).length;
  const dataAccessEvents = events.filter(
    (e) => e.eventType === 'DATA_ACCESS',
  ).length;
  ...
  return {
    ...
    complianceScore: Math.min(
      100,
      ((dataExportEvents > 0 ? 1 : 0) +
        (consentEvents > 0 ? 1 : 0) +
        (dataAccessEvents > 0 ? 1 : 0)) *
        33.33,
    ),
  };
}
...
function analyzeCCPACompliance(events: any[]) {
  ...
  return {
    ...
    complianceScore: Math.min(
      100,
      ((dataExportEvents > 0 ? 1 : 0) + (dataDeletionEvents > 0 ? 1 : 0)) * 50,
    ),
  };
}
...
function convertReportToCSV(report: any): string {
  ...
  // Basic metrics
  csvLines.push(
    `Total Events,${report.complianceMetrics?.totalEvents || 0},"Overall audit event count"`,
  );
  csvLines.push(
    `Recent Events,${report.complianceMetrics?.recentEvents || 0},"Events in last 30 days"`,
  );
  csvLines.push(
    `Unique Users,${report.complianceMetrics?.uniqueUsers || 0},"Number of unique users with events"`,
  );

  // Compliance scores
  if (report.certifications) {
    csvLines.push(
      `GDPR Compliance,${report.certifications.gdpr?.complianceScore || 0}%,"GDPR compliance score"`,
    );
    csvLines.push(
      `CCPA Compliance,${report.certifications.ccpa?.complianceScore || 0}%,"CCPA compliance score"`,
    );
    csvLines.push(
      `SOC2 Compliance,${report.certifications.soc2?.complianceScore || 0}%,"SOC2 compliance score"`,
    );
  }
