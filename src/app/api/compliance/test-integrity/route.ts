...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Running integrity tests for user ${user.uid}, type: ${testType}`);
    ...
    if (testType === 'full' || testType === 'hashing') {
      try {
        ...
        const hasValidHash = testEvent.hash && 
...
function generateRecommendations(results: IntegrityTestResult[]): string[] {
  ...
  if (failedTests.length === 0) {
    recommendations.push('✅ All integrity tests passed - your audit trail is functioning correctly');
  ...
  failedTests.forEach(test => {
    switch (test.testName) {
      case 'Event Creation':
        recommendations.push('🔧 Check Firestore permissions and audit service configuration');
        break;
      case 'Event Retrieval':
        recommendations.push('🔧 Verify database connectivity and query permissions');
        break;
      case 'Integrity Verification':
        recommendations.push('⚠️ Critical: Audit trail integrity compromised - investigate immediately');
        break;
      case 'Hash Generation':
        recommendations.push('🔧 Check cryptographic functions and audit event manager');
        break;
      case 'Data Export':
        recommendations.push('🔧 Verify export functionality and data access permissions');
        break;
    }
  });
  
  if (failedTests.length > 2) {
    recommendations.push('🚨 Multiple critical failures detected - contact system administrator');
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      availableTests: [
        { id: 'full', name: 'Full Integrity Test Suite', description: 'Runs all available integrity tests' },
        { id: 'creation', name: 'Event Creation Test', description: 'Tests audit event creation' },
        { id: 'retrieval', name: 'Event Retrieval Test', description: 'Tests audit event retrieval' },
        { id: 'verification', name: 'Integrity Verification Test', description: 'Tests cryptographic integrity' },
        { id: 'hashing', name: 'Hash Generation Test', description: 'Tests event hash generation' },
        { id: 'export', name: 'Data Export Test', description: 'Tests data export functionality' }
      ],
      recommendations: [
