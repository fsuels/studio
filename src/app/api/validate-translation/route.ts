...
function validateTranslation(
  ...
  Object.entries(LEGAL_TERMS_DB).forEach(([englishTerm, termData]) => {
    if (englishWords.some(word => word.includes(englishTerm.replace('-', ' ')))) {
      ...
      if (!hasValidTranslation) {
        ...
        result.issues.push(`Missing translation for legal term: ${englishTerm}`);
        result.recommendations.push(`Consider using: ${expectedSpanishTerms.join(' or ')}`);
      }
    }
  });
  ...
  result.confidence = Math.round(
    (lengthScore * 0.2 + terminologyScore * 0.5 + structureScore * 0.2 + variableScore * 0.1)
  );
  ...
  try {
    ...
    if (!englishText || !spanishText || !documentId) {
      return NextResponse.json(
        { error: 'Missing required fields: englishText, spanishText, documentId' },
        { status: 400 }
      );
    }

    // Perform validation
    const validationResult = validateTranslation(englishText, spanishText, documentId);
...
export async function GET() {
  return NextResponse.json({
    ...
    version: '1.0.0',
    methods: ['POST'],
    description: 'Validates Spanish legal translations against English originals'
  });
}