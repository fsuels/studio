...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const timeOfDayMultiplier = Math.sin((todayMs / (24 * 60 * 60 * 1000)) * Math.PI) * 0.3 + 0.7;
    ...
    if (now.getHours() < 9 || now.getHours() > 18) {
      ...
      baseMetrics.avgResponseTime = Math.floor(baseMetrics.avgResponseTime * 1.8); // Slower response
    }
    ...
    const dailyPatterns = {
      ...
      // End of day spike
      ...(now.getHours() >= 16 && now.getHours() <= 18 && {
...
function generateRecommendations(metrics: any): string[] {
  ...
  if (metrics.openIssues > 5) {
    recommendations.push('Consider adding more support agents during peak hours');
  }
  ...
  if (metrics.conversionRate < 3.0) {
    recommendations.push('Analyze checkout flow for optimization opportunities');
  }
  ...
  if (metrics.revenueChange > 5) {
    recommendations.push('Great revenue growth! Consider scaling successful initiatives');
  }
  
  if (metrics.openIssues < 3 && metrics.avgResponseTime < 60) {
    recommendations.push('Excellent support performance! Maintain current standards');
  }
