        ...
        return NextResponse.json({
          ...
          data: {
            summary: {
              ...
              avgLTV: intelligence.customerLTV.reduce((sum, c) => sum + c.ltv, 0) / intelligence.customerLTV.length,
              churnRisk: intelligence.churnPredictions.filter(c => c.churnRisk === 'high' || c.churnRisk === 'critical').length,
              revenueLeakage: intelligence.revenueLeakage.reduce((sum, leak) => sum + leak.amount, 0)
            },
      ...
      case 'cohort_analysis':
        return NextResponse.json({
          ...
          data: {
            ...
            retentionSummary: calculateRetentionSummary(intelligence.cohortAnalysis),
            revenueRetention: calculateRevenueRetentionSummary(intelligence.cohortAnalysis),
      ...
      case 'churn_prediction':
        return NextResponse.json({
          ...
          data: {
            ...
            riskDistribution: calculateChurnRiskDistribution(intelligence.churnPredictions),
            preventionStrategies: generatePreventionStrategies(intelligence.churnPredictions),
            expectedImpact: calculateChurnImpact(intelligence.churnPredictions, intelligence.customerLTV)
          }
        });
      ...
      case 'revenue_leakage':
        return NextResponse.json({
          ...
          data: {
            ...
            recoveryOpportunities: identifyRecoveryOpportunities(intelligence.revenueLeakage),
            preventionPlan: generateLeakagePreventionPlan(intelligence.revenueLeakage),
...
function groupCustomersBySegment(customers: any[]) {
  ...
  return {
    high_value: {
      ...
      avgLTV: segments.high_value.length > 0 ? segments.high_value.reduce((sum, c) => sum + c.ltv, 0) / segments.high_value.length : 0
    },
    medium_value: {
      count: segments.medium_value.length,
      totalLTV: segments.medium_value.reduce((sum, c) => sum + c.ltv, 0),
      avgLTV: segments.medium_value.length > 0 ? segments.medium_value.reduce((sum, c) => sum + c.ltv, 0) / segments.medium_value.length : 0
    },
    low_value: {
      count: segments.low_value.length,
      totalLTV: segments.low_value.reduce((sum, c) => sum + c.ltv, 0),
      avgLTV: segments.low_value.length > 0 ? segments.low_value.reduce((sum, c) => sum + c.ltv, 0) / segments.low_value.length : 0
    },
    at_risk: {
      count: segments.at_risk.length,
      totalLTV: segments.at_risk.reduce((sum, c) => sum + c.ltv, 0),
      avgLTV: segments.at_risk.length > 0 ? segments.at_risk.reduce((sum, c) => sum + c.ltv, 0) / segments.at_risk.length : 0
    }
  };
}
...
function calculateRetentionSummary(cohorts: any[]) {
  ...
  if (cohorts.length > 0) {
    summary.avgMonth1 = cohorts.reduce((sum, c) => sum + c.retentionRates.month1, 0) / cohorts.length;
    summary.avgMonth3 = cohorts.reduce((sum, c) => sum + c.retentionRates.month3, 0) / cohorts.length;
    summary.avgMonth6 = cohorts.reduce((sum, c) => sum + c.retentionRates.month6, 0) / cohorts.length;
    summary.avgMonth12 = cohorts.reduce((sum, c) => sum + c.retentionRates.month12, 0) / cohorts.length;
    summary.avgMonth24 = cohorts.reduce((sum, c) => sum + c.retentionRates.month24, 0) / cohorts.length;
  }
...
function compareCohorts(cohorts: any[]) {
  ...
  return {
    ...
    sizeChange: ((latest.cohortSize - previous.cohortSize) / previous.cohortSize) * 100,
    revenueChange: ((latest.totalRevenue - previous.totalRevenue) / previous.totalRevenue) * 100,
    retentionChange: {
...
function generatePreventionStrategies(predictions: any[]) {
  const highRisk = predictions.filter(p => p.churnRisk === 'high' || p.churnRisk === 'critical');
...
function calculateChurnImpact(churnPredictions: any[], customerLTV: any[]) {
  const atRiskCustomers = churnPredictions.filter(p => p.churnRisk === 'high' || p.churnRisk === 'critical');
  ...
  return {
    ...
    averageLossPerCustomer: atRiskCustomers.length > 0 ? potentialLoss / atRiskCustomers.length : 0,
...
function identifyRecoveryOpportunities(leakage: any[]) {
  return leakage.map(leak => ({
    ...
    priority: leak.percentage > 5 ? 'high' : leak.percentage > 2 ? 'medium' : 'low',
...
function calculateLeakageTrends(orders: any[]) {
  ...
  return Array.from(monthlyLeakage.values()).sort((a, b) => a.month.localeCompare(b.month));
}