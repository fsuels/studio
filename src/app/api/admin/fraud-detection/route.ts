...
import { advancedFraudDetection, type FraudRiskAssessment } from '@/lib/advanced-fraud-detection';
  ...
  try {
    ...
    switch (type) {
      ...
      case 'risk_assessments':
        const assessments = await getFraudAssessments(timeframe, riskLevel, page, limit);
        return NextResponse.json({
          success: true,
          data: assessments
        });
  ...
  try {
    ...
    switch (action) {
      ...
      case 'bulk_assess':
        ...
        for (const order of orders) {
          try {
            ...
            bulkResults.push({ orderId: order.orderId, success: true, assessment: result });
          } catch (error) {
            bulkResults.push({ 
      ...
      case 'update_feedback':
        ...
        await updateAssessmentFeedback(assessmentId, actualOutcome, fraudConfirmed);
...
async function getFraudOverview(timeframe: string) {
  ...
  const highRiskCount = assessments.filter(a => a.riskLevel === 'high' || a.riskLevel === 'very_high').length;
  const autoDeclined = assessments.filter(a => a.recommendation === 'decline').length;
  const manualReviews = assessments.filter(a => a.recommendation === 'manual_review' || a.recommendation === 'review').length;
  
  const avgProcessingTime = assessments.reduce((sum, a) => sum + a.processingTime, 0) / totalAssessments;
  const avgRiskScore = assessments.reduce((sum, a) => sum + a.overallScore, 0) / totalAssessments;
  ...
  const topRiskFactors = Object.entries(factorCounts)
    .sort(([,a], [,b]) => b - a)
    .slice(0, 10)
    .map(([factor, count]) => ({ factor, count, percentage: (count / totalAssessments) * 100 }));

  // Chargeback predictions
  const chargebackPredictions = assessments.map(a => a.chargebackLikelihood);
  const avgChargebackRisk = chargebackPredictions.reduce((sum, p) => sum + p.probability, 0) / chargebackPredictions.length;
  const highChargebackRisk = chargebackPredictions.filter(p => p.riskBand === 'D' || p.riskBand === 'E').length;
...
async function getFraudAssessments(timeframe: string, riskLevel?: string, page: number = 1, limit: number = 50) {
...
async function getChargebackPredictions(timeframe: string) {
  ...
  return {
    overview: {
      ...
      avgChargebackRisk: (predictions.reduce((sum, p) => sum + p.probability, 0) / predictions.length * 100).toFixed(2),
      highRiskTransactions: predictions.filter(p => p.riskBand === 'D' || p.riskBand === 'E').length,
      estimatedChargebacks: predictions.reduce((sum, p) => sum + p.probability, 0).toFixed(1),
      estimatedLosses: predictions.reduce((sum, p) => sum + p.expectedLoss, 0).toFixed(2)
    },
    riskBands: {
...
async function getFraudTrends(timeframe: string) {
  ...
  const recentAvg = recent.reduce((sum, t) => sum + t.avgRiskScore, 0) / recent.length;
  const previousAvg = previous.reduce((sum, t) => sum + t.avgRiskScore, 0) / previous.length;
  const riskScoreChange = ((recentAvg - previousAvg) / previousAvg * 100).toFixed(1);

  const recentDeclined = recent.reduce((sum, t) => sum + t.declined, 0);
  const previousDeclined = previous.reduce((sum, t) => sum + t.declined, 0);
  const declineRateChange = ((recentDeclined - previousDeclined) / previousDeclined * 100).toFixed(1);
  ...
  return {
    ...
    insights: {
      riskScoreChange: {
        ...
        direction: parseFloat(riskScoreChange) > 0 ? 'increasing' : 'decreasing'
      },
      declineRateChange: {
        value: declineRateChange,
        direction: parseFloat(declineRateChange) > 0 ? 'increasing' : 'decreasing'
      },
      seasonalPatterns: [
...
async function updateAssessmentFeedback(assessmentId: string, actualOutcome: string, fraudConfirmed: boolean) {
  // In production, update the assessment with actual outcome for ML training
  console.log(`Updated assessment ${assessmentId}: outcome=${actualOutcome}, fraud=${fraudConfirmed}`);
}
...
function generateMockAssessments(count: number): FraudRiskAssessment[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const riskLevel = overallScore >= 800 ? 'very_high' :
                     overallScore >= 600 ? 'high' :
                     overallScore >= 400 ? 'medium' :
                     overallScore >= 200 ? 'low' : 'very_low';
    
    assessments.push({
      overallScore,
      riskLevel: riskLevel as any,
      recommendation: overallScore >= 700 ? 'decline' :
                     overallScore >= 500 ? 'manual_review' :
                     overallScore >= 300 ? 'review' : 'approve',
      ...
      chargebackLikelihood: {
        ...
        riskBand: ['A', 'B', 'C', 'D', 'E'][Math.floor(Math.random() * 5)] as any,
...
function generateMockRiskFactors() {
  const allFactors = [
    { category: 'velocity', factor: 'high_email_velocity', severity: 'high', impact: 200, description: 'Email used in 5 orders in 24h' },
    { category: 'velocity', factor: 'high_ip_velocity', severity: 'medium', impact: 150, description: 'IP used in 8 orders this week' },
    { category: 'device', factor: 'vpn_detected', severity: 'medium', impact: 100, description: 'VPN or proxy usage detected' },
    { category: 'device', factor: 'new_device', severity: 'low', impact: 50, description: 'First time seeing this device' },
    { category: 'geographic', factor: 'country_mismatch', severity: 'medium', impact: 120, description: 'IP country differs from billing' },
    { category: 'geographic', factor: 'high_distance', severity: 'high', impact: 180, description: 'IP 800km from billing address' },
    { category: 'payment', factor: 'high_value_first_order', severity: 'medium', impact: 130, description: 'High-value first-time purchase' },
    { category: 'behavioral', factor: 'unusual_time', severity: 'low', impact: 60, description: 'Order placed at unusual hour' }
  ];
...
function generateMockChargebackPredictions(count: number) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const riskBand = probability >= 0.15 ? 'E' :
                    probability >= 0.08 ? 'D' :
                    probability >= 0.04 ? 'C' :
                    probability >= 0.02 ? 'B' : 'A';
