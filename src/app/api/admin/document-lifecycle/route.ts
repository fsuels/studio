...
import { documentLifecycle, type DocumentStatus, type DocumentPriority } from '@/lib/document-lifecycle';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    switch (type) {
      ...
      case 'heatmap':
        const heatmapMetrics = documentLifecycle.generateWorkflowMetrics(timeframe);
        return NextResponse.json({
          success: true,
          data: {
      ...
      case 'workflow_metrics':
        const workflowMetrics = documentLifecycle.generateWorkflowMetrics(timeframe);
        return NextResponse.json({
          success: true,
      ...
      case 'documents':
        const documents = await getDocumentsList(status as DocumentStatus, priority as DocumentPriority, timeframe);
        return NextResponse.json({
          success: true,
          data: documents
        });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'auto_unblock':
        const unblockResult = await documentLifecycle.autoUnblockStalledDocuments();
        return NextResponse.json({
          success: true,
          data: unblockResult
        });
...
function generateMockDocuments(count: number) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    documents.push({
      ...
      priority,
      createdAt,
      updatedAt: new Date(new Date(createdAt).getTime() + Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString(),
      ...
      stallReason: Math.random() < 0.15 ? 'Customer not responding to e-signature request' : undefined,
      stalledAt: Math.random() < 0.15 ? new Date(Date.now() - Math.random() * 3 * 24 * 60 * 60 * 1000).toISOString() : undefined,
      stallThreshold: 24 * 3600, // 24 hours
      lastReminderSent: Math.random() < 0.3 ? new Date(Date.now() - Math.random() * 24 * 60 * 60 * 1000).toISOString() : undefined,
      reminderCount: Math.floor(Math.random() * 3),
      escalationLevel: Math.floor(Math.random() * 3),
      assignedTo: Math.random() < 0.5 ? `admin_${Math.floor(Math.random() * 5) + 1}` : undefined,
      value: Math.random() < 0.7 ? Math.floor(Math.random() * 500) + 35 : undefined,
      tags: [],
      notes: '',
      version: 1,
      events: []
...
function generateStatusTimeline(createdAt: string, currentStatus: DocumentStatus) {
  ...
  for (let i = 1; i <= currentIndex; i++) {
    ...
    const statusKey = statusOrder[i].replace('pending_', 'pending') as keyof typeof timeline;
