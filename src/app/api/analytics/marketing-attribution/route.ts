    ...
    switch (type) {
      case 'dashboard':
        ...
        return NextResponse.json({
          ...
          data: dashboardData,
          metadata: {
            totalRecords: dashboardData.channels.length + dashboardData.campaigns.length,
            timeframe,
      ...
      case 'campaigns':
        const campaignData = await getCampaignPerformance(timeframe, channels, campaigns);
        return NextResponse.json({
          success: true,
          data: campaignData
        });
      ...
      case 'attribution_comparison':
        const attributionComparison = await getAttributionModelComparison(timeframe);
        return NextResponse.json({
          success: true,
      ...
      case 'conversion_paths':
        const conversionPaths = await getTopConversionPaths(timeframe, attribution);
        return NextResponse.json({
          success: true,
...
// Helper functions
async function generateMarketingDashboard(request: CampaignAnalyticsRequest): Promise<MarketingDashboardData> {
  ...
  return {
    ...
    channels: await getChannelROI(request.timeframe?.startDate || '30d', request.attributionModel || 'last_touch'),
    campaigns: await getCampaignPerformance(request.timeframe?.startDate || '30d', request.channels, request.campaigns),
    attribution: {
      modelComparison: {
      ...
      conversionPaths: [
        ...
        { path: 'LinkedIn → Email → Google → Order', conversions: 45, revenue: 14200 },
...
async function getChannelROI(timeframe: string, attribution: string): Promise<ChannelROI[]> {
...
async function getCampaignPerformance(timeframe: string, channels?: string[], campaigns?: string[]): Promise<CampaignPerformance[]> {
...
async function getDiscountAnalytics(timeframe: string): Promise<DiscountCodeAnalytics[]> {
...
async function generateMarketingInsights(timeframe: string): Promise<MarketingInsight[]> {
  ...
  return [
    ...
    },
    {
      ...
      description: 'Organic search traffic dropped 12% compared to last month. SEO optimization needed.',
      impact: -8500,
      confidence: 73,
      dataPoints: [
    ...
    },
    {
      ...
      description: 'WELCOME20 discount code generated $12k in incremental revenue this month.',
      impact: 12000,
      confidence: 95,
      dataPoints: [
...
// Utility functions
function getTimeframeFromString(timeframe: string): { startDate: string; endDate: string } {
  const now = new Date();
