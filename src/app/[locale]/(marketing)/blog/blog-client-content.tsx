...
function getCategory(article: (typeof blogArticles)[0]) {
  ...
  if (
    ...
    title.includes('prenuptial') ||
    title.includes('minors') ||
    title.includes('pet agreements')
  )
    return 'Family';
  if (
    title.includes('home') ||
    ...
    return 'Real Estate';
  if (title.includes('green card') || title.includes('immigrants'))
    return 'Immigration';
  if (
    title.includes('hire') ||
    title.includes('employee') ||
    title.includes('remote work')
  )
    return 'Employment';
  if (
...
export default function BlogClientContent({ locale }: BlogClientContentProps) {
  ...
  if (!isHydrated) {
    return (
      <div className="max-w-5xl mx-auto px-4 py-12 h-screen animate-pulse bg-muted"></div>
    );
  }
      ...
      <div className="grid gap-4">
        {pageData.length === 0 ? (
          <p className="text-muted-foreground italic">
            {t('No articles found for your criteria.', {
              defaultValue: 'No articles found matching your criteria.',
            })}
          </p>
        ) : (
          pageData.map((post) => (
            ...
            >
              <div className="block border rounded-xl p-4 shadow hover:shadow-md transition hover:bg-muted cursor-pointer">
                <h2 className="text-xl font-semibold mb-1">
                  {post[`title${langSuffix}` as keyof typeof post]}
                </h2>
                ...
                <p className="text-sm text-muted-foreground">
                  {post[`summary${langSuffix}` as keyof typeof post]}
                </p>
                <span className="mt-2 inline-block text-primary text-sm hover:underline">
                  {t('blog.readMore')} →
                </span>
              </div>
            </Link>
