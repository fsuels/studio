...
import { getDocumentsForCountry, usStates, allDocuments } from '@/lib/document-library'
...
import { SchemaMarkup, generateDocumentFAQs, generateDocumentHowToSteps } from '@/components/seo/SchemaMarkup'
...
export async function generateMetadata({ params }: StateDocumentPageProps): Promise<Metadata> {
  ...
  const documentName = document.translations?.[locale]?.name || document.name || document.id
  const documentDesc = document.translations?.[locale]?.description || document.description || ''
  ...
  return {
    ...
    description,
    keywords: isSpanish
      ? [`${documentName} ${stateName}`, `plantilla ${documentName}`, `${documentName} gratis`, `formularios ${stateName}`]
      : [`${stateName} ${documentName}`, `${documentName} template`, `free ${documentName}`, `${stateName} forms`],
    openGraph: {
      title,
      description,
      type: 'website',
...
export default function StateDocumentPage({ params }: StateDocumentPageProps) {
  ...
  const metaTags = generateDocumentMetaTags(documentName, stateName, undefined, locale)
