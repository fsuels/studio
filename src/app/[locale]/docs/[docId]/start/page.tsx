...
export async function generateStaticParams() {
  console.log(
    '[generateStaticParams /docs/[docId]/start] Starting generation...',
  );
  if (!documentLibrary || documentLibrary.length === 0) {
    console.warn(
      '[generateStaticParams /docs/[docId]/start] documentLibrary is empty or undefined. No paths will be generated.',
    );
    return [];
  }
  if (!localizations || localizations.length === 0) {
    ...
    console.warn(
      '[generateStaticParams /docs/[docId]/start] localizations is empty or undefined. Defaulting to EN/ES if possible, otherwise no paths.',
    );
    ...
    for (const locale of defaultLocales) {
      for (const doc of documentLibrary) {
        ...
        if (doc && doc.id && doc.id !== 'general-inquiry' && doc.schema) {
          // Added check for doc.schema
          params!.push({ locale: locale, docId: doc.id });
        } else if (!doc || !doc.id) {
          console.warn(
            `[generateStaticParams /docs/[docId]/start] Encountered a document with missing id in locale ${locale}. Skipping.`,
          );
        } else if (!doc.schema) {
          console.warn(
            `[generateStaticParams /docs/[docId]/start] Document with id ${doc.id} is missing a schema in locale ${locale}. Skipping.`,
          );
        }
      }
    }
    if (params!.length === 0)
      console.warn(
        '[generateStaticParams /docs/[docId]/start] No params generated with fallback locales.',
      );
    return params;
  }
  ...
  for (const localeObj of localizations) {
    const locale =
      typeof localeObj === 'string'
        ? localeObj
        : (localeObj as { id: string }).id; // Handle both formats
    if (!locale) {
      console.warn(
        `[generateStaticParams /docs/[docId]/start] Invalid locale object encountered:`,
        localeObj,
      );
      continue;
    }
    for (const doc of documentLibrary) {
      if (!doc.id) {
        console.warn(
          `[generateStaticParams /docs/[docId]/start] Document with missing ID encountered:`,
          doc,
        );
        continue;
      }
      ...
      if (doc && doc.id !== 'general-inquiry' && doc.schema) {
        ...
        console.warn(
          `[generateStaticParams /docs/[docId]/start] Document with id ${doc.id} is missing a schema in locale ${locale}. Skipping generation for this combination.`,
        );
      }
    }
  }
  console.log(
    `[generateStaticParams /docs/[docId]/start] Generated ${params!.length} params!.`,
  );
  if (params!.length === 0) {
    console.warn(
      '[generateStaticParams /docs/[docId]/start] No params were generated. Check documentLibrary and localizations content and structure.',
    );
