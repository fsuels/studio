...
export async function generateStaticParams() {
  ...
  if (!supportedCountries || supportedCountries.length === 0) {
    console.warn('[generateStaticParams /docs/[docId]/start] supportedCountries is empty.');
    return [];
  }
  if (!localizations || localizations.length === 0) {
    ...
    for (const locale of defaultLocales) {
      for (const country of supportedCountries) {
        for (const doc of getDocumentsForCountry(country)) {
        ...
        if (doc && doc.id && doc.id !== 'general-inquiry' && doc.schema) {
            params!.push({ locale: locale, country, docId: doc.id });
        } else if (!doc || !doc.id) {
  ...
  for (const localeObj of localizations) {
    const locale = typeof localeObj === 'string' ? localeObj : (localeObj as { id: string }).id;
    if (!locale) {
