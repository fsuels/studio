...
import { formatDuration, type DocumentStatus, type DocumentPriority } from '@/lib/document-lifecycle';

interface DocumentLifecycleDashboardProps {
  className?: string;
}

export default function DocumentLifecycleDashboard({ className }: DocumentLifecycleDashboardProps) {
  ...
  const fetchData = async () => {
    try {
      ...
      const [overviewRes, heatmapRes, stalledRes, metricsRes] = await Promise.all([
        fetch(`/api/admin/document-lifecycle?type=overview&${params}`),
        fetch(`/api/admin/document-lifecycle?type=heatmap&${params}`),
        fetch(`/api/admin/document-lifecycle?type=stalled_analysis&${params}`),
        fetch(`/api/admin/document-lifecycle?type=workflow_metrics&${params}`)
      ]);

      const [overviewData, heatmapData, stalledData, metricsData] = await Promise.all([
        overviewRes.json(),
        heatmapRes.json(),
        stalledRes.json(),
        metricsRes.json()
  ...
  const fetchRealtimeData = async () => {
    try {
      const response = await fetch('/api/admin/document-lifecycle?type=realtime_status');
  ...
  const handleAutoUnblock = async () => {
    try {
      ...
      if (result.success) {
        ...
        alert(`Auto-unblock completed: ${result.data.successful}/${result.data.attempted} documents unblocked`);
      }
    } catch (err) {
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-6 text-sm">
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-semibold">{realtimeStatus.activeDocuments}</span>
                  <span className="text-muted-foreground">active</span>
                </div>
                <div className="flex items-center gap-1">
                  <PauseCircle className="h-4 w-4 text-red-600" />
                  <span className="font-semibold">{realtimeStatus.stalledDocuments}</span>
                  <span className="text-muted-foreground">stalled</span>
                </div>
                <div className="flex items-center gap-1">
                  <Clock className="h-4 w-4 text-orange-600" />
                  <span className="font-semibold">{realtimeStatus.overDueDocuments}</span>
                  <span className="text-muted-foreground">overdue</span>
                </div>
                <div className="flex items-center gap-1">
                  <AlertTriangle className="h-4 w-4 text-red-600" />
                  <span className="font-semibold">{realtimeStatus.urgentDocuments.length}</span>
        ...
        <TabsContent value="overview" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold">{overview.overview.totalActiveDocuments}</p>
                      <div className="text-sm text-muted-foreground">
                        ${overview.overview.totalValue?.toLocaleString() || '0'} value
                      </div>
                    </div>
              ...
              <Card>
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-muted-foreground">Stalled Documents</p>
                      <p className="text-2xl font-bold text-red-600">{overview.overview.stalledDocuments}</p>
                      <div className="text-sm text-muted-foreground">
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold">{formatDuration(overview.overview.averageCompletionTime || 0)}</p>
                      <div className="text-sm text-muted-foreground">
                        Target: 5 days
                      </div>
                    </div>
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold">{formatPercentage(overview.overview.completionRate || 0)}</p>
                      <div className="text-sm text-muted-foreground">
                        Last 30 days
                      </div>
                    </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {overview.statusDistribution.map((status: any, index: number) => (
                    <div key={index} className="p-4 border rounded-lg">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-semibold">{formatDuration(status.avgTimeInStatus)}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span className="text-muted-foreground">Stalled</span>
                          <span className={`font-semibold ${status.stalledCount > 0 ? 'text-red-600' : 'text-green-600'}`}>
                            {status.stalledCount}
                          </span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span className="text-muted-foreground">Bottleneck Score</span>
                          <span className="font-semibold">{status.bottleneckScore.toFixed(0)}/100</span>
                        </div>
                      </div>
                      ...
                      <div className="mt-3">
                        ...
                        <Progress value={status.bottleneckScore} className="h-2" />
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    <div key={index} className="flex items-center justify-between p-3 border rounded-lg bg-red-50/50 border-red-200">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">{doc.customerEmail}</div>
                          <div className="text-xs text-red-600">
                            Stalled for {formatDuration(doc.stalledFor)} in {doc.currentStatus.replace('_', ' ')}
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        ...
                        <Button size="sm" variant="outline" onClick={() => handleStatusUpdate(doc.documentId, 'pending_review')}>
          ...
          {overview && overview.topBottlenecks && (
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {overview.topBottlenecks.map((bottleneck: any, index: number) => (
                    <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${getHeatmapIntensity(bottleneck.bottleneckScore)}`}>
                          <span className="text-sm font-semibold text-white">{index + 1}</span>
                        </div>
                        <div>
                          <div className="font-medium capitalize">{bottleneck.status.replace('_', ' ')}</div>
                          <div className="text-sm text-muted-foreground">
                            {bottleneck.count} documents • {formatDuration(bottleneck.avgTimeInStatus)} avg time
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="text-right">
                          <div className="text-sm font-semibold text-red-600">{bottleneck.bottleneckScore.toFixed(0)}</div>
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  Color intensity indicates bottleneck severity (red = high bottleneck, green = smooth flow)
                </p>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-6">
                  {heatmapData.statusDistribution.map((status: any, index: number) => (
                    <div key={index} className={`p-4 rounded-lg border-2 ${getHeatmapIntensity(status.bottleneckScore)} text-white`}>
                      <div className="flex items-center gap-2 mb-2">
                        {getStatusIcon(status.status)}
                        <span className="font-medium text-sm capitalize">{status.status.replace('_', ' ')}</span>
                      </div>
                      <div className="text-2xl font-bold">{status.count}</div>
                      <div className="text-sm opacity-90">{formatPercentage(status.percentage)}</div>
          ...
          {heatmapData && heatmapData.performanceMetrics && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {Object.entries(heatmapData.performanceMetrics.avgTimeByStatus || {}).map(([status, time]: [string, any]) => (
                      <div key={status} className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          {getStatusIcon(status as DocumentStatus)}
                          <span className="capitalize">{status.replace('_', ' ')}</span>
                        </div>
                        <span className="font-semibold">{formatDuration(time)}</span>
                      </div>
                    ))}
                  </div>
                </CardContent>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {Object.entries(heatmapData.performanceMetrics.completionRateByType || {}).map(([type, rate]: [string, any]) => (
          ...
          {stalledAnalysis && (
            <>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {stalledAnalysis.stalledDocuments.slice(0, 10).map((doc: any, index: number) => (
                      <div key={index} className="p-4 border rounded-lg">
                        ...
                        <div className="mb-3">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {doc.recommendedActions.map((action: string, i: number) => (
                        ...
                        <div className="space-y-2">
                          <div className="text-sm font-medium">Unblocking Steps:</div>
                            ...
                            <div key={i} className="flex items-center justify-between text-xs p-2 bg-gray-50 rounded">
                              ...
                              <div className="flex items-center gap-2">
                                <Badge variant="outline" className={
                                  step.responsibility === 'customer' ? 'bg-blue-50' :
                                  step.responsibility === 'admin' ? 'bg-orange-50' : 'bg-green-50'
                                }>
                                  {step.responsibility}
                                </Badge>
                                <span>{formatDuration(step.estimatedTime)}</span>
                                {step.automated && <Badge variant="secondary">Auto</Badge>}
                              </div>
                            </div>
                          ))}
                        </div>
              ...
              {stalledAnalysis.unblockingOpportunities && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Card>
                    ...
                    <CardContent>
                      <ul className="space-y-2">
                        {stalledAnalysis.unblockingOpportunities.automationOpportunities.map((opportunity: string, i: number) => (
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <ul className="space-y-2">
                        {stalledAnalysis.unblockingOpportunities.processImprovements.map((improvement: string, i: number) => (
                          <li key={i} className="flex items-center gap-2 text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-2xl font-bold">{formatPercentage(workflowMetrics.performanceMetrics?.escalationRate || 0)}</p>
                      </div>
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-muted-foreground">Automation Efficiency</p>
                        <p className="text-2xl font-bold">{formatPercentage(workflowMetrics.performanceMetrics?.automationEfficiency || 0)}</p>
                      </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-center text-muted-foreground py-8">
                      Workflow trends visualization would be implemented here with charts
                    </div>
                  </CardContent>
                </Card>
              )}
          ...
          {realtimeStatus && (
            <>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {Object.entries(realtimeStatus.statusDistribution).map(([status, count]: [string, any]) => (
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {realtimeStatus.recentActivity.map((activity: any, index: number) => (
                      <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-medium">{activity.documentType}</div>
                            <div className="text-sm text-muted-foreground">{activity.customerEmail}</div>
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <div className="flex items-center gap-2 text-sm">
                            <Badge className={getStatusColor(activity.statusChange.from)}>
                              {activity.statusChange.from.replace('_', ' ')}
                            </Badge>
                            <ArrowRight className="h-3 w-3" />
                            <Badge className={getStatusColor(activity.statusChange.to)}>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="space-y-3">
                    {realtimeStatus.urgentDocuments.map((doc: any, index: number) => (
                      <div key={index} className="flex items-center justify-between p-3 border rounded-lg bg-red-50/50 border-red-200">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-red-100 rounded-full flex items-center justify-center">
                            <AlertTriangle className="h-4 w-4 text-red-600" />
                          ...
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">{doc.customerEmail}</div>
                            <div className="text-xs text-red-600">
                              Stalled for {formatDuration(doc.stalledFor)} • ${doc.value?.toLocaleString()}
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
