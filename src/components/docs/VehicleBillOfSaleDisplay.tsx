...
export default function VehicleBillOfSaleDisplay({ locale }: VehicleBillOfSaleDisplayProps) {
  ...
  const { addItem } = useCart(); // Example usage, adjust as needed
  ...
  const handleStartProcess = () => {
    const itemName = t('metaTitle'); // Or a more specific item name for cart
    const priceCents = 1995; // Assuming a price for this document
  ...
  const tocItems = [
    { id: "what-is", key: "toc.whatIs" },
    { id: "why", key: "toc.why" },
    { id: "covered", key: "toc.covered" },
    { id: "types", key: "toc.types" },
    { id: "components", key: "toc.components" },
    { id: "how-to", key: "toc.howTo" },
    { id: "state-rules", key: "toc.stateRules" },
    { id: "checklist", key: "toc.checklist" },
    { id: "supporting", key: "toc.supporting" },
    { id: "why-us", key: "toc.whyUs" },
    { id: "faq", key: "toc.faq" }
  ];
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <article className="prose dark:prose-invert max-w-4xl mx-auto">
        ...
        <section id="table-of-contents" className="mb-12 p-6 bg-muted/50 rounded-lg border border-border">
          <h2 className="text-xl font-semibold text-foreground mb-4">{t('tocTitle')}</h2>
          <ul className="list-decimal list-inside space-y-1 text-primary">
            {tocItems.map((item, index) => (
              <li key={item.id}>
                <a href={`#${item.id}`} className="hover:underline">{index + 1}. {t(item.key)}</a>
              </li>
            ))}
          </ul>
        </section>
        ...
        {sections.map((section, index) => (
          <section key={section.id} id={section.id} className="mb-10 scroll-mt-20">
            <h2 className="text-2xl font-semibold text-foreground mb-4 border-b border-border pb-2">{index + 1}. {t(section.titleKey)}</h2>
            ...
            {section.tableKey && (
              <div className="overflow-x-auto">
                <Table className="my-4">
                  <TableHeader>
                    <TableRow>
                      {(t(`${section.tableKey}.headers`, { returnObjects: true }) as string[]).map(header => <TableHead key={header}>{header}</TableHead>)}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
            ...
            {section.ctaKey && <p className="text-muted-foreground mt-4">{t(section.ctaKey)}</p>}
             {section.id === 'why-us' && ( // Specific CTA for this section
              <div className="mt-6">
