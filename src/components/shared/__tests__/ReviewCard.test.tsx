...
describe('ReviewCard Component', () => {
  it('renders review information correctly', () => {
    ...
    expect(screen.getByText('"This is an excellent service!"')).toBeInTheDocument();
  ...
  it('truncates long quotes when maxQuoteLength is set', () => {
    const longQuote = 'This is a very long review that should be truncated at some point';
  ...
  it('applies correct size variants', () => {
    const { rerender } = render(<ReviewCard review={mockReview} size="sm" data-testid="review-card" />);
    let card = screen.getByTestId('review-card');
    expect(card).toHaveClass('px-4', 'py-4', 'text-sm');

    rerender(<ReviewCard review={mockReview} size="lg" data-testid="review-card" />);
  ...
  it('applies variant styles correctly', () => {
    const { rerender } = render(
      <ReviewCard review={mockReview} variant="elevated" data-testid="review-card" />
    );
    let card = screen.getByTestId('review-card');
    expect(card).toHaveClass('shadow-md');

    rerender(<ReviewCard review={mockReview} variant="flat" data-testid="review-card" />);
    card = screen.getByTestId('review-card');
