...
import { CardComponentProps, ClassNameHelpers, setDisplayName } from '@/lib/component-standards';

export interface ReviewCardProps extends Omit<CardComponentProps, 'title' | 'description'> {
  review: Review;
  size?: 'sm' | 'md' | 'lg';
  showStars?: boolean;
...
const ReviewCard = React.memo<ReviewCardProps>(function ReviewCard({
  ...
  const truncatedQuote = maxQuoteLength && review.quote.length > maxQuoteLength
  ...
  const reviewLocation = showLocation && review.location ? review.location : null;
