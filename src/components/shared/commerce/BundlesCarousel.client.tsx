...
export default function BundlesCarousel({ max = 6 }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { loadBundles } = await import('@/lib/firestore/bundles').catch(
        () => ({ loadBundles: null }),
      );
    ...
    <section className="w-full max-w-6xl mx-auto mt-8">
      ...
      <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-2">
        {bundles.map((b) => (
          ...
          >
            <Card className="card-hover min-w-[260px] w-64 shrink-0 border border-border shadow-sm">
              {b.imageUrl && (
                <Image
                  src={b.imageUrl}
                  alt={b.name}
