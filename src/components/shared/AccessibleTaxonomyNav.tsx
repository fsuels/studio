...
const AccessibleTaxonomyNav: React.FC<AccessibleTaxonomyNavProps> = ({
  ...
  const navigateHorizontally = (direction: number) => {
    ...
    const categories = containerRef.current.querySelectorAll(
      '[data-category], [data-situation]',
    );
    const currentCategory = (document.activeElement as HTMLElement)?.closest(
      '[data-category], [data-situation]',
    );

    if (!currentCategory) return;

    const currentIndex = Array.from(categories).indexOf(
      currentCategory as Element,
    );
  ...
  return (
    <div
      ...
      aria-label={
        ariaLabel ||
        t('accessibility.documentNavigation', {
          defaultValue: 'Document navigation',
        })
      }
