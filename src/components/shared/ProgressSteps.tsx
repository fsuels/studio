    ...
    <nav aria-label="Progress" className="mb-8">
        ...
        <div className="w-full overflow-x-auto scrollbar-hide pb-2">
          ...
          >
            ...
            {Array.from({ length: total }, (_, i) => i + 1).map((step) => {
              ...
              return (
                <li key={step} className="relative flex-shrink-0">
                  {' '}
                  {/* Items should not shrink, allow natural width */}
      ...
      <div className="sm:hidden mt-4 h-1.5 w-full bg-muted rounded-full overflow-hidden">
        <div
          ...
          style={{
            width: `${Math.max(0, (current - 1) / Math.max(1, total - 1)) * 100}%`,
          }}
        />
      </div>
    </nav>
