...
export default function ReEngagementTools({ docId, totalQuestions = 0, locale }: Props) {
  ...
  useEffect(() => {
    ...
    const expiry = storedExpiry ? parseInt(storedExpiry, 10) : Date.now() + 10 * 60 * 1000
    if (!storedExpiry) localStorage.setItem('promoCTAExpiry', expiry.toString())
    setShowCTA(true)
    ...
    <>
      {showCTA && minutes > 0 && (
        ...
        >
          ...
          <button onClick={handleDismissCTA} aria-label="Dismiss" className="ml-4">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
      {progress > 0 && progress < 100 && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 md:left-4 md:translate-x-0 z-40 bg-background border border-border p-3 rounded shadow">
          <p className="text-xs mb-2">You’ve completed {Math.round(progress)}% of your document</p>
