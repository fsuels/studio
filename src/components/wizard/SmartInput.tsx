...
const SmartInput = React.memo(
  React.forwardRef<HTMLInputElement, SmartInputProps>(
    ({ rhfProps, type, className, ...rest }, forwardedRef) => {
      const { setValue, watch, trigger } = useFormContext();

      const {
        name: rhfName,
        ref: rhfRegisterRefFromProps,
        onChange: rhfInternalOnChangeFromProps,
        onBlur: rhfOnBlurFromProps,
        ...otherRhfProps
      } = rhfProps;
      ...
      const [displayValue, setDisplayValue] = useState<string>(() => {
        const initialVal =
          typeof watchedRHFValue === 'string' ? watchedRHFValue : '';
      ...
      useEffect(() => {
        ...
        const newDisplayValue =
          type === 'tel' ? formatPhoneNumber(currentRHFValue) : currentRHFValue;
      ...
      const handleInputChange = useCallback(
        (event: React.ChangeEvent<HTMLInputElement>) => {
          ...
          setValue(rhfName, valueToSetInRHF, {
            shouldValidate: true,
            shouldDirty: true,
          });
