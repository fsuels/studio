...
jest.mock('../HeaderSearch', () => {
  return function MockHeaderSearch({ clientLocale, mounted, className }: any) {
    return <div data-testid="header-search">Search Component {mounted ? 'mounted' : 'not mounted'}</div>
  }
})

jest.mock('../HeaderUserMenu', () => {
  return function MockHeaderUserMenu({ clientLocale, mounted }: any) {
    return <div data-testid="header-user-menu">User Menu {mounted ? 'mounted' : 'not mounted'}</div>
  }
})

jest.mock('../HeaderMegaMenu', () => {
  return function MockHeaderMegaMenu({ clientLocale, mounted, isMegaMenuOpen, onOpenChange }: any) {
    return (
      <div data-testid="header-mega-menu">
        Mega Menu {isMegaMenuOpen ? 'open' : 'closed'}
        <button onClick={() => onOpenChange(!isMegaMenuOpen)}>Toggle Mega Menu</button>
      </div>
    )
  }
})
...
jest.mock('../HeaderMobileMenu', () => {
  return function MockHeaderMobileMenu({ clientLocale, mounted, isMobileMenuOpen, onToggle, onClose }: any) {
    return (
...
jest.mock('@/components/layout/Nav', () => {
  return function MockNav({ locale }: { locale: string }) {
    return <div data-testid="nav">Nav for {locale}</div>
  }
})
