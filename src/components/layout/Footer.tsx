...
const Footer = React.memo(function Footer() {
  ...
  const { t: tFooter } = useTranslation("common"); // All footer keys are in common.json under "footer"
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    if (!email.includes('@')) {
      toast({
        title: tFooter('footer.errorInvalidEmailTitle', { defaultValue: 'Invalid Email' }),
        description: tFooter('footer.errorInvalidEmailDesc', { defaultValue: 'Please enter a valid email address.' }),
        variant: 'destructive',
      });
      return;
    }
    ...
    toast({
      title: tFooter('footer.subscribedSuccessTitle', { defaultValue: 'Subscribed!' }),
      description: tFooter('footer.subscribedSuccessDesc', { defaultValue: 'Thanks for joining our mailing list.' }),
    });
    setEmail('');
    setIsLoading(false);
  };
  ...
  const footerLinks = [
    {
      sectionTitleKey: 'footer.sectionProductTitle', // Key for "Product" section title
    ...
    {
      sectionTitleKey: 'footer.sectionResourcesTitle', // Key for "Resources" section title
    ...
    {
      sectionTitleKey: 'footer.sectionLegalTitle', // Key for "Legal" section title
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs">
              {tFooter('subhead', { defaultValue: 'AI-Powered Legal Document Generation. Fast, Affordable, Secure.'})}
            </p>
          </div>
          ...
          {footerLinks.map((section) => (
              <div key={section.sectionTitleKey}>
                <h4 className="font-semibold text-foreground mb-3">
                  {tFooter(section.sectionTitleKey, { defaultValue: section.defaultSectionTitle })}
                </h4>
          ...
          <div>
            <h4 className="font-semibold text-foreground mb-3">
              {tFooter('footer.sectionCommunityTitle', {defaultValue: "Community & Contact"})}
            </h4>
            <div className="flex space-x-3 mb-4">
              <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer"
                 aria-label={tFooter('footer.socialLinkedInAria', {defaultValue: "LinkedIn Page"})}
                 className="text-muted-foreground hover:text-primary transition-colors">
                <Linkedin className="h-5 w-5" />
              </a>
              <a href="https://twitter.com" target="_blank" rel="noopener noreferrer"
                 aria-label={tFooter('footer.socialTwitterAria', {defaultValue: "Twitter Profile"})}
                 className="text-muted-foreground hover:text-primary transition-colors">
            ...
            <div className="text-xs space-y-1 mb-6">
              <p className="flex items-center gap-1.5">
                ...
                <span>{tFooter('footer.supportPhoneNumber', {defaultValue: "+1 (800) 555-0199 (Support)"})}</span>
              </p>
              <p className="flex items-center gap-1.5">
                <Clock className="h-3.5 w-3.5 text-primary/80"/>
                <span>{tFooter('footer.supportHours', {defaultValue: "Mon–Fri, 9am–6pm EST"})}</span>
              </p>
            </div>

            <h4 className="font-semibold text-foreground mb-3">
              {tFooter('footer.getCreditsTitle', {defaultValue: "Get 3 Free Credits"})}
            </h4>
            <form onSubmit={handleSubscribe} className="flex gap-2 items-center">
              <div className="relative flex-grow">
                ...
                <Input
                  type="email"
                  placeholder={tFooter('footer.emailPlaceholder', {defaultValue: "Enter your email"})}
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  className="pl-10 pr-2 py-2 h-12 text-base bg-background border-input"
                  aria-label={tFooter('footer.emailPlaceholderAria', {defaultValue: "Email for newsletter"})}
                  disabled={isLoading}
                />
              </div>
              <Button type="submit" size="icon"
                      className="bg-primary text-primary-foreground hover:bg-primary/90 h-12 w-12 shrink-0"
                      aria-label={tFooter('footer.subscribeButtonAria', {defaultValue: "Subscribe to newsletter"})}
                      disabled={isLoading}>
                {isLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <Send className="h-5 w-5" />}
              </Button>
            </form>
            <p className="text-xs mt-2">
              {tFooter('footer.joinMailingListDesc', {defaultValue: "Join our mailing list for exclusive offers."})}
            </p>
          </div>
        </div>
        ...
        <div className="border-t border-border pt-6 text-center text-xs">
          {tFooter('footer.copyright', { year: currentYear, defaultValue: `© ${currentYear} 123LegalDoc. All rights reserved.` })}
        </div>
      </div>
      ...
      {isHydrated && (
        <div className="fixed bottom-4 right-4 z-50">
          <Button variant="outline" size="icon"
                  ...
                  aria-label={tFooter('footer.openChatAria', { defaultValue: "Open chat" })}>
