...
export default function PromissoryNoteDisplay({
  ...
  const handleStartProcess = () => {
    ...
    const itemName = t('metaTitle', {
      defaultValue: 'Promissory Note Template & How-To Guide | 123LegalDoc',
    });
    const priceCents = 500; // Assuming a base price for Promissory Note
    track('add_to_cart', {
      item_id: 'promissory-note',
      item_name: itemName,
  ...
  const informationalSections: Section[] = [
    {
      ...
      itemsKey: 'sections.whatIs.items',
      type: 'list',
      lastParagraphKey: 'sections.whatIs.lastParagraph',
    },
    {
      id: 'when-to-use',
      titleKey: 'sections.whenToUse.title',
      tableKey: 'sections.whenToUse.table',
      type: 'table',
    },
    {
      id: 'varieties',
      titleKey: 'sections.varieties.title',
      itemsKey: 'sections.varieties.items',
      type: 'list',
    },
    {
      id: 'clauses',
      titleKey: 'sections.clauses.title',
      tableKey: 'sections.clauses.table',
      type: 'table',
    },
    {
      id: 'how-to-complete',
      titleKey: 'sections.howToComplete.title',
      itemsKey: 'sections.howToComplete.items',
      totalTimeKey: 'sections.howToComplete.totalTime',
      type: 'ordered-list',
    },
    {
      id: 'state-rules',
    ...
    {
      id: 'supporting-docs',
      titleKey: 'sections.supportingDocs.title',
      itemsKey: 'sections.supportingDocs.items',
      type: 'list',
    },
    {
      id: 'why-us',
  ...
  const renderSectionContent = (section: Section) => {
    ...
    if (section.type === 'list' && section.itemsKey) {
      ...
      const lastParagraph = section.lastParagraphKey
        ? t(section.lastParagraphKey)
        : null;
      if (Array.isArray(items)) {
        return (
          <>
            ...
            {lastParagraph && (
              <p className="text-muted-foreground mt-2">{lastParagraph}</p>
            )}
          </>
        );
      }
    ...
    if (section.type === 'table' && section.tableKey) {
      const headers = t(`${section.tableKey}.headers`, {
        returnObjects: true,
      }) as string[];
      const rows = t(`${section.tableKey}.rows`, {
        returnObjects: true,
      }) as string[][];
      return (
              ...
              <TableRow>
                {Array.isArray(headers) &&
                  headers.map((header: string) => (
                    <TableHead
                      key={header}
                      className="text-foreground bg-muted/50 font-semibold"
                    >
                      {header}
                    </TableHead>
                  ))}
  ...
  return (
    <div className="container mx-auto px-4 py-12">
          ...
          >
            ...
            <AccordionContent className="px-6 pb-4 pt-0 text-muted-foreground">
              <div className="prose prose-sm dark:prose-invert max-w-none text-foreground">
                {' '}
                {/* Ensure text-foreground for prose children */}
      ...
      <section className="text-center py-8 bg-secondary/30 rounded-lg border border-border">
        ...
        >
          <Link
            href={`/${locale}/docs/promissory-note/start`}
