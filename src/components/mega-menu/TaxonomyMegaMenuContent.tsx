...
const TaxonomyMegaMenuContent: React.FC<TaxonomyMegaMenuContentProps> = ({ 
  ...
  const [docPreviews, setDocPreviews] = React.useState<Record<string, DocPreview>>({});
  ...
  const loadDocPreview = React.useCallback(async (slug: string) => {
  ...
  const getPopularDocsForSituation = React.useCallback((situationKey: string) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      {doc?.popular && <Star className="h-3 w-3 text-yellow-500" />}
                    </div>
...
const SituationCard: React.FC<{
  ...
  const popularDocs = React.useMemo(() => getPopularDocs(situationKey), [situationKey, getPopularDocs]);
              ...
              return (
                ...
                >
                  ...
                  {doc?.complexity && (
                    <Badge 
                      variant="secondary" 
                      className={cn("text-xs px-1 py-0", getComplexityColor(doc.complexity))}
                    >
                      {doc.complexity}
                    </Badge>
                  )}
