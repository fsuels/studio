...
- `src/lib/document-library.ts` orchestrates lazy loading through the metadata registry and enrichment utilities. Prefer `getAllDocuments()` or `getDocumentLibrary()` when you need fully hydrated `LegalDocument` objects.
...
4. The resulting `LegalDocument` objects are cached per jurisdiction and merged with `documentLibraryAdditions` for globally available templates.
...
## Agent Tips
- Add new templates by copying an existing jurisdiction folder, updating metadata, and registering the document in the metadata registry (or via `scripts/generate-document-library.js`).
- Run `npm run verify-templates` after editing schemas or metadata to catch missing fields.
- Use the fixtures in `tests/fixtures/` when writing regression tests; avoid sampling the entire library at runtime to keep suites deterministic.
