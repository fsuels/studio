...
- Baseline capture plan: (1) add log-based metric in Stackdriver/Sentry filtering `[DocumentGen]` with labels `operation`, `status`, `durationMs`; (2) run smoke requests for top 5 docs (wizard + API) and confirm entries for `api.generatePdf`, `service.generatePdfDocument`, and `pdf.*` operations; (3) export aggregated counts (success/error) and P95 duration into ops dashboard (Looker/Datastudio) and share snapshot; (4) schedule daily job to compare metrics to success ≥99.5% / latency ≤3s targets, alert if breached.
